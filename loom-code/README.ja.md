...
⚠️ Codex CLI manifest は build 済みで Claude Code 変体と同期して v0.9.0 までバンプ済み、しかし実 Codex CLI 環境での install + 検証 ritual はユーザ指示により延期中。準備ができたら [`tests/codex-cli/README.md`](tests/codex-cli/README.md) を参照。
...
| # | Skill | Stage | 何をするか |
|---|---|---|---|
| Router | [`using-loom-code`](skills/using-loom-code/) | Always-on | SessionStart 自動注入；5 つの load-bearing rules + Skill Priority テーブル |
...
| **Codex CLI** | ⚠️ Manifest は build + v0.9.0 までトラック済；実機 install + 検証 ritual はユーザ指示により延期中（`tests/codex-cli/README.md` 参照） |
