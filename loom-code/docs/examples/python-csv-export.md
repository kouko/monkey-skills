...
> **Demonstrates**: full loom-code flow from brainstorming through finish-branch
> **Skills exercised**: `using-loom-code` → `brainstorming` → `writing-plans` → `subagent-driven-development` (with implementer + spec-reviewer + code-quality-reviewer triad) → `tdd-iron-law` → `requesting-code-review` → `verification-before-completion` → `finishing-a-development-branch`
...
Router auto-routes to `brainstorming` (Stage 1). Agent walks the 5-axis framework BEFORE jumping to implementation. The brief lands at `docs/loom/specs/2026-05-16-csv-export.md`:
...
**Agent halts here**: brief written, asks user to approve before proceeding to plan-writing. *"Brief at `docs/loom/specs/2026-05-16-csv-export.md`. Confirm scope before I invoke writing-plans?"*
...
Router routes to `writing-plans`. Reads the brief, produces atomic plan at `docs/loom/plans/2026-05-16-csv-export.md`:
...
## What this example demonstrates about loom-code

| Skill | What you saw |
|---|---|
| `using-loom-code` | Auto-injected at session start; routed to brainstorming on the user's open-ended prompt |
...
- [`../../skills/using-loom-code/SKILL.md`](../../skills/using-loom-code/SKILL.md) — router charter the agent loads
