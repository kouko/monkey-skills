...
#
# Phase 2.5 v0.4.0 build deliverable. Confirms loom-code is
...
#
# Usage:
#   bash loom-code/tests/codex-cli/test-skill-loading.sh
...
To install + run this test:
  ...
  codex plugin marketplace add /path/to/monkey-skills/.worktrees/loom-code-design
...
# -------------------------------------------------------------------------
# Step 1 — verify loom-code is installed

if ! codex plugin list 2>&1 | grep -q "loom-code"; then
  cat <<'EOF'
FAIL: loom-code not installed in Codex CLI.
...
echo "OK: loom-code installed"

# -------------------------------------------------------------------------
# Step 2 — confirm loom-code is installed AND enabled
...
if ! echo "${LIST_OUTPUT}" | grep -E 'loom-code' | grep -qi 'enabled'; then
  echo "FAIL: loom-code not shown as enabled in 'codex plugin list':"
  echo "${LIST_OUTPUT}" | grep -E 'loom-code' || echo "  (no loom-code row found)"
  exit 1
fi

echo "OK: loom-code installed + enabled"
...
# loom-code ships; verifying each is loadable requires opening a
