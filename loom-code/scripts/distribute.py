...
``domain-teams:code-team`` into ``loom-code`` skills as functional copies.
...
Layout:
    ...
    functional copy (this plugin)
      loom-code/skills/<skill>/{standards,rubrics,checklists}/<file>
...
3. ``loom-code/scripts/verify-drift.py`` runs in CI and fails on byte diff.
...
# loom-code/ — parent of scripts/
ROOT = Path(__file__).resolve().parent.parent

# monkey-skills/ repo root — parent of loom-code/, sibling of domain-teams/
...
# functional-copy destinations (relative to loom-code/).
#
...
#
# Plugin-level agent files in ``loom-code/agents/`` carry one or more
# canonical text blocks between BEGIN/END markers. The canonical text lives
# once in ``loom-code/scripts/_*.md``; ``distribute_agent_injections``
...
AGENT_BASELINE_BEGIN = (
    ...
    "loom-code/scripts/distribute.py from "
    "loom-code/scripts/_baseline.md — do not edit in place -->"
)
...
AGENT_REVIEWER_DISCIPLINE_BEGIN = (
    ...
    "loom-code/scripts/distribute.py from "
    "loom-code/scripts/_reviewer-discipline.md — do not edit in place -->"
)
...
AGENT_RULE_SHEET_BEGIN = (
    ...
    "loom-code/scripts/distribute.py from "
    "loom-code/scripts/_rule-sheet.md — do not edit in place -->"
)
...
def distribute_agent_injections() -> int:
    ...
    for agent_rel in AGENT_BASELINE_TARGETS:
        dst = ROOT / agent_rel
        if not dst.is_file():
            raise FileNotFoundError(f"agent missing: loom-code/{agent_rel}")
        ...
        if rebuilt != current:
            ...
            print(
                f"[deploy-agent-injections] loom-code/{agent_rel} "
                f"({' + '.join(tags)})"
            )
    return written
