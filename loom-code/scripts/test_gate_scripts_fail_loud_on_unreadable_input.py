...
EXEMPT = {
    ...
    "codex_scaffold.py": "writes a repo's .codex hook scaffold and probes "
                         "it; reads no backlog store or handoff brief, and "
                         "wraps its CLI in a fail-closed exit-2 handler",
    "loom_checker.py": "provisional — every rule runs under one fail-closed "
                       "handler (exit 2, no traceback) pinned by "
                       "test_loom_checker_cli.py; promote to FAMILY once its "
                       "store reads have per-rule unreadable-input cases",
    ...
    "propose_review_batches.py": "proposes one plan's review batches, "
                                 "not a backlog store or handoff brief",
