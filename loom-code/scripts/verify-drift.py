...
Repair locally with ``python3 loom-code/scripts/distribute.py``.
...
def main() -> int:
    ...
    for canonical_subpath, dests in ROUTE.items():
        ...
        for rel_dst in dests:
            dst = ROOT / rel_dst
            if not dst.is_file():
                failures.append(f"MISSING            loom-code/{rel_dst}")
                continue
    ...
    for agent_rel in AGENT_BASELINE_TARGETS:
        dst = ROOT / agent_rel
        if not dst.is_file():
            failures.append(f"MISSING-AGENT      loom-code/{agent_rel}")
            continue
        try:
            expected_text = expected_agent_text(agent_rel)
        except ValueError as e:
            failures.append(f"INJECTION-MARKERS  loom-code/{agent_rel}: {e}")
            continue
        ...
        if actual_bytes == expected_bytes:
            continue
        failures.append(
            f"INJECTION-DRIFT    loom-code/{agent_rel}\n"
    ...
    if failures:
        ...
        print(
            ...
            f"\nFix: python3 loom-code/scripts/distribute.py"
        )
        return 1
    print(
