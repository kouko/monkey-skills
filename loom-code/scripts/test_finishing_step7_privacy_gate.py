...
def test_step7_names_public_cross_plugin_skill():
    """Step 7 must name the public privacy-gate skill, rather than paths
    private to the independently installable loom-workflow plugin."""
    step7 = _step7_slice(_text())
    assert "loom-workflow:git-memory" in step7, \
        "Step 7 must delegate the privacy gate through git-memory's public skill"
