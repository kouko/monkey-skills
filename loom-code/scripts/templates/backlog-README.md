...
repo has a `docs/loom/PURPOSE.md`. A repo with no such file is NOT
silently exempt — betting on a COMMITTED-NEXT entry there prompts the
author to write one first, rather than skipping the field. Where
required, the value must take one of a closed two-form grammar:
`serves: unrelated — <reason>` (em dash; the reason clause after
`unrelated` is mandatory — a bare `unrelated` with no reason is a
violation), or `serves: <any other non-empty text>` describing the
...
line accumulates in the entry's body rather than opening a new entry
per arc, so one theme's whole history stays readable in one place.
...
copy shipped inside the loom-code plugin, resolved from wherever the
plugin is installed. The commands:
...
by `--write`, and is detected as drift by `--check`.

## Archive rule

Close an entry with the loom-code plugin's `archive_change_folder.py`
(shipped beside `backlog_index.py` in the plugin's `scripts/`
directory), not a hand `mv` — the script validates every path before
touching the filesystem and rolls back on a stamp failure, which a
manual move does not:

```
python3 <loom-code plugin>/scripts/archive_change_folder.py <identifier> [root] [--date YYYY-MM-DD] [--unit folder|file]
```
...
In repos that also adopt the loom family practice-memory store
(`docs/loom/memory/`), that store deletes stale facts outright and
relies on git history as the archive — see its own README. **This
backlog store does the opposite on purpose**: it archives (moves +
stamps), and never deletes. The two stores answer different questions —
memory asks "what do we currently believe", where a superseded fact
should stop being read at all, while this backlog asks "what did we
decide to do, and when did we close it", where the closed item itself
is the record. A reader of one store's charter should not assume the
other store's policy applies — they are different, and that is
intentional.
