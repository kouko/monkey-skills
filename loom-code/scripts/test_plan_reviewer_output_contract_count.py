...
def test_check_id_range_upper_bound_matches_highest_applicable_check():
    ...
    applicable check id in the table. Would fail today (RED): the line
    still reads `<1-4, 6-14, 16-17>`, omitting Check 18."""
    text = _text()
    ...
    assert str(highest) in check_id_line, (
        f"gaps.check_id range must include the highest applicable check "
...
def test_needs_revision_mapping_upper_bound_matches_highest_applicable_check():
    ...
    state; both now must say up to 18). Would fail today (RED)."""
    text = _text()
    highest = _applicable_ids(text)[-1]
    ...
    assert str(highest) in needs_revision_line, (
        f"NEEDS_REVISION verdict-mapping line must include the highest "
