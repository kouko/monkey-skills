...
def test_plugin_version_and_changelog_at_0_98_1():
    ...
    assert '"version": "0.98.1"' in plugin_text, (
        "loom-code/.claude-plugin/plugin.json must read version 0.98.1"
    )

    changelog_text = CHANGELOG_MD.read_text(encoding="utf-8")
    assert "## [0.98.1]" in changelog_text, (
        "loom-code/CHANGELOG.md must carry a `## [0.98.1]` heading"
    )
