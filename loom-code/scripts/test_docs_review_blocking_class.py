...
def test_plugin_version_and_changelog_at_0_79_0():
    ...
    plugin.json is bumped to 0.79.0 and CHANGELOG.md carries a matching
    `## [0.79.0]` heading. Both read from the
    ...
    assert '"version": "0.79.0"' in plugin_text, (
        "loom-code/.claude-plugin/plugin.json must read version 0.79.0"
    )

    changelog_text = CHANGELOG_MD.read_text(encoding="utf-8")
    assert "## [0.79.0]" in changelog_text, (
        "loom-code/CHANGELOG.md must carry a `## [0.79.0]` heading"
    )
