...
def test_plugin_version_and_changelog_at_0_44_0():
    ...
    declaration hardening, then the 2026-08-02 two-unit archive script):
    plugin.json is bumped to 0.44.0 and CHANGELOG.md carries a matching
    `## [0.44.0]` heading. Both read from the WORKING TREE, never a
    ...
    assert '"version": "0.44.0"' in plugin_text, (
        "loom-code/.claude-plugin/plugin.json must read version 0.44.0"
    )

    changelog_text = CHANGELOG_MD.read_text(encoding="utf-8")
    assert "## [0.44.0]" in changelog_text, (
        "loom-code/CHANGELOG.md must carry a `## [0.44.0]` heading"
    )
