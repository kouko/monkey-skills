...
def test_plugin_version_and_changelog_at_0_105_0():
    ...
    assert '"version": "0.105.0"' in plugin_text, (
        "loom-code/.claude-plugin/plugin.json must read version 0.105.0"
    )

    changelog_text = CHANGELOG_MD.read_text(encoding="utf-8")
    assert "## [0.105.0]" in changelog_text, (
        "loom-code/CHANGELOG.md must carry a `## [0.105.0]` heading"
    )
