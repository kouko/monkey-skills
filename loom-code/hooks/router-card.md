...
5. **Research before asking.** Non-trivial design / strategy / tech-stack question to the user MUST cite WebSearch findings (2-4 industry approaches w/ sources). *"X or Y?"* without industry context = violation. Use `brainstorming` Axis 4 protocol for the research. Delivery floor: non-trivial decision asks default to the `AskUserQuestion` tool (its triage card rides the call); a prose ask must LEAD with a one-line state anchor + stakes — a bare prose fork question is the same violation, just invisible to the hooks. **Before you fire that ask**: if the fork is genuinely complex (≥3 trade-offs, ≥2 implementation paths, or architectural blast radius), run `loom-workflow:brief-before-asking` FIRST — brief, then ask, never the reverse.
