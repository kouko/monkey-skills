...
|---|---|---|---|---|---|
| 0 | `using-loom-code` | router | 1 | planned | `using-superpowers` |
...
- ✅ `loom-code/TECH-SPEC.md`
- ✅ `loom-code/ROADMAP.md`
- ⏳ `loom-code/.claude-plugin/plugin.json`（skeleton）
- ⏳ `loom-code/.codex-plugin/plugin.json`（skeleton）
- ⏳ `loom-code/README.md` / `.ja.md` / `.zh-TW.md`（skeleton）

**Q-lock**（8 項；見 PRODUCT-SPEC §8）：
- Q1 harness = Claude Code + Codex CLI
- Q2 code-team 並存
- Q3 先寫設計文件
- Q4 worktree `feat/loom-code-design`
...
| Router | `skills/using-loom-code/{SKILL.md, README×3, references/{codex-tools.md, claude-code-tools.md}}` |
...
- 開新 session → `using-loom-code` 自動載入（透過 hook）
...
| # | Item | 來源 | 狀態 |
|---|---|---|---|
| P15-1 | `hooks/session-start` 加 `LOOM_CODE_MODE=off` 退場條件 | Phase 0 規劃 | ✅ 已 ship（commit 9cba15c，Phase 1 一併做了） |
...
| P15-12 (Phase 1) | Plugin-level agent files (`loom-code/agents/`) + 12-rule engineering baseline SSOT (`scripts/_baseline.md`) + `distribute.py` / `verify-drift.py` extension for in-file baseline-block sync. Phase 1 scope: 1 agent (`implementer.md`) as proof-of-mechanism. | User adopted 12-rule CLAUDE.md template + asked "為何不全部塞 agent file"。Active-enforcement need: baked-into-system-prompt > passive reference doc. SSOT-and-functional-copy pattern extended from whole-file (existing) to in-file section (new). | ✅ 已 ship（v0.5.2，ritual PASS — (a) plugin-level agent resolved cleanly via `Agent({subagent_type:"loom-code:implementer"})`; (b) baseline visible in system prompt (agent quoted Rule 1 + Rule 12 verbatim); (c) returned BLOCKED rather than NEEDS_CONTEXT — stronger than predicted, agent correctly discriminated "ask-and-proceed" vs "external-state-must-change"; bonuses: refused "work without stopping" reminder as fabrication cover, cited TDD-iron-law cross-skill content unprompted） |
...
| Tool mapping | `skills/using-loom-code/references/codex-tools.md`（完整 + ⚠️ TBD verify markers） | ✅ build done |
...
- ⏳ Codex session 啟動 → using-loom-code 自動載入（待 ritual）
...
**Acceptance**：
- 與 Superpowers 並存（LOOM_CODE_MODE=on）測試通過
- 與 Superpowers 並存（LOOM_CODE_MODE=off）測試通過
...
| SSOT drift | Edit to skills/*/standards/ etc. | `loom-code/scripts/verify-drift.py` |
| Skill structure | New skill or subfolder | `scripts/check-skill-structure.py` |
| Plugin manifest validity | Any change to a skill's frontmatter / plugin.json | `claude plugin validate loom-code` |
...
claude plugin validate loom-code

# 2. 重裝到 local (30 秒) — 必跑
claude plugin uninstall loom-code
claude plugin install loom-code@monkey-skills --scope local

# 3. 1 個壓測 prompt (~90 秒) — 必跑；新 skill 的核心措辭至少壓一次
#    從 loom-code/tests/{skill-triggering,tdd-iron-law-pressure}/prompts/
...
| Q4 | 2026-05-15 | worktree `feat/loom-code-design` | 0 | None |
...
| P1-A | tbd | using-loom-code ≤2000 tokens | 1 | Low |
