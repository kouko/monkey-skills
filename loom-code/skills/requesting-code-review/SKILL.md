...
2. **Resolve the dispatch profile** in [`dispatch-profile.md`](../using-loom-code/references/dispatch-profile.md), then **dispatch TWO `code-reviewer` subagents in parallel, with byte-identical prompts** (role `loom-code:code-reviewer`; plugin-level agent at [`code-reviewer.md`](../../agents/code-reviewer.md)). The two dispatches are issued as one-shot calls — Claude issues both `Agent()` calls in one message — but because each arm carries a stable `name:`, neither call auto-returns; the orchestrator actively polls each named arm via `SendMessage` to collect its round-1 verdict, and this polling IS Step 3's "wait for both" step. **Sanctioned carve-out to "must not name async teammates"** ([environment gotchas](../using-loom-code/references/environment-gotchas.md) A1): dispatch each arm with a stable `name:` (`code-review-arm-a` / `code-review-arm-b`) — A1's actual exemption condition is being "prepared to actively poll it via `SendMessage` for the result," which both this round-1 retrieval and the ledger-driven convergence loop's rounds-2+ re-dispatch (Step 6; [`references/review-loop-convergence.md`](references/review-loop-convergence.md)) satisfy. Open each prompt verbatim with **"You ARE the reviewer"**. Give both the full immutable context packet, diff range, absolute rubric/checklist paths, branch context, the profile's dispatch record, and identical conditional additions. Neither arm derives plugin paths/version from `target_repo`; both carry [`loom-code/scripts/_baseline.md`](../../scripts/_baseline.md).
...
6. **If the user fixed findings and wants re-review, run the ledger-driven convergence loop — read [`references/review-loop-convergence.md`](references/review-loop-convergence.md) before dispatching any confirmation; its rules are binding.** Round 1's two arms are dispatched with stable `name:` (`code-review-arm-a` / `-b`) recorded in a per-finding ledger. Rounds 2+ dispatch only to arms holding open entries, as an inherited delta confirmation (`SendMessage` to the SAME named arm with a post-fix packet); the orchestrator — never the arm — maps its ordinary verdict to `CONFIRMED_RESOLVED` (only when every original finding assigned to that arm is closed) or `STILL_BLOCKING` + reason, and an entry closes only on a verbatim post-fix quote plus which clause it satisfies. All entries resolved → converge and mint a schema-valid terminal wrapper; any `STILL_BLOCKING` with a cycle left → next cycle for open-entry arms only; `STILL_BLOCKING` at the cap (round 1 plus at most two delta-confirmation cycles) → quality STOP surfaced to the user. New gating findings on a confirmation round are admissible only inside the fix diff; anything else is non-gating debt recorded in `docs/loom/backlog/`. A fresh full two-arm round may replace a cycle (escalation valve) but never closes an open entry on its own. Session death, compaction, or a lost handle before confirmation → one fresh whole-diff round 1, disclosed as never delta-confirmed — never a ledger flip.
...
- [`references/review-loop-convergence.md`](references/review-loop-convergence.md) — the ledger-driven delta-confirmation loop (Step 6): per-finding ledger, named-arm carve-out, the two-cycle cap, delta admissibility, the escalation valve, terminal-wrapper minting, and the lost-handle restart rule.
