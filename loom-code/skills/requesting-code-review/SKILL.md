...
2. **Dispatch a `code-reviewer` subagent** (role identifier `loom-code:code-reviewer`; plugin-level agent, v0.6.0 / P15-12 Phase 2, at [`loom-code/agents/code-reviewer.md`](../../agents/code-reviewer.md)) to review the branch diff. Dispatch it as a one-shot, blocking call that waits for and returns the verdict directly — see your host's tool-mapping reference under `using-loom-code/references/` (`claude-code-tools.md` / `codex-tools.md`) for the exact call shape, and [environment-gotchas](../using-loom-code/references/environment-gotchas.md) §A1 for a Claude-Code-specific mistake to avoid (naming the dispatch call turns it into an async mailbox teammate whose output is never delivered — Codex has no equivalent pitfall). **Open the dispatch prompt with the role anchor from the agent's §Input contract — "You ARE the reviewer" — verbatim**; "review request" phrasing without it can role-confuse the dispatched agent into acting as an orchestrator ("I've dispatched the review" — to nobody). The orchestrator passes: diff range, paths to rubrics + checklists, branch context (recent commits, related issues if known). The agent carries the 12-rule engineering baseline ([`loom-code/scripts/_baseline.md`](../../scripts/_baseline.md)) baked into its system prompt.
...
3. **Wait for verdict — then mint the gate marker**. Reviewer returns structured review with per-dimension scores, severity-tagged findings, and overall verdict. Save the verdict text to a temp file and run `python3 <plugin-root>/scripts/loom_gate_markers.py review-pass --verdict-file <file>` (resolve `<plugin-root>` as `../..` from this skill's base dir). The script validates the §Verdict structure schema BEFORE writing `.git/loom/review-pass.json`; `NEEDS_REVISION` or a malformed verdict refuses to mint (exit 3/4) — a failed review can never produce a pass marker. The marker binds the current HEAD sha: any later commit/amend invalidates it, and the `hooks/git-guard.py` PreToolUse gate blocks `git push` / `gh pr create` until a fresh verdict re-mints at the new HEAD (review what you ship, not what you reviewed an amend ago).
