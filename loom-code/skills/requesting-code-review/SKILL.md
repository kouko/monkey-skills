...
**Complex remediation fork → brief before you ask.** A finding can open a genuine design fork (e.g. an architectural 🔴 with two viable remediations). When that fork is complex (≥3 trade-offs, ≥2 implementation paths, or architectural blast radius), do not compress it into a fix/defer/merge ask — run `loom-workflow:brief-before-asking` (6-block briefing, Mental Model first) before the `AskUserQuestion`. Same trigger as `brainstorming`'s rule — `brainstorming` carries the canonical trigger rule; `loom-workflow:brief-before-asking` owns the 6-block format.

**Boundary — those rules govern the relay TO the user ONLY.** They do **not** touch what the `code-reviewer` emits. Its structured verdict MUST stay machine-precise and keep every evidence citation — do NOT loosen R2 ([`code-reviewer.md`](../../agents/code-reviewer.md) §Rule R2: every finding needs `where:` with a path + anchor (verbatim string or stable heading), or a commit SHA. A line number is optional precision; missing evidence flips the verdict to `NEEDS_REVISION`). Plain language is for the human relay; reviewer output stays exact.
...
At the start of each review round (round 1 included), run `python3 scripts/plan_card.py <plan-path> --set-stage "review:round-N"` — the script prints the refreshed card; relay it — and commit the flip with that round's verdict or fixes; hand-edit only when the script is absent. When only the repo-root copy is missing, use the active host adapter's installed plugin root and run `python3 <installed-plugin-root>/scripts/plan_card.py <plan-path> --set-stage "review:round-N"`; "absent" means neither copy is present.

1. **Determine diff scope, then route by file type**. First use the active host's immutable review-context adapter — [`claude-code-tools.md`](../using-loom-code/references/claude-code-tools.md) or [`codex-tools.md`](../using-loom-code/references/codex-tools.md) — to derive `<installed-plugin-root>` from the loaded reference path and run `python3 <installed-plugin-root>/scripts/review_context.py --repo <path>` once. Never derive the root from a consumer repository, working directory, cache, or host environment variable. Keep its full JSON packet unchanged: `target_repo`, `reviewed_sha`, `plugin_version`, and absolute `resources` paths. Resolve scope through the packet's `review_scope.py` resource (`resources.review_scope`) — `python3 <resources.review_scope> --repo <target_repo> --reviewed-sha <reviewed_sha>` — or, if the user specifies one, an explicit commit range (`git diff <SHA1>..<SHA2>`). An explicit range is eligible for this gate only when the explicit range endpoint equals `reviewed_sha`; otherwise it REFUSES before dispatch or marker minting, because a verdict for an older endpoint cannot authorize the current packet. Exit 0 prints the changed-file list, one path per line; a non-zero exit is a refusal, with the reason (and, on a stale base, a ready-to-run `git rebase --onto` remedy) on stderr.
...
2. **Resolve the dispatch profile** in [`using-loom-code`'s portable profile](../using-loom-code/references/dispatch-profile.md), then **dispatch TWO `code-reviewer` subagents in parallel, with byte-identical prompts** (a panel; role identifier `loom-code:code-reviewer`; plugin-level agent at [`loom-code/agents/code-reviewer.md`](../../agents/code-reviewer.md)) to review the branch diff. Each dispatch is a one-shot, blocking call that waits for and returns its own verdict directly — see your host's tool-mapping reference under `using-loom-code/references/` (`claude-code-tools.md` / `codex-tools.md`) for the exact per-host call shape (Claude Code: issue both `Agent()` calls in **one** assistant message so they run concurrently), and [environment-gotchas](../using-loom-code/references/environment-gotchas.md) §A1 for a Claude-Code-specific mistake to avoid (naming a dispatch call turns it into an async mailbox teammate whose output is never delivered — Codex has no equivalent pitfall). **Open each dispatch prompt with the role anchor from the agent's §Input contract — "You ARE the reviewer" — verbatim**; "review request" phrasing without it can role-confuse the dispatched agent into acting as an orchestrator ("I've dispatched the review" — to nobody). Both prompts receive the full immutable context packet from Step 1, diff range, absolute rubric/checklist paths, branch context (recent commits, related issues if known), and the profile's dispatch record — "byte-identical" means identical **to each other**; conditional additions (e.g. the PRINCIPLES.md path below) go to both. The packet is copied verbatim, so no arm derives plugin paths or version from `target_repo`. Both agents carry the 12-rule engineering baseline ([`loom-code/scripts/_baseline.md`](../../scripts/_baseline.md)) baked into their system prompt.
...
4. **Harvest the deliberate-simplification ledger**. Before producing the review summary, inspect the `LOOM-SIMPLIFY:` markers that record deliberate, scope-bounded shortcuts the branch shipped. Reuse the file list `review_scope.py` already resolved in Step 1, but read every path from the immutable packet snapshot rather than the mutable working tree:
   ...
   Record hits in `simplification_ledger`; each needs checkable `ceiling:`, `upgrade:`, and `ref:`, or becomes a finding. No hits means an explicitly empty ledger. A missing path or failed snapshot read is evidence failure: record it as a finding and use the packet snapshot, never the mutable working tree or current HEAD. The convention is in [`../subagent-driven-development/standards/deliberate-simplification.md`](../subagent-driven-development/standards/deliberate-simplification.md). Only after those checks, if the panel verdict is `PASS` or `PASS_WITH_NOTES` with a valid, empty simplification ledger, run `python3 <resources.gate_markers> review-pass --repo <target_repo> --verdict-file <file> --expected-head <reviewed_sha>`. An R3 downgrade alone does not block this marker path; a simplification finding prevents this marker path. The script validates the verdict before writing and refuses if the packet target's HEAD drifts from `reviewed_sha`; `NEEDS_REVISION` or malformed input also refuses. `loom_gate_markers.py validate --verdict-file <file>` is dry-run. The marker binds `reviewed_sha`; any later commit needs a fresh verdict.
