...
1. **Determine diff scope**. Default: `git diff main...HEAD` (everything on this branch not on main). Override with explicit commit range if user specifies (`git diff <SHA1>..<SHA2>`).
   - **Docs-only dispatch mode:** when the diff is non-empty AND every file in the branch diff (`git diff main...HEAD --name-only`) ends in `.md`, add this to both reviewer dispatches — any non-`.md` file in the diff keeps the default code path above unchanged: (a) **whole-artifact scope** — each reviewer reads every changed artifact whole, the diff only as context, and asks explicitly: does any UNCHANGED claim in this file contradict the change, or the current code? (an unchanged line in a document is an untouched line, not a correct one — documents have no tests); (b) score these five prose dimensions in place of the code-shaped ones for this dispatch — **omission** (an obligation or referent the text needs and lacks), **ambiguity** (an absolute — "only", "never", "zero" — without support), **inconsistency** (two passages contradicting, including changed-vs-unchanged), **incorrect-fact** (a citation that does not support its claim — open the source), **missing population** (a measured number without its denominator or scope); (c) run `python3 loom-code/scripts/check_doc_citations.py <changed .md files>` first and fold its output into the dispatch packet; (d) every finding in this dispatch carries `class: instruction | evidence` — **instruction**: text a reader or executor will act on (a rule, a step, an acceptance criterion, a prescribed command or path, a citation used as an instruction), e.g. a bullet instructing an implementer to derive `kpi_id` from a canonical field slug while the shipped code does the opposite (audit §2); **evidence**: a narrative claim about what happened or is true (a measurement, an absolute, a provenance attribution, a citation supporting a claim), e.g. a claim attributed to a source section that does not state it — the audit attributed a variant to the brief's §Users, while §Users says "three comparative years" with no statement-type distinction, so the attribution was wrong (audit §4.3); a finding whose class is unclear is tagged `instruction` (fail closed). The measured 0% false-positive rate is scoped to the `docs/loom/{audits,specs,plans}` population; documents quoting tool output or citing deliberately-broken examples (dogfood notes, fixtures) trigger false findings, so reviewers treat pre-pass findings inside fenced code blocks, blockquotes, table cells, and inline examples as advisory, not as defects.
...
findings:
  - severity: 🔴 fatal | 🟡 should-fix | 🟢 nit
    ...
    class: instruction | evidence              # docs-mode dispatches only — §Process Step 1(d); omitted for code-branch findings
...
**Docs-only mode**: the rule above is unchanged — docs mode selects what is fed into it, not its thresholds. Apply it to instruction-class findings only (§Process Step 1(d)); evidence-class findings are carried into the verdict as recorded observations that do not gate. A finding missing `class:` counts as instruction (fail closed), consistent with a finding missing `where:` flipping the whole verdict. An evidence-class finding against narrative prose the branch left UNCHANGED (§Process Step 1(a)) must be superseded by an appended correction naming what it replaces, never edited in place.
