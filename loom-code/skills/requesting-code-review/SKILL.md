...
3. **Offer to run review now**: "Dispatching `requesting-code-review` — back in ~30s." Then resolve scope via the resolver — `review_scope.py`, the same call §Process Step 1 makes — or an explicit commit range if the user specified one, and run the §Process panel (Steps 2-3: two reviewers, union, re-aggregate) on that scope; this entry point uses the same panel default, never a single-reviewer dispatch. A refusal STOPS here, before any dispatch — see §Pinned refusal contract under §Process Step 1.
