...
2. **Dispatch code-reviewer subagent** via `Agent({subagent_type: "loom-code:code-reviewer", prompt: <branch + diff body>})` — **unnamed** (no `name:` — see [environment-gotchas](../using-loom-code/references/environment-gotchas.md) §A1, adding `name:` turns this into a mailbox-semantics teammate whose output is never delivered; `description:` is still required as always and is unrelated to this gotcha) — plugin-level agent (v0.6.0 / P15-12 Phase 2) at [`loom-code/agents/code-reviewer.md`](../../agents/code-reviewer.md). **Open the dispatch prompt with the role anchor from the agent's §Input contract — "You ARE the reviewer" — verbatim**; "review request" phrasing without it can role-confuse the dispatched agent into acting as an orchestrator ("I've dispatched the review" — to nobody). The orchestrator passes: diff range, paths to rubrics + checklists, branch context (recent commits, related issues if known). The agent carries the 12-rule engineering baseline ([`loom-code/scripts/_baseline.md`](../../scripts/_baseline.md)) baked into its system prompt.
