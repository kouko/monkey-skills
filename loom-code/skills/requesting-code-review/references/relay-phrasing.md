...
3. **Numbers and symbols carry their meaning.** Translate `🔴/🟡/🟢`, `PASS_WITH_NOTES`, and the Beck / Martin / OWASP / 徳丸本 citations into one plain sentence ("nothing blocking, two things worth a look before merge") — don't dump the raw verdict block at the user. When conversation language is not English, the [`adjudication-view`](../../using-loom-code/protocols/adjudication-view.md) protocol (verdict mode) is the mechanism for the user-facing findings rendition.
