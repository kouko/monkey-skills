...
> contract — see [`loom-code/hooks/plain-relay.md`](../../../hooks/plain-relay.md)
...
3. **Numbers and symbols carry their meaning.** Translate `🔴/🟡/🟢`, `PASS_WITH_NOTES`, and the Beck / Martin / OWASP / 徳丸本 citations into one plain sentence ("nothing blocking, two things worth a look before merge") — don't dump the raw verdict block at the user. The [`adjudication-view`](../../using-loom-code/protocols/adjudication-view.md) protocol (verdict mode) is the mechanism for the user-facing findings rendition, firing whenever that protocol's own §Firing conditions are met.
...
7. **Relay via the shared family card.** Open the relay with the family rollup card per `loom-code/hooks/family-relay.md §Family relay discipline` — don't restate its rules here. Walk findings at the user's pace, not as a dump; ≥2 remediation options default to a markdown comparison table; when a finishing gate-STOP already led with the progress card, the rollup opens after it — the two compose, never duplicate.
