...
**Queue-relation gate (unconditional):** (fires at intake, alongside the On-ramp choice gate, before drafting Task 1) run `python3 loom-code/scripts/check_direction_freshness.py <brief-path>` on the source brief. Exit 0 means the brief's `## Queue relation` line resolves. Exit 1 means the brief path is unreadable. Exit 2 means the line is unresolved — STOP: do not draft, relay the printed question verbatim to the user, wait for their answer, write it into the brief's `## Queue relation` line, then re-run. The unlanded-direction-change advisory this script prints on every run, including exit 0, is not itself a gate — it never blocks. Grammar SSOT: [`../brainstorming/references/handoff-brief-format.md`](../brainstorming/references/handoff-brief-format.md).
