...
**Field-microstructure gate (unconditional):** before dispatching the reviewer, also run `python3 loom-code/scripts/check_field_microstructure.py <plan-path>` — on every plan, never conditional. It runs a second time in `--brief <brief-path>` mode against the source brief, checking that document's own paragraph-length rule. A non-zero exit from either run blocks the plan-document-reviewer dispatch: fix the flagged field or paragraph and re-run. When the plan-mode run fails, fix the plan directly — it is yours to fix. When the brief-mode run fails on a brief you did not author, the fix still happens before you dispatch the reviewer: STOP planning, fix the brief (the same STOP-and-fix discipline as the On-ramp choice gate above), and re-run — do not draft a plan on top of a brief this gate has flagged, and do not treat it as the brief-writer's problem to catch later.
