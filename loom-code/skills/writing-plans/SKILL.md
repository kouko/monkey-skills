...
**Proposal-status intake gate (unconditional, intake):** when the brief names a source proposal, before drafting Task 1 run `python3 loom-code/scripts/check_proposal_status.py <proposal-path>`. Exit 2 → STOP: do not draft; the proposal's `Status:` line is not `ratified — <name>, <YYYY-MM-DD>`; relay the printed question to the user, wait, ratify the proposal, re-run. Exit 1 → STOP (proposal missing or unreadable; fix the path or permissions).
