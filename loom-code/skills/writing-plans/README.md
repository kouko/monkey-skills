...
- **Brief item covered**: one traceability referent, in any of three accepted forms — a quote / reference from the brief's Smallest End State / Decision, a loom-spec change-folder join key, or a `BI-<n>` identifier the brief declares. A task that delivers no brief outcome (release administration, a version bump) writes `none — <reason>`; the reason is mandatory. The exact grammar, the tie-break when a task could cite two items, and the identifier rules live in [`references/plan-format.md`](references/plan-format.md) §`Brief item covered`
