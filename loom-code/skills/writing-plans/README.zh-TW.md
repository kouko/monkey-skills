...
- **Description**: 命令式第一行控制在 300 字元上限內。同一個上限也原封不動套用在每個巢狀項目自己折行後的文字上——沒有逐項豁免。計畫表頭的 `Goal:` 那一行不受字元上限限制，只保留不可有巢狀本文這條規則。超出的部分再放進更深一層的巢狀項目或表格 — 詳見[`references/plan-format.md`](references/plan-format.md) §`Field-value grammar`
