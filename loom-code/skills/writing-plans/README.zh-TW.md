...
- **Description**: 命令式第一行控制在 300 字元上限內。同一個上限也原封不動套用在每個巢狀項目自己折行後的文字上——沒有逐項豁免——計畫表頭的 `Goal:` 那一行也共用這同一個 300 字元上限，不是另一組會脫鉤的獨立限制。超出的部分再放進更深一層的巢狀項目或表格 — 詳見[`references/plan-format.md`](references/plan-format.md) §`Field-value grammar`
...
- **Brief item covered**: 一個 traceability referent，三種形式擇一 — 引 brief 的 Smallest End State / Decision 對應段落、loom-design change-folder 的 join key、或 brief 宣告的 `BI-<n>` 識別碼。不交付任何 brief 成果的任務（release 行政作業、版本 bump）寫 `none — <reason>`，理由是必填。完整文法、一個任務同時對到兩個 item 時的 tie-break、以及識別碼規則見 [`references/plan-format.md`](references/plan-format.md) §`Brief item covered`
