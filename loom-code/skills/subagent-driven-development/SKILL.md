...
3. **If `status: DONE` or `DONE_WITH_CONCERNS`**, resolve the installed plugin root through the active host adapter and run `python3 "<installed-plugin-root>/scripts/review_context.py" --repo <target_repo>` **once per reviewer fan-out**. Treat its output as one unchanged immutable context packet: `target_repo`, `reviewed_sha`, `plugin_version`, and `resources`. `resources` is a map of approved absolute paths; never derive plugin paths from `target_repo`, the working directory, or a consumer checkout. Copy this packet verbatim into every reviewer dispatch, including a retry after an implementation change (which starts a fresh fan-out and therefore a fresh packet). Use the approved packet resource `review_scope` only: `python3 "<review_scope>" --repo <target_repo> --reviewed-sha <reviewed_sha>`. If review_scope exits non-zero, REFUSE the fan-out: do not dispatch any reviewer. The only reviewer artifact scope is the file list printed by review_scope for that packet; do not derive a base or recompute a diff. Give every spec-reviewer, code-quality-reviewer, and docs-reviewer prompt that SHA-bound scope result, the same packet, and the immutable repository citation cross-read contract: `git -C "<target_repo>" show <reviewed_sha>:<path>`. Read all reviewer evidence from paths at `<reviewed_sha>`, never a later mutable HEAD. Do not use mutable working-tree reads for reviewer evidence. Then dispatch **`spec-reviewer`** and **`code-quality-reviewer`** **in parallel** (same fan-out step — see `dispatching-parallel-agents` §3) — both plugin-level agents (v0.6.0 / P15-12 Phase 2), role identifiers `loom-code:spec-reviewer` and `loom-code:code-quality-reviewer`. See [`loom-code/agents/spec-reviewer.md`](../../agents/spec-reviewer.md) + [`loom-code/agents/code-quality-reviewer.md`](../../agents/code-quality-reviewer.md) for input/output contracts. Wait for both. Worktree-isolated dispatch: see [`references/dispatch-hygiene-notes.md`](references/dispatch-hygiene-notes.md).
