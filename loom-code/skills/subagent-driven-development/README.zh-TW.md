...
`using-loom-code` 偵測到下列任一條件成立時自動 routing：
...
| `code-quality-reviewer` | evaluator | PASS / PASS_WITH_NOTES / NEEDS_REVISION + 八維度評分 + findings (🔴 / 🟡 / 🟢) | artifact / rubrics / checklist / 9 standards | 只產 verdict |
...
2. 同 commit 跑 `python3 loom-code/scripts/distribute.py`。
3. CI 跑 `loom-code/scripts/verify-drift.py` 強制 byte-identical。
