...
**1. Hard cap: 2 review rounds.** After round 2 ends with NEEDS_REVISION (PASS and PASS_WITH_NOTES are both passing verdicts and end the review) → STOP and surface the surviving findings to the user. Hand them the decision. A third round runs ONLY on explicit user authorization — never silently.
...
Fail-closed on purpose, and the asymmetry is **visibility, not cost**. An unbounded fallback round is genuinely expensive: it re-opens the pre-existing pool this contract exists to stop resampling, it can gate on findings that have nothing to do with the fixes, and under Directive 1's cap it can consume a round. But every one of those costs is visible, and you can decide about them. A wrongly-guessed range suppresses findings inside the span it excluded and leaves nothing behind to notice — the reviewer never saw them, so no one can weigh what was lost.
