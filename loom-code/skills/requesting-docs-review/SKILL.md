...
**Where the range comes from — the round has to leave itself a marker.** `delta-scoped: <commit range>` is `<the previous round's `reviewed_sha`>..HEAD`. The sha is not the branch base, not the merge-base, and not recoverable from the diff — the round that reviewed it is the only thing that knows it, which is why `reviewed_sha:` is REQUIRED in every panel verdict (§Verdict structure) and surfaced with it (Step 5). **Within one session that is enough**: the orchestrator carries it from round to round.

**Across a session boundary it is not yet wired, and this contract will not pretend otherwise.** No step persists a round's verdict to a known path, and `.git/loom/origin-ledger.json` records only rounds where someone ran `loom_gate_markers.py review-pass` — which nobody does on the `NEEDS_REVISION` verdict that is precisely what triggers a round 2. So a resuming session usually has nothing to read. **When no prior `reviewed_sha` can be found, this round is `unbounded`** — never a guessed range (`docs/loom/backlog/2026-08-04-a-delta-scoped-round-cannot-resume-across-a-session.md`).
