...
**Where the range comes from — the round has to leave itself a marker.** `delta-scoped: <commit range>` is `<the sha the previous round was dispatched against>..HEAD`, so **every dispatch records the sha it reviewed** (Step 3) and the next round reads it back. Nothing else in this contract carries that sha: it is not the branch base, not the merge-base, and not recoverable from the diff. **If the previous round's sha was not recorded — a handoff, a new session, an orchestrator that skipped Step 3's record — this round is `unbounded`.** Fail-closed on purpose: an unbounded round costs a wider read, a wrongly-guessed range silently suppresses findings inside the part of the delta it excluded, and a suppressed finding leaves no trace to notice.
