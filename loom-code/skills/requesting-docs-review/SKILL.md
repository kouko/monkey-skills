...
- **Directive 2 — Host-specific terminal confirmation.** After a fix, first bind the post-fix SHA with a complete immutable context packet. Claude Code sends the delta only to the SAME reviewer via `SendMessage`, never a fresh whole-corpus re-sample; Codex performs a labelled fresh whole-artifact review through its host adapter. Both return `CONFIRMED_RESOLVED` or `STILL_BLOCKING` + reason for the post-fix packet. `STILL_BLOCKING` after this one cycle → STOP; surface the finding and the reviewer's reason to the user — no second cycle without explicit user authorization.
