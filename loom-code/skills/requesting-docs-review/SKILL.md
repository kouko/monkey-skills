...
**Where the range comes from — the round has to leave itself a marker.** `delta-scoped: <commit range>` is `<the previous round's `reviewed_sha`>..HEAD`. That field is REQUIRED in every panel verdict (§Verdict structure) and is the only carrier of the sha that outlives the session producing it — it is not the branch base, not the merge-base, and not recoverable from the diff. A round resuming after a handoff reads the previous round's verdict artifact and takes the range from there. **When no prior `reviewed_sha` can be found, this round is `unbounded`** — never a guessed range.
...
reviewed_sha: {the HEAD sha this round reviewed — REQUIRED; Directive 2's
              next-round range starts here, and this field is the only
              carrier that survives the session it was produced in}
