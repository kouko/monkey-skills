...
6. **Delta confirmation — only when round 1 raised a gating verdict.** Once the surviving instruction-class findings are fixed, dispatch the SAME `docs-reviewer` subagent instance(s) that raised each one — via `SendMessage` to that same session, never a fresh `Agent` dispatch — scoped to the delta only (Directive 2). Each confirms `CONFIRMED_RESOLVED` or `STILL_BLOCKING` + reason. `CONFIRMED_RESOLVED` is terminal (Directive 3) — report "no gating findings" or "confirmed resolved," never "clean." `STILL_BLOCKING` after this one cycle STOPs the review: surface the finding and the reviewer's reason to the user: no second confirmation cycle, and no fallback to a fresh round 1, without their explicit authorization.
