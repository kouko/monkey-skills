...
**Across a session boundary the READ side is not wired.** A durable record does exist — minting a marker appends `head_sha` to `<git-common-dir>/loom/origin-ledger.json` — but no step here tells a resuming orchestrator to consult it, and it holds only the last **minted** round, not the last round: a `NEEDS_REVISION` round mints nothing, so the entry can be several rounds stale. Reading it anyway is safe and worth doing, because a stale sha yields a range WIDER than the true one, which is the direction this contract already requires. **When no prior `reviewed_sha` can be found at all, this round is `unbounded`** — never a guessed range (`docs/loom/backlog/2026-08-04-a-delta-scoped-round-cannot-resume-across-a-session.md`).
