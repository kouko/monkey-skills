...
- **(a) Fix, then run one delta-scoped verification round** — *the default recommendation*. It reads whole artifacts but raises only against the fixes (Directive 2), so it costs a fraction of a full round. That cost drop is what makes it the default. The authorized verification round receives the surviving findings it verifies (Directive 2's round-N handoff).
- **(b) Fix and ship without re-review.** State the risk concretely, never as a general caution: **a fix round is where defects get written.**
...
5. **Surface to user.** Present the verdict, its `reviewed_sha:` (the next round's range starts there), the instruction-class findings (these gated), and the evidence-class findings (recorded observations). Do NOT auto-fix — remediation is the user's decision, even for a one-word nit.
6. **Round 2 — only if the user fixed and wants re-review.** Fresh subagents, same panel shape, packet per Directive 2 (round-1 findings verbatim; fix-verification duty first). After round 2, Directive 1 governs: a qualifying round-2 verdict shape triggers the one automatic delta-scoped round 3, reported in the rollup; every other extra round needs explicit user authorization.
...
|---|---|---|
| *"Just one more round to be safe."* | The cap IS the design. What "to be safe" means here is waiting for an empty round, and for an artifact with many small real defects that state is not reachable — so it cannot be what you wait for. An authorized extra round is a different thing and is not what this row refuses: Directive 1 makes it delta-scoped by default, so it terminates; what it still cannot do is certify the artifact clean. The automatic delta round is not what this row refuses either: it runs on Directive 1's mechanical conditions, not on anyone's "to be safe". | Refuse. Directive 1: only the mechanically-qualifying round-2 shape gets the one automatic delta round; any other shape STOPs and surfaces surviving findings, and a further round needs explicit user authorization. Round-2 findings are presumed real — decide them on severity, never discount them as round artifacts. |
...
| **Upstream orchestrator** | `finishing-a-development-branch` | Invokes `requesting-code-review` as its review step; a bounded-cap STOP from this skill surfaces to the user instead of entering the silent fix→re-review loop |
...
- Does **not** run rounds beyond the bounded cap on its own authority — the one auto-delta round runs only when Directive 1's mechanical conditions all hold; a fourth round needs explicit user authorization (Directive 1).
