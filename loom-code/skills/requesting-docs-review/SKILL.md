...
- **Directive 1 — Bounded cap.** 2 review rounds, plus at most one automatic delta round when round 2's structured verdict meets three mechanical conditions; PASS and PASS_WITH_NOTES are both passing verdicts that end review; any other round-2 shape STOPs and surfaces the surviving findings to the user; a fourth round runs only on explicit user authorization (measurement provenance: [`references/design-evidence.md`](references/design-evidence.md), author-facing).
- **Directive 2 — Round-N handoff.** Round N's dispatch packet carries round N-1's surviving findings verbatim; reviewers verify each prior finding against quoted current text before raising anything new (no re-litigating a closed finding in new words); from round 2 onward what a reviewer may raise narrows to the delta — plus any contradiction in either direction between the delta and unchanged text — while what it reads stays whole-artifact.
- **Directive 3 — Oscillation stop.** A finding that resurfaces after being fix-verified ends the loop immediately — surface it to the user as an oscillation, not as a routine finding.
- **Directive 4 — Appended corrections.** Evidence-class fixes for prose the branch left unchanged are appended corrections naming what they replace — never in-place rewrites.
