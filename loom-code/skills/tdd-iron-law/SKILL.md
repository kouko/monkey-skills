...
|---|---|---|
| **Throwaway / spike** | Unfenced case: code you will delete within the same session, never commit, never reference again. Fenced case: committed prototype work is exempt ONLY on a `prototype/<map-id>/<ticket-slug>` branch whose ticket exists — `docs/loom/maps/<map-id>/tickets/<ticket-slug>.md` with `type: prototype`, in a checker-valid map (loom-workflow:decision-map map-format §Live-map criterion) — a reviewer-checkable artifact, not a branch name the author chooses. Inside that fence the exemption follows the prototype contract's own lifecycle instead of the same-session test: committed, retained read-only, cited as primary source, never merged. | "I'll clean it up later." That's not throwaway — that's tests-last. |
