...
`using-loom-code` が実装作業（新機能 / バグ修正 / リファクタ / マイグレーション）を検知した時点で自動呼び出し。さらに `subagent-driven-development` の implementer サブエージェント内部でも自己呼び出しされる。
...
[`standards/tdd-standard.md`](standards/tdd-standard.md) は `domain-teams/skills/code-team/standards/tdd-standard.md` のバイト一致 functional copy に 5 行の SSOT ヘッダを付けたもの。ドリフトは `loom-code/scripts/verify-drift.py` が監視。規律を変更する場合、`domain-teams:code-team` 側の canonical を編集し、同じコミットで `loom-code/scripts/distribute.py` を実行する。
...
- [`../using-loom-code/SKILL.md`](../using-loom-code/SKILL.md) — ルーター（この skill の流れ上の位置）。
