...
**git-memory is never waived by an exemption.** These exemptions waive the close-out *orchestration* (review / verification / PR), not `loom-workflow:git-memory` (P3-D). git-memory is a mandatory gate before *every* commit — even a trivial direct-to-main commit goes through it (it will usually return "no trailers, routine," and the invocation itself is the discipline). Skipping the close-out flow does NOT mean skipping git-memory.
