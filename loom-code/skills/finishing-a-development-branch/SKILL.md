...
8. git hygiene before the close-out commit:
   - Close-out sub-checks — all orchestrator-only, ONCE per branch; a parallel wave
     ...
     | Review-due reminder | The repo has `docs/loom/backlog/`. | Run `python3 scripts/backlog_index.py --review-due --as-of YYYY-MM-DD` with an explicit as-of date during close-out; relay the due entries for human review. No CI wiring — add it only after an observed miss. | No store → skip silently. |
