...
8. git hygiene before the close-out commit:
   ...
   - **Purpose-linked betting.** Never list or promote a bet on automatic close-out. If the user explicitly asks, print `docs/loom/PURPOSE.md` verbatim first; after promotion, run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/check_north_star_link.py" <backlog-store>`. Exit 0 proceeds; any nonzero output is STOP-and-ask, then record the answer where that output directs and re-run. Status semantics remain in `docs/loom/backlog/README.md`.
