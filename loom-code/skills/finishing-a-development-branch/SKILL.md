...
8. git hygiene before the close-out commit:
   - Close-out sub-checks — all orchestrator-only, ONCE per branch; a parallel wave
     ...
     | Map-parts check | The closing plan carries a decision-map join key — grep the plan's `## Notes` section for the literal ` / Part: ` infix (the binding line's exact form is `Map part: <map-id> / Part: <name>`, produced per `loom-workflow:decision-map`'s map-format §Parts rule); no match found = unbound, never inferred from topic similarity. | BEFORE the close-out commit, invoke the `loom-workflow:decision-map` skill's parts flipper with the part's join key and the branch's last content commit (HEAD as it stands before this close-out commit is added — deliberately not Step 9c's post-commit "final HEAD" convention, since the flipped MAP.md is itself staged into the close-out commit) to flip that part's Parts row; then stage the flipped `docs/loom/maps/<map-id>/MAP.md` into THIS close-out commit (explicit file list, matching sibling rows). This mechanical flip supersedes hand-kept multi-plan progress tables. | No join key in the plan, or the maps store/skill absent → say loudly, never silently skip: "map-parts: N/A — <reason>". |
