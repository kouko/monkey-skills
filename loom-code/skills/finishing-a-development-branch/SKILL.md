...
8. git hygiene before the close-out commit:
   - Close-out sub-checks — all orchestrator-only, ONCE per branch; a parallel wave
     ...
     | Adversarial audit | Step 3.5's code or prose signal fired (§Step 3.5). | Relay `adversarial audit: fired — reproduced k / held m / n-a j` VERBATIM into the close-out card AND the plan's `## Notes`; its `reproduced` STOP gate is `python3 loom-code/scripts/check_attack_catalogue.py docs/loom/ATTACK-CATALOGUE.md --repo <root>` (Step 3.5). | Store absent, or no signal fired → `adversarial audit: N/A — <reason>`. |
     | Cold reader | Step 3.5's prose signal fired (§Step 3.5). | Relay `cold reader: fired — <scenario verdict> / <temptation verdict>` VERBATIM into the close-out card AND the plan's `## Notes`. | No signal fired → `cold reader: N/A — <reason>`. |
