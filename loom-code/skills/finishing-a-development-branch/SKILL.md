...
9c. Gate markers at the FINAL HEAD — the `hooks/git-guard.py` PreToolUse gate blocks
    ...
    - Mint the review marker at the final HEAD (save the Step-3 verdict text to a file,
      ...
      INDEX regen, memory trailers, plan Stage flips — this branch's `--set-stage
      "finishing"` and any stale-scan same-shape fixes — and backlog status flips
      with their regenerated index). ANY substantive post-verdict change → re-dispatch
