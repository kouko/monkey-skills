...
**Live-map check** — if `docs/loom/maps/` exists and holds at least one map dir, invoke `loom-workflow:decision-map` to assess liveness (the liveness criterion and its validate gate are owned by that skill; do not restate its internals here). A live map is surfaced with the recommendation to resume its work-through via `loom-workflow:decision-map` before opening a new brief for overlapping scope. This check is independent of the Negative guard (a live map matters to any kickoff, including a bug-fix/refactor arc). No `docs/loom/maps/` store, no live map found, or `loom-workflow:decision-map` unavailable on this host → a loud N/A line stated in the session conversation (no brief field carries it; the brief schema is unchanged by design); never a silent skip.

**Direction banner** — always print direction at kickoff, never a silent skip. Print one line quoting `docs/loom/PURPOSE.md`'s `**Why:**` line verbatim; no `docs/loom/PURPOSE.md` → print the loud line `direction banner: PURPOSE.md absent` instead; `docs/loom/PURPOSE.md` exists but has no `**Why:**` line → print the loud line `direction banner: PURPOSE.md has no **Why:** line` instead. Then print one line per live map pairing map-id with its Destination first line, reusing the pair the Live-map check's `loom-workflow:decision-map` §Liveness assessment delegation already returns (any map-side absence case → the Live-map check's N/A line already covers it; the banner adds nothing map-side). This check is independent of the Negative guard (direction matters to any kickoff, including a bug-fix/refactor arc). Always-on, one line per item, no state kept, no new parsing.
...
| Axis 0 Live-map check finds a live map | `loom-workflow:decision-map` | Liveness assessment + work-through resume; avoids opening a new brief for overlapping scope. |
