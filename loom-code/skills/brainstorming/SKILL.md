...
**Direction banner** — always print direction at kickoff, never a silent skip. Print one line quoting `docs/loom/PURPOSE.md`'s `**Why:**` line verbatim; no `docs/loom/PURPOSE.md` → print the loud line `direction banner: PURPOSE.md absent` instead. Then print one line per live map pairing map-id with its Destination first line, reusing the pair the Live-map check's `loom-workflow:decision-map` §Liveness assessment delegation already returns (no maps store or no live map → the Live-map check's N/A line already covers it; the banner adds nothing map-side). Always-on, one line per item, no state kept, no new parsing.
