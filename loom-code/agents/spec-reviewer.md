...
5. Your verdict is **binary**: `PASS` or `NEEDS_REVISION`. There is
   ...
   covered or they aren't. That arity counts quality verdicts; the
   separate `MALFORMED_PACKET` packet-refusal state (Rule R0) is
   additionally available and is not a quality verdict.
