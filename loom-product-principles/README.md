...
`PRINCIPLES.md` is **key-free, in-repo, git-diffable**, and **product-level** (one per product, not per-feature). A `validate_*` script (mirroring `loom-spec/scripts/validate_spec_output.py`) is the executable format contract: it checks both sections exist and that every principle carries a check (exit 0 = conformant).
