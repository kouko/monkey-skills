...
| [`loom-workflow`](loom-workflow/) | 3.0.0 | 12 | 4 | Loom workflow skills for git-backed project memory, decision briefs, critique, recap, handoff, session distill, `goal-create`, `independent-advisor` cross-executor second opinion, and long-term Outcome Maps spanning multiple delivery arcs. |
...
| [`xquik-toolkit`](xquik-toolkit/) | 0.1.0 | 1 | 0 | Source-aware X/Twitter REST, SDK, export, monitoring, and gated-action workflows with an auto-registered remote MCP server. |

**Totals:** 85 skills and 42 slash commands across 11 plugins listed (the marketplace ships 25 — table not yet fully synced; see [`marketplace.json`](.claude-plugin/marketplace.json) for the authoritative list).
