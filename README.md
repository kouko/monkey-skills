...
| [`loom-workflow`](loom-workflow/) | 4.1.4 | 10 | 4 | Loom workflow tools for git-backed project memory, `critique` (proposal / complexity), recap, handoff, session distill, `goal-create`, `independent-advisor` cross-executor second opinion, and long-term Outcome Maps whose delivery arcs are intents. |
| [`loom-memory`](loom-memory/) | 0.1.0 | 0 | 0 | Optional, passively-triggered repository-memory capability with an OKF v0.2-compatible store profile. Installs and works standalone — no dependency on loom-code, loom-design, or loom-workflow. Claude Code + Codex. |
