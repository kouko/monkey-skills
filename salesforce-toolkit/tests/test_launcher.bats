...
#
#   1. salesforce-mcp on PATH → exec salesforce-mcp "$@"
...
# received) and exit code. No real salesforce-mcp / npx invocation.
...
# and `bash`, but `salesforce-mcp` / `npx` resolve only when the test placed
...
@test "(a) salesforce-mcp on PATH → exec salesforce-mcp with forwarded args" {
  make_stub "$TMPDIR_TEST/salesforce-mcp"
  # No npx stub in $TMPDIR_TEST → proves selection is salesforce-mcp first.
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"STUB salesforce-mcp --orgs DEFAULT_TARGET_ORG --toolsets data,metadata"* ]]
}
...
@test "(c) neither salesforce-mcp nor npx → exit 127 + stderr mentions sf-setup" {
