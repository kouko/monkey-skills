...
| モード | 内容 | 用途 |
|---|---|---|
| **Shared**（デフォルト） | `--profile <name>` で日常使い Chrome のログイン状態を再利用 | 単一ユーザー・単一マシン — 最速セットアップ |
| **Dedicated**（`--dedicated`） | `~/.local/share/collab-toolkit/profiles/dedicated/` の単一統合プロファイル。Google SSO がサービス間でカスケード → 5 サービス全部で通常 2-3 回のログインで完了。セットアップは Claude 主導（ターミナル操作不要）。 | Shared profile の cookies が転送できない、マルチアカウント、日常 Chrome cookies からの分離、マルチマシン移植 |
