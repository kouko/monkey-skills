...
- **Adopted across every flag-mode gate surface**: `copywriter-evaluator.md` Mode-2 aggregation + prior-findings-check gains precedence over its own stale count-based verdict rules; `copywriting-form-check-stage` SKILL + rubric (rows annotated `Class:`, count-based Verdict Rules replaced); `copywriting-voice-quadrant-stage` + `copywriting-voice-tone-stage` SKILL gate passages; `copywriting-voice-tone-stage/rubrics/voice-consistency-gate.md` (rows annotated + verdict rules replaced + new DIVERGED header); `copywriting-neta-injection/rubrics/neta-safety-gate.md` (same treatment; its SKILL verdict enum aligned to the three-valued schema enum); `copywriting-audit-stage` variant re-gating aggregation. The two checklist-mode gates (Intake Completeness, ethics FATAL/FIXABLE) are exempt per the vocabulary's own carve-out — checklist mechanics unchanged.
- **Tier-2 provenance amendment**: `CLAUDE.md` §Provenance & Divergence Principle gains a scoped "Exception — plugin-owned gate verdict rules" sanctioning this plan-mandated verdict-rules replacement (a rubric's `## Verdict Rules` block is plugin-owned enforcement mechanics, not upstream editorial prose — the additive-only + raise-upstream discipline is unchanged everywhere else). Logging the three rubric verdict-rules replacements (form-appropriate-gate, voice-consistency-gate, neta-safety-gate) here satisfies the exception's CHANGELOG-logging requirement.
...
- **Schema SSOT pointer** (`skills/using-copywriting-toolkit/SKILL.md` §Handoff Envelope Schema): the router's illustrative envelope example omitted `retries`, `audit_trail[]`, and `express_mode_used` — fields `CLAUDE.md` §Handoff Envelope / §Immutable fields — preservation contract declares mandatory/immutable, so the two copies had silently drifted. Added the three fields to the illustration and a pointer line naming CLAUDE.md as the canonical schema (point, don't copy — the router no longer restates per-field semantics).
- **CI structure scan wired + CHK-SKL-011 provenance exemption**: added `copywriting-toolkit` to `.github/workflows/skill-structure.yml`'s structure job. Running `check-skill-structure.py` against it surfaced 13 CHK-SKL-011 hits (`domain-teams/skills/copywriting-team/...` substrings) — all intentional provenance citations required by this plugin's Tier-1 byte-identical policy (CLAUDE.md §Provenance & Divergence Principle), not runtime path drift. Added a minimal `<!-- CHK-SKL-011-exempt: ... -->` inline marker mechanism to `scripts/check-skill-structure.py`, scoped to CHK-SKL-011 only, and applied it to the 13 cited lines across `copywriting-audit-stage`, `copywriting-ethics-check-stage`, `copywriting-form-check-stage`, `copywriting-light-action`, `copywriting-long-form-extended`, `copywriting-long-form-pasona`, `copywriting-mid-form`, `copywriting-short-form`. New test `scripts/test_check_skill_structure.py` proves marked lines pass, unmarked violations anywhere still fail, and no other plugin (`domain-teams`, `loom-code`) is affected — checker output against both is byte-identical before/after.
- **Nested-dispatch guard**: added a MUST-guard clause to `CLAUDE.md` §`agents/copywriter-evaluator.md` (evaluator) plus a one-line pointer in `copywriting-form-check-stage`, `copywriting-ethics-check-stage`, and `copywriting-audit-stage`: gate evaluations require a real Agent-tool dispatch of `copywriter-evaluator`; a subagent with no Agent tool must STOP and surface to the parent orchestrator rather than silently degrading to self-review (repo memory precedent: `docs/loom/memory/skill-in-subagent-loses-internal-orchestration.md`, loom #601/#603).
- **Router multilingual trigger hooks**: appended ZH/JA trigger phrases (寫文案 / 廣告文案 / 文案發想 / コピーライティング / キャッチコピー / セールスコピー) to `using-copywriting-toolkit`'s frontmatter description, hard-wrapped ≤76 chars/line (excluding YAML indent), existing English hooks unchanged.
- **Envelope-example alignment sweep** (whole-branch review round 1 follow-up): the schema SSOT pointer fix above relocated the `retries`/`audit_trail[]`/`express_mode_used` drift instead of closing it — the CLAUDE.md canonical example itself, both gate skills' Returned Envelope examples, and several other mid-pipeline envelope examples across the plugin still omitted the three fields. Added them everywhere a full envelope shape is illustrated (`CLAUDE.md`, `copywriting-intake`, `copywriting-ideation`, `copywriting-voice-quadrant-stage`, `copywriting-voice-tone-stage`, `copywriting-neta-injection`, `copywriting-ethics-check-stage`, `copywriting-form-check-stage`, `using-copywriting-toolkit`); left explicit append-only delta examples and documented minimal alt-entry shapes unchanged. Also aligned `using-copywriting-toolkit`'s Audit Alt-Entry hand-off block to the `phase-audit-entry` / `external_copy` / `brief.review_focus` / `form` shape that `copywriting-audit-stage` Preconditions and `CLAUDE.md` §External Caller Guide already agree on.

- `plugin.json`: 1.14.1 → 1.14.2
