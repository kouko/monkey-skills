...
3. **Instinct-axis subagent dispatch**: Launch 3-5 subagents (one per instinct trigger), per the same `loom-code:dispatching-parallel-agents` pattern the base protocol uses (`copy-ideation-parallel.md` §5, including that section's per-host dispatch call shape). Each subagent produces 8 candidates using VS probability (same as base protocol), but the divergence direction comes from the instinct trigger rather than a 曼陀羅 direction.
