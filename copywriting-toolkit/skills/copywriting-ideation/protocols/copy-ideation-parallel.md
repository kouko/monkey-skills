...
5. **Parallel dispatch**: Use the `dispatching-parallel-agents` pattern (concrete per-host dispatch call shape: `loom-code/skills/using-loom-code/references/{claude-code-tools.md,codex-tools.md}`), 8 subagents concurrently. Each subagent outputs 8 candidates → 64 total (VS single-agent path outputs 40-80). Main worker performs no quality filtering at this stage.
