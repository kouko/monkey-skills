---
title: Voice Anchor Meta — Detail (deep path)
tier: 2
---

# Voice Anchor Meta — Detail

**Load scope**: Phase 6 Pass 3 Register Signal branch only (NOT Craft Gate, NOT Axis Extreme). Deep-path meta — needed for cross-language / cross-cultural lookup and correction verification.

**Content**: verified lineage map + cross-reference-valid-for registry + cross-cultural unified label rubric + drift corrections catalog. Schema + rubric + over-mimic mitigation index live in [voice-anchor-meta-core.md](voice-anchor-meta-core.md).

## Purpose

When Pass 3 enters Register Signal branch (voice_reference is not a craft-gate master), this file provides the reference layer:
- Is the cited anchor documented or just stylistic parallel? (lineage map)
- Can this anchor cross-fertilize the target language? (cross-ref registry)
- What is the cross-cultural equivalent register? (label rubric)
- Has this anchor had any attribution drift? (corrections catalog)

## Verified lineage map

**Only 6 copywriter → literary-anchor lineages are primary-source verifiable.** All other claimed connections are stylistic parallel, NOT documented influence. Toolkit must not claim undocumented citation in rationale outputs.
