...
CACHE_DIR = Path(_CACHE_BASE) / "edinet"
...
def _load_code_list(refresh: bool = False) -> dict[str, dict]:
    ...
    cache = _cache_path("code_list.json")
    if not refresh:
        cached = _load_cache(cache, ttl_seconds=7 * 86400)
...
def _fetch_documents_list(target_date: date) -> dict:
    ...
    cache = _cache_path(f"documents_{date_str}.json")
    cached = _load_cache(cache, ttl_seconds=86400)
    if cached is not None:
        return cached
...
def _fetch_document_csv_zip(doc_id: str) -> bytes:
    """Download the type=5 CSV ZIP for a filing (cached 30 days)."""
    cache = CACHE_DIR / f"doc_{doc_id}_csv.zip"
