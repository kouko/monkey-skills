...
- `analysis-kpi` — Layer 2 bitemporal KPI store (`kpi_store.py`
  ...
  the out-of-scope `kpi_memo_feed` quarterly pool). A **TW** (台股) filer's
  MOPS iXBRL likewise lands via `kpi_tw_ingest.py ingest`, so this tearsheet
  renders TW multi-period series (TWD amounts) with no code change.
