...
def fetch_series(series: str, periods: int, api_key: str | None, use_api: bool) -> dict:
    ...
    t0 = time.monotonic()
    cache_path = cache_util.cache_path("fred", f"{series.upper()}_{periods}")
    cached = cache_util.load_cache(cache_path, 86400)
    if cached:
        cached["_cache"] = "hit"
...
def main():
    ...
    if args.no_cache:
        for s in series_list:
            path = cache_util.cache_path("fred", f"{s.upper()}_{args.periods}")
            if path.exists():
                path.unlink()
