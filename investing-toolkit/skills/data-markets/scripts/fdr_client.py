...
Cache: resolved by cache_util (XDG/uv precedence ladder; see cache_util.py
       module docstring) under `<cache_dir>/fdr/{preset}.json`.  TTL: 24h flat.
...
CACHE_SOURCE = "fdr"
CACHE_TTL_SECONDS = 86400  # 24 hours (flat — matches pre-migration behavior)
...
def fetch_preset(preset: str, use_cache: bool = True) -> dict:
    ...
    path = cache_util.cache_path(CACHE_SOURCE, preset)
    if use_cache:
        cached = cache_util.load_cache(path, CACHE_TTL_SECONDS)
