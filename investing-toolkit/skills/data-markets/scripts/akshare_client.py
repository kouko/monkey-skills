...
Cache: $INVESTING_TOOLKIT_CACHE/akshare/{preset}.json  TTL: 24h
       ...
       (Cache directory resolution + read/write delegated to cache_util.py.)
Sources:
...
def fetch_preset(preset: str, use_cache: bool = True) -> dict:
    ...
    cache_path = cache_util.cache_path("akshare", preset)
    if use_cache:
        cached = cache_util.load_cache(cache_path, ttl=CACHE_TTL_SECONDS)
...
def main() -> None:
    ...
    if args.no_cache:
        for preset in presets:
            cp = cache_util.cache_path("akshare", preset)
            if cp.exists():
                cp.unlink()
