...
# ///
"""
pack_kr.py — data-markets Layer 1 module for Korea (KOSPI / KOSDAQ + macro)
...
no primary-source equity client wired in yet (DART deferred). Regime-pack
pulls BOK ECOS-KEYSTAT via FinanceDataReader (fdr_client.py, co-located).

No CLI shell — this module exposes `build_pack(pack_name, tickers)`, the
generalized dispatcher shared identically across all 5 market pack
modules. T4's unified pack.py facade owns argument parsing, market
auto-detection, and exit-code translation (`_partial` → exit 2, etc.);
this module never calls `sys.exit`.

Pack types (SUPPORTED_PACKS):
  snapshot          (single ticker; yfinance info + price summary)
  memo-fetch        (single ticker; yfinance financials, Tier 2 only)
  comps-multiples   (single or batch; multiples-only)
  screener-batch    (batch; lightweight screening fields)
  regime-pack       (no ticker; BOK ECOS-KEYSTAT 54-indicator pull)
...
Auto-suffix rule for tickers:
  ...
  - Use force_kosdaq=True to force .KQ for ambiguous numeric tickers
    (not exposed via build_pack's generalized interface — see
    normalize_ticker directly if a caller needs it)

Cache: $INVESTING_TOOLKIT_CACHE (inherited from underlying clients via
       cache_util; this module itself does not cache)
"""
...
def normalize_ticker(ticker: str, force_kosdaq: bool = False) -> str:
    """Append .KS (or .KQ if force_kosdaq) to bare 6-digit numeric tickers.

    Edge-case handling:
      - 6-digit numeric: auto-suffix .KS (or .KQ with force_kosdaq).
...
# ---------------------------------------------------------------------------
# Unified dispatcher — required interface, identical across all 5 markets
# ---------------------------------------------------------------------------

def build_pack(pack_name: str, tickers: list[str]) -> dict:
    """Dispatch to the pack_* function matching `pack_name`.

    `tickers` is ignored for `regime-pack` (indicators default to "all" —
    the old CLI's `--indicators` selection is out of scope for this
    ...
    if pack_name == "snapshot":
        return pack_snapshot(normalized[0])
    if pack_name == "memo-fetch":
        return pack_memo_fetch(normalized[0])
    if pack_name == "comps-multiples":
        return pack_comps_multiples(normalized)
    if pack_name == "screener-batch":
        return pack_screener_batch(normalized)

    # Unreachable — SUPPORTED_PACKS enumerates every branch above.
    return {"error": f"unhandled pack: {pack_name}", "_partial": True}
