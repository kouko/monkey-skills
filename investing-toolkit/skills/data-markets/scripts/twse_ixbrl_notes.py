...
Pulls a small NAMED-concept set (~5 categories: financial instruments by
measurement category, Mainland-China investment, related-party balances +
flows, endorsement/guarantee limit + used, employee-benefit expense +
net liability) directly from the flat fact-record list produced by
twse_ixbrl_parser.parse_ixbrl_facts. This is deliberately NOT a general
note-table-reconstruction engine: an annual verification pass across
real filings falsified the "notes are a machine-readable goldmine at
scale" thesis (docs/loom/specs/2026-07-19-tw-ixbrl-ingestion.md §Annual
verification), so scope was cut down to this curated set. A concept
absent from a given filing's facts is OMITTED from the result dict,
never zero-filled.
...
# record's "concept" key. Grouped by the ~5 curated categories.
_CURATED_CONCEPTS: dict[str, str] = {
    ...
    # endorsement/guarantee limit + used
    "endorsement_guarantee_limit": "tifrs-notes:LimitOfTotalGuaranteeEndorsementAmount",
    "endorsement_guarantee_secured_with_collateral": (
        "tifrs-notes:AmountOfEndorsementsGuaranteesSecuredWithCollateral"
    ),
...
# LOOM-SIMPLIFY: when a concept repeats multiple times at the SAME
# (period, end-date+duration) key — e.g. a per-counterparty limit
# tagged once per row inside an ix:tuple — this picks the
# first-encountered fact among the tie rather than resolving which
# tuple row it belongs to (that needs ix:tuple nesting parsing, i.e.
# the general note-table-reconstruction engine explicitly out of scope
# per docs/loom/specs/2026-07-19-tw-ixbrl-ingestion.md §Annual
# verification) | ceiling: a curated field needs per-row/per-
# counterparty disambiguation (tracked as a future note-table-
# reconstruction task) | upgrade: parse ix:tuple structure to select
# the correct row instead of tie-breaking on first-encountered |
# ref: test_twse_ixbrl_notes.py::test_curated_notes_field_shape_and_absence
