...
def _cached_post(endpoint: str, body: dict, cache_key: str,
    ...
    ttl=None (TTL_HISTORICAL) means "never expires" — cache_util.load_cache
    always wants a concrete int, so None is mapped onto cache_util's
    `immutable` cadence TTL (~100y) at this call boundary.
    """
    path = cache_util.cache_path("mops", cache_key)
    effective_ttl = ttl if ttl is not None else cache_util.compute_ttl("immutable", None)
    cached = cache_util.load_cache(path, effective_ttl)
...
def main():
    ...
    if args.no_cache:
        ...
        try:
            mops_cache_dir = cache_util.resolve_cache_dir() / "mops"
            for p in mops_cache_dir.glob("*"):
