...
> Claude Code 跨國投資 plugin — Data / Analysis / Report 三層架構（three-layer architecture）涵蓋 5 個市場（US/JP/TW/KR/CN）。12 個 skill 嚴格分層：`data-markets`（market 由 ticker 自動判定）只做 pure I/O、`analysis-*` 只做 pure compute、`report-*` 負責 orchestration 並把分析委派給 `domain-teams:investing-team`。
...
│  data-markets — market 由 ticker 自動判定                │
└──────────────────────────────────────────────────────────┘
...
| `/report-stock-snapshot {ticker}` | `report-stock-snapshot` | Snapshot card；依 ticker suffix 自動 route 至 `data-markets` |

## Skill 清單（12）

### Layer 1：Data（1 skill，5 市場）

`data-markets/scripts/pack.py` 提供 5 種 pack 模式 — `snapshot` / `memo-fetch` / `comps-multiples` / `screener-batch` / `regime-pack`，並支援 `--ticker`（單檔）與 `--tickers`（批次）兩種模式。market 由 ticker suffix 自動判定，可用 `--market` 覆寫（`--pack regime-pack` 沒有 ticker 維度，因此必填）。

| Market | 內含 client | 說明 |
|-------|-------------|------|
| `us` | yfinance、sec_edgar、fred | SEC EDGAR Tier A primary；yfinance 原生 batch |
| `jp` | yfinance、edinet、tdnet、boj、estat、ecb | EDINET-key tier routing 邏輯放在 `pack.py` 內 |
| `tw` | yfinance、mops、twse_openapi、finmind、cbc、dgbas、ndc、statgov | MOPS + TWSE Tier A；FinMind Tier 2 fallback |
| `kr` | yfinance、fdr | FDR 走 BOK ECOS-KEYSTAT |
...
| `report-stock-snapshot` | 依 ticker suffix 自動偵測國家 → `data-markets/pack.py --pack snapshot` → 排版 card | Snapshot card（Markdown） |
| `report-portfolio-review` | 逐部位 → 依國家批次呼叫 `data-markets` → `analysis-portfolio` → `analysis-macro-regime` overlay → 排版 | Portfolio review（Markdown） |
| `report-screener-list` | 解析 ticker list → 依國家分組 → 平行 `data-markets --pack screener-batch` → 串接 → `analysis-screener` → 輸出 top-N 表格 | Screener result table（Markdown） |

### Router

| Skill | 說明 |
|-------|------|
| `using-investing-toolkit` | 入口 — 上述 11 個 skill 的 intent routing |

## Quick start 範例

```bash
# AAPL 投資 memo — 串接 data-markets（US）→ analysis-dcf + analysis-comps + analysis-technical → investing-team
/report-equity-memo AAPL

# 跨國家 screener
/report-screener-list AAPL,MSFT,2330.TW,7203.T --preset quality

# Snapshot card；ticker suffix 自動 route data-markets 至 TW market
...
Adapter 集中在 `data-markets/scripts/`，是唯一的 canonical copy（Layer 1 合併後，舊有的 per-market MD5-locked 副本同步機制已廢除）。詳見 [`docs/adr/0001-data-analysis-report-layers.md`](docs/adr/0001-data-analysis-report-layers.md)。
...
- 歡迎新增 data adapter 的 PR；請依照既有 `*_client.py` pattern（`register_mcp_tools()` + subprocess CLI + cache TTL 標頭），並補上 contract test fixture。Adapter 直接放在 `data-markets/scripts/`（唯一 canonical copy），並整合進對應的 `pack_<market>.py`
