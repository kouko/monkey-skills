...
> Claude Code 向けクロスカントリー投資 plugin — Data / Analysis / Report の three-layer architecture で 5 市場（US/JP/TW/KR/CN）をカバー。12 個の skill が層を厳密に分離する：`data-markets`（market は ticker から自動判定）は pure I/O、`analysis-*` は pure compute、`report-*` は orchestration を担い分析は `domain-teams:investing-team` に委譲する。
...
```
┌─ Layer 1: Data（1 skill、5 市場、pure I/O）──────────────┐
│  data-markets — ticker から market を自動判定            │
└──────────────────────────────────────────────────────────┘
...
|---------|-----------|------|
| `/using-investing-toolkit` | `using-investing-toolkit` | Router — 目的を伝えると dispatch される |
...
| `/report-equity-memo {ticker}` | `report-equity-memo` | フル memo pipeline → `domain-teams:investing-team` |
| `/report-screener-list {tickers}` | `report-screener-list` | クロスカントリー grouping 付き multi-criteria screener、複合 score ranking |
| `/report-portfolio-review` | `report-portfolio-review` | P&L snapshot + macro regime overlay + rebalance |
| `/report-stock-snapshot {ticker}` | `report-stock-snapshot` | Snapshot card；ticker suffix で `data-markets` に自動 route |

## Skill カタログ（12）

### Layer 1: Data（1 skill、5 市場）

`data-markets/scripts/pack.py` は 5 種類の pack mode — `snapshot` / `memo-fetch` / `comps-multiples` / `screener-batch` / `regime-pack` — を備え、`--ticker`（単一）と `--tickers`（バッチ）両方をサポート。market は ticker suffix から自動判定、`--market` で上書き可能（`--pack regime-pack` は ticker 次元を持たないため必須）。

| Market | 同梱 client | 備考 |
|-------|-------------|------|
| `us` | yfinance、sec_edgar、fred | SEC EDGAR Tier A primary、yfinance ネイティブ batch |
| `jp` | yfinance、edinet、tdnet、boj、estat、ecb | EDINET-key tier routing は `pack.py` 内 |
| `tw` | yfinance、mops、twse_openapi、finmind、cbc、dgbas、ndc、statgov | MOPS + TWSE Tier A、FinMind Tier 2 fallback |
| `kr` | yfinance、fdr | FDR 経由で BOK ECOS-KEYSTAT |
...
| `report-stock-snapshot` | ticker suffix から国を自動判定 → `data-markets/pack.py --pack snapshot` → card 整形 | Snapshot card（Markdown） |
| `report-portfolio-review` | ポジション毎 → 国別バッチで `data-markets` → `analysis-portfolio` → `analysis-macro-regime` overlay → 整形 | Portfolio review（Markdown） |
| `report-screener-list` | リスト解析 → 国別 grouping → `data-markets --pack screener-batch` を並列実行 → concatenate → `analysis-screener` → top-N 表 | Screener result table（Markdown） |

### Router

| Skill | 説明 |
|-------|------|
| `using-investing-toolkit` | Entry point — 上記 11 skill の intent routing |

## Quick start 例

```bash
# AAPL の equity memo — data-markets（US）→ analysis-dcf + analysis-comps + analysis-technical → investing-team を orchestrate
/report-equity-memo AAPL

# クロスカントリー mixed list の screener
/report-screener-list AAPL,MSFT,2330.TW,7203.T --preset quality

# Snapshot card；ticker suffix で data-markets が TW market に自動 route
...
Adapter は `data-markets/scripts/` を単一 canonical copy とします（Layer 1 統合により、旧来の per-market MD5-locked コピー同期は廃止されました）。詳細は [`docs/adr/0001-data-analysis-report-layers.md`](docs/adr/0001-data-analysis-report-layers.md) を参照。
...
investing-toolkit                              domain-teams:investing-team
  report-equity-memo                ───→       Deep Equity Research Memo protocol
    Layer 1: data-markets/pack.py                2 MUST gate
...
- 新規 data adapter の PR を歓迎します。既存の `*_client.py` pattern（`register_mcp_tools()` + subprocess CLI + cache TTL header）に従い、contract test fixture を追加してください。Adapter は `data-markets/scripts/`（単一 canonical copy）に直接配置し、対応する `pack_<market>.py` に組み込んでください
