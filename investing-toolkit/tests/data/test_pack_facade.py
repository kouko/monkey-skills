...
def test_kpi_topline_backfill_non_us_ticker_exits_64_usage_error():
    """kpi-topline-backfill is US-only (SEC EDGAR companyconcept annual
    backfill, Task 4, docs/loom/plans/2026-07-25-company-total-revenue.md):
    a .TW ticker is REFUSED with an explicit US-only usage_error — mirrors
    the sibling kpi-quarterly precedent above. Without registering this
    pack in `US_ONLY_PACKS`, the ticker would fall through to
    `pack_tw.build_pack` and raise a bare, unhandled `ValueError` instead
    of a correctly-classified usage error."""
    proc = _run_pack_py("--ticker", "2330.TW", "--pack", "kpi-topline-backfill")
    assert proc.returncode == 64, f"stdout={proc.stdout}\nstderr={proc.stderr}"
    payload = json.loads(proc.stdout)
    status = payload["_status"]
    assert status["status"] == "usage_error"
