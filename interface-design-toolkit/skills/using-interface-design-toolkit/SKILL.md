...
<EXTREMELY-IMPORTANT>
**You have interface-design-toolkit.** When the user wants to design a product's interface, interaction, or UX, route through this skill before producing any design artifact.
...
3. interface-design-toolkit skills loaded into context — this router + invoked generators.
...
- **`product-principles-toolkit:product-principles`** — produces the `PRINCIPLES.md` this toolkit reads as governing context. The principles layer comes **before** interface design; this toolkit consumes its output, never duplicates it.
...
- Does **not** author or override `PRINCIPLES.md` — that belongs to `product-principles-toolkit`.
