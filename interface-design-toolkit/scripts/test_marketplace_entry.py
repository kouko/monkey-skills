"""Regression guard: interface-design-toolkit is registered in the root marketplace.json.

Pure-config test (tdd-iron-law exemption) — asserts the plugins[] entry
exists, points at ./interface-design-toolkit/, and that its name +
...
def test_exactly_one_interface_design_toolkit_entry():
    entries = [p for p in _load_plugins() if p.get("name") == "interface-design-toolkit"]
    assert len(entries) == 1, f"expected exactly one interface-design-toolkit entry, got {len(entries)}"


def test_interface_design_toolkit_source():
    entry = next(p for p in _load_plugins() if p.get("name") == "interface-design-toolkit")
    assert entry["source"] == "./interface-design-toolkit/", f"unexpected source: {entry.get('source')!r}"
