...
the hard constraints the task and the host enforce:
  ...
  - the render-variant FLAG-only rule, with the full state machine deferred to
    `spec-toolkit:spec-expansion` (the seam is named).
...
def test_body_names_spec_expansion_seam():
    """ui-flows.md is the rich seed to spec-toolkit:spec-expansion."""
    text = _text()
    assert "spec-expansion" in text or "spec-toolkit:spec-expansion" in text, \
