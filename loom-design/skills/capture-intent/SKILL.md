...
the machine-read internal artifacts later stations own — nothing in it is
translated to English, English being the language of the plan, the spec,
the review record, evidence notes, test docstrings and names, and commit
messages — while the decision-point dialogue, the blind-run report and
the pull-request body stay in the user's language.
