...
Read `docs/loom/PRINCIPLES.md` first, when present, as the governing
constraint every token must be defensible against. If it is absent, read
`docs/loom/PURPOSE.md` instead when that exists, say explicitly that the
mood below is derived rather than inherited, and proceed only on the
user's say-so — never invent a constitution to satisfy this step.
