"""Queue state, freeze gate, worktree lifecycle and reconcile engine.

The non-CLI half of loom-pipeline batch mode: parsing the human-editable
...
def check_frozen(entry: dict, project_path: Path, skills_root: Path) -> tuple[bool, str]:
    ...
    loom-design validator (``python3 <skills_root>/scripts/spec/
    ...
    validator_script = Path(skills_root) / "scripts" / "spec" / "validate_spec_output.py"
