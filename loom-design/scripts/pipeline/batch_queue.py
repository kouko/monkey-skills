...
The command-line half: argparse wiring, the ``_cmd_*`` subcommand
handlers and ``main``. Intent (``QUEUE.toml``) is separate from state
(machine-owned ``queue-state.json``) — see
docs/loom/plans/2026-07-03-loom-pipeline-v1-1-batch-mode.md
§Settled open questions 1. Every piece of that machinery — queue/state
loading, the freeze gate, worktree lifecycle, the reconcile engine and
the circuit breaker — lives in the sibling module ``queue_core`` and is
imported below.
