...
- **⚠️ `notion-fetch` 可能回快取/as-of 快照——continuity 重驗時必查時間戳**:`notion-fetch` 有時回傳的是快取版本,而非即時資料;回傳中的 `last_edited_time`(或 `as-of` 欄位)記錄了這份資料的實際截止時間,**不是**「你剛才發出請求的時間」。若在 continuity 重驗步驟中用 `notion-fetch` 確認某頁的最新狀態,**必須檢查這個時間戳**:若它早於今日(`< {TODAY}`),表示拿到的是舊快照——依照 `prioritization-framework.md §5` 的 as-of 降級規則,將該項目的 continuity 信心降為「推論」並加 caveat「重驗資料非即時 (as-of X)」。(對應 prioritization-framework.md §5 as-of 降級規則。)
