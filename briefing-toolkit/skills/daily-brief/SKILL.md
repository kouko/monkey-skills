...
**triage 要一次做滿**(去重 / 排序 / 焦點門檻 / 需回覆 / 信心 / 跨日 diff 全部跑完),產出一份**外部化的排序清單(完整母體)**——這份排序清單就是下一步兩個產物共同的單一來源,**render 任何檔案前先把它定下來**。
...
寫進指定資料夾,**依此生成順序**(母體先、晨報後):
1. **`<YYYY-MM-DD>_完整事項.md`(母體)** — 把上一步外部化的排序清單**全量** render 成零省略行動表,每列可點;此步定下「共 N 筆」。
2. **`<YYYY-MM-DD>_完整事項.csv`(母體的機讀鏡像)** — 與完整表逐列對應,鎖住**唯一識別碼欄 = 隔天 delta 的 join key,不可省略**。
3. **`<YYYY-MM-DD>_晨報.md`(從母體 curate)** — PDB(President's Daily Brief)風格 6 段 + 動態焦點,**從完整表挑子集再改寫**,不另外搜尋;可引用確切「N 筆」與焦點。
兩份 `.md` 開頭都放資料源涵蓋聲明。
> **為何母體先、晨報後(WHY)**:晨報是完整表的 **curated 子集**,不是另一次搜尋。先把完整母體外部化,晨報才能①從完整候選集挑焦點(焦點選得準、不漏不灌)、②保證每筆焦點/要回覆都在表內、計數對得上(防漂移)。先寫晨報 = 在母體未定前憑印象挑,易與後補的表兜不攏。**閱讀順序(晨報先看)≠ 生成順序(母體先產)。**
→ **MANDATORY:讀 `references/brief-templates.md`**(晨報 6 段版型 + 完整表 / CSV schema + 涵蓋聲明 + 日期前綴命名規則 + §生成順序)。
