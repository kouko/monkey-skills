...
| # | Signal | Route 先 |
|---|---|---|
| 1 | 4DX 自体が合うか不明 | `4dx-meta-strategy-triage`（6 verdict gate） |
| 2 | 「日常業務に追われて目標が進まない」 | `4dx-meta-whirlwind-triage`（7 日監査、80/20 capacity） |
| 3 | 目標が曖昧 / 多優先 / activity 表現のみ | `4dx-d1-wig-formulation`（1 つの *From X to Y by When*） |
| 4 | WIG はあるが日々何をすべきか不明 | `4dx-d2-lead-measures`（predictive + influenceable） |
| 5 | tracking が noisy / 見ない / 30 個の metric DB | `4dx-d3-scoreboard`（≤4 要素、5 秒テスト） |
| 6 | D1-D3 はあるが cadence がない | `4dx-d4-cadence`（週次 Account → Review → Plan） |
| 7 | 実践が停滞 / lapse / momentum 喪失 | `4dx-sustain-momentum-rescue`（破損層の診断） |
