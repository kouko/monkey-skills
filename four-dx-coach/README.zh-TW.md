...
## 兩種模式：coach 與 consultant

這個 plugin 有 **兩種互補模式**：

- **Coach 模式**（11 個 skill）—— 對話驅動、Socratic、step-by-step。適合從零（或片段）開始，想要*一個決策一個決策地*把方法論走過一遍：agent 問、你答、我們收斂。
- **Consultant 模式**（1 個 skill — `4dx-audit`）—— 文件合成、診斷式、roadmap 輸出。適合你已經有現成 context（策略 doc / OKR sheet / KPI dashboard / WIG-Session 紀錄 / 過去嘗試的 chat history），想要 consultant 把*手上的東西對應到 4DX*，逐 layer 診斷狀態，並給出有順序的下一步，路由回對應的 coach skill。

何時用哪個：

| 你手上有的是… | 用… |
|---|---|
| 模糊的意圖（「我想開始用 4DX」） | Coach 模式 → `using-four-dx-coach`（router） |
| 特定階段的問題（「WIG 怎麼寫？」） | Coach 模式 → 直接對應的 `4dx-d{1-4}` skill |
| 既有資料 + 「幫我用 4DX 套」 | Consultant 模式 → `4dx-audit` |
| 4DX 已經在跑但卡住，又說不清是哪裡卡 | Consultant 模式 → `4dx-audit` |

Audit 結尾會路由到具體的 coach skill 做深度工作 —— consultant 模式是入口 + roadmap，不取代 coach skill。

## Architecture

12 個 skill，分四類：

- **1 個 plugin router**（`using-four-dx-coach`）—— 處理冷啟動 / 跨 topic / 非 4DX query 的 dispatcher（coach 模式）。
- **1 個 consultant-mode entry point**（`4dx-audit`）—— 將 user 提供的 artifact 對齊 4DX 五層模型，逐層診斷狀態，再路由回 coach skill。
...
| Skill | 模式 | 功能 |
|---|---|---|
| [`using-four-dx-coach`](skills/using-four-dx-coach/) | Coach（router） | 入口路由 —— cold-start / cross-topic / 非 4DX query；scope triage 到 personal / team-leader / team-member 或 4DX 不適用時轉介 |
| [`4dx-audit`](skills/4dx-audit/) | Consultant | 讀取 user 提供的 artifact（策略 doc / OKR / dashboard / WIG-Session 紀錄），對應到 4DX 五層模型（WIG / Lead / Lag+Scoreboard / Cadence / Substrate），逐層診斷狀態，輸出排序過的建議 + 路由到對應 coach skill |
