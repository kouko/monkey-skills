...
- fire for any edit under ``docs/loom/memory/``, including the store's own
  ...
  ``type: Memory Store Guide`` — so a bad edit to it breaks the invariant
  just as surely as a bad edit to any lesson),
- derive the repo root from the edited path and invoke the real validator,
- exit 0 when the store is valid, exit 2 on a violation (with a stderr
  fix-hint naming the offending file). A PostToolUse exit 2 does not undo
  the write — the file has landed; the hook surfaces the problem to the
...
shipped three times. The first two shipped and were caught by CI after
push, under a job named ``plugin version bump`` — which names a different
check, so the failure did not point at the store. The third was caught one
step later than here, at branch close-out, by an orchestrator following
prose.
...
_ENTRY_DESC = "A one-line description that index.md must carry byte-identical"
...
def _valid_entry_text() -> str:
    return (
        "---\n"
        f"name: {_ENTRY_NAME}\n"
        f"description: {_ENTRY_DESC}\n"
        "type: practice\n"
        "sources:\n"
...
def _broken_entry_text() -> str:
    """Missing `sources` — a real validator violation, named against the
    entry's own filename (so the reader sees which file to fix)."""
    return (
        "---\n"
        f"name: {_ENTRY_NAME}\n"
        f"description: {_ENTRY_DESC}\n"
...
def _make_store(tmp_path: Path, *, indexed: bool, with_checker: bool = True) -> Path:
    ...
    if indexed:
        entry.write_text(_valid_entry_text(), encoding="utf-8")
        (store / "index.md").write_text(lm.generate_index(store), encoding="utf-8")
    else:
        # Broken frontmatter AND no index.md: either alone already violates
        # the profile, so this fixture is invalid regardless of which
        # invariant a future validator change happens to check first.
        entry.write_text(_broken_entry_text(), encoding="utf-8")
    return entry


def test_blocks_when_the_store_is_invalid(tmp_path: Path) -> None:
    """The recurring defect class: an entry is written broken/un-reindexed."""
    ...
    assert _ENTRY_NAME in result.stderr, \
        "the validator's own report must reach the reader, naming the file"
    # The hook's OWN contribution, not the validator's: the regenerate-index
    ...
    # instead of writing — the Fix block must tell the reader what to do
    # THEN, not just what to do when it succeeds.
    assert "If regenerate-index itself refuses" in result.stderr, \
        "the fix-hint must cover regenerate-index's own refusal, not just the happy path"
...
def test_fires_on_the_readme_itself(tmp_path: Path) -> None:
    """An edit to the store's own README must be checked too — README.md is
    itself a store concept file."""
...
def test_no_ops_when_the_validator_is_absent(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, (
        "a repo without the validator must not be blocked; got "
        f"{result.returncode}\n{result.stderr}"
    )
...
# so the directory is unreachable either way. The env var stays in the hook
# as cheap defence should the validator ever grow a local import; it is
# documented there, not guarded here. Writing a test that cannot fail is
# worse than writing none — it reports coverage that does not exist.
