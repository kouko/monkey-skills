...
(b) Output quality / A/B variant exploration with human judgment
    → Hand off to `dev-workflow:skill-tuning`. Do not handle here.
...
> The blind comparator uses a subagent as judge (LLM-as-judge). For taste-sensitive output where LLM-judge is known to be unreliable (creative writing, voice, design feel), use `dev-workflow:skill-tuning` instead — that skill uses human judgment per iteration and accumulates a preference log.
...
Required components:
- `dev-workflow/skills/skill-tuning/SKILL.md` — variant-generation editor prompt, A/B harness protocol, preference logging
- `dev-workflow/commands/skill-tuning.md`
- LICENSE / NOTICE
- `dev-workflow/skills/skill-tuning/README.{en,ja,zh-TW}.md`
