...
| `skill-tuning` | `/skill-tuning` | 對既有 skill 進行輸出品質 A/B — 產生不同 output trait 的變體、盲測呈現、捕捉使用者偏好。Constitution 是底線、taste 是上限、preference log 累積成 RLHF-lite dataset（skill-evolution 的 Phase B）|
...
### Skill-evolution 架構（skill-refactor + 未來的 skill-tuning）
...
- `skill-refactor`（v1.6.0）處理既有 skill 的*行為保留*重構
- `skill-tuning`（本 release，v1.7.0）處理 taste-sensitive 輸出
  A/B 由人類判斷 + preference log
...
└── skills/
    ...
    │   ├── README.{en,ja,zh-TW}.md
    │   ├── references/            ← equivalence-check / multi-judge / refactor-moves / golden-anchor / test-prompts-schema / constitution-schema（共用 convention 的 canonical SoT）
    │   └── scripts/               ← equivalence_check / multi_judge / golden_compare
    └── skill-tuning/
        ├── SKILL.md               ← Phase B: 輸出品質 A/B，人類 judge + preference log
        ├── LICENSE / NOTICE       ← 獨立設計；NOTICE 記錄與 darwin-skill 的設計差異 9 點
        ├── README.{en,ja,zh-TW}.md
        ├── references/            ← ab-harness / constitutional-judging / preference-log-schema / self-trained-judge-pipeline + 3 個共用 convention 的內建 functional copy
