...
| `dev-workflow:skill-creator-advance`              | Behavioral| test prompt 的 pass/fail       |
| `domain-teams:skill-team`                         | Structural| convention gate 的 PASS/FAIL   |

一個 skill 可以通過 `skill-team` 的 gate，卻在 `skill-judge` 拿 D
（沒有 convention 違規，但內容大多 redundant）。也可以在
`skill-judge` 拿 A，卻在 `skill-team` gate 失敗（內容很好，但 directory
