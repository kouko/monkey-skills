...
tasting は意図的に動作を変えて良い出力を探す（人間判断を使う —
...
auto-revert を駆動）と異なり、skill-tasting は人間 ADOPT が
...
- **`dev-workflow:skill-refactor`** — 姉妹 Phase A skill；動作
  保持、LLM-judge 等価；tasting は動作変更、人間 judge。組み合わ
  せ可能：先に refactor でトークンを縮め、次に taste で品質最適化。
- **`dev-workflow:skill-creator-advance`** — tasting が同じ形状
  内で好まれる出力の variant がないことを示すとき、再設計に hand off。
- **`dev-workflow:skill-judge`** — variants への advisory チェック
  （advisory のみ；taste は理解しない）。
- **`copywriting-toolkit:voice-anchors`** — コピーライティングの
  対応概念；この skill はその curation 規律を借りる。
- **`dev-workflow:proposal-critique`** — 複数 tasting 提案を
  triage するとき。
...
`skill-refactor`（Phase A）と `skill-tasting`（Phase B）の分割は
基礎的 — Fowler の Two Hats を skill に適用：refactor は動作保持、
tasting は動作変更。これらを 1 つの skill に混ぜる（`darwin-skill`
