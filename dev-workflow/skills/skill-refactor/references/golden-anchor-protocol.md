...
> **Shared convention.** This file is bundled in both
> `dev-workflow:skill-refactor/references/` and
> `dev-workflow:skill-tasting/references/` (when shipped). Same-PR
> drift rule: any edit to the protocol must update both copies in
> the same PR. The two copies are intentionally synchronized
> functional copies, not delegated paths.
