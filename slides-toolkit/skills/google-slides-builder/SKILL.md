...
**Google Slides backend 的執行層**。讀 `slide-plan.json` v1.2，依序跑 4 個 core recipe（create-presentation → create-slides → insert-text → insert-image），回傳 Drive URL。**不做設計判斷**（設計諮詢 → `slides-design`）、**不做首次設定**（auth / gws 安裝 → `google-slides-setup`）、**不做 template management**（v0.3 移除；見 PRODUCT-SPEC §3.2 Non-Goals 與 §3.5 Phase 2+ trigger）。
