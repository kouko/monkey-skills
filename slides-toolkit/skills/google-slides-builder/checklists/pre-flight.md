# Pre-flight checklist — google-slides-builder

10 項檢查；任一 fail 則依指定 exit code 中止 pipeline，**不**進 recipe 階段。每項提供 shell-runnable check 或明確判定準則。

**使用方式**：builder 依序跑這 10 項，全通過才進 Step 2（resolve template）。失敗訊息均為 stderr + structured JSON（stderr 一行 JSON + 一行人讀 hint）。

---

## 1. [ ] `slide-plan.json` `target == "google-slides"`

```bash
jq -e '.target == "google-slides"' slide-plan.json > /dev/null
```

- Fail → **exit 12** + hint：`unsupported target "<value>"; MVP only supports google-slides (Phase 2+ trigger: PRODUCT-SPEC §3.5)`

## 2. [ ] `slide-plan.json` validates against minimal schema

```bash
jq -e '
  .version == "1.1"
  and (.output_title | type == "string" and length > 0)
  and (.backend_config.template_ref | type == "string" and length > 0)
  and (.slides | type == "array")
' slide-plan.json > /dev/null
```

必填欄位皆備、型別正確。Fail → **exit 15**。

## 3. [ ] `backend_config.template_ref` 在 `templates/registry.md` 找得到

```bash
ref=$(jq -r '.backend_config.template_ref' slide-plan.json)
awk -F'|' -v r="$ref" '
  NR>1 && $2 ~ "^ *"r" *$" {found=1; exit}
  END {exit !found}
' templates/registry.md
```

- Fail → **exit 12** + hint：`template_ref "<ref>" not in registry; edit skills/google-slides-builder/templates/registry.md to add an entry`

## 4. [ ] 所有 `slides[].images[].local_path` 檔案存在 + 大小合格

對每個 image entry 跑：
- Path 解析（absolute / `~` 展開 / cwd-relative；見 TECH-SPEC §9 OPEN-8）
- `[[ -f "$resolved" ]]`
- Size: `find "$resolved" -size -50M | grep -q .`
- Extension: `.png` / `.jpg` / `.jpeg` / `.gif`

```bash
jq -r '.slides[].images[]?.local_path' slide-plan.json | while read -r p; do
  resolved=$(eval echo "$p")
  [[ "$resolved" != /* ]] && resolved="$PWD/$resolved"
  [[ -f "$resolved" ]] || { echo "missing: $resolved" >&2; exit 14; }
  size=$(stat -f%z "$resolved" 2>/dev/null || stat -c%s "$resolved")
  [[ "$size" -lt 52428800 ]] || { echo "too big: $resolved" >&2; exit 14; }
done
```

- Fail → **exit 14**（缺檔 or 過大 or 格式不支援）

## 5. [ ] `gws auth whoami` 正常（token 未過期）

```bash
scripts/google-slides/credential-check.sh
```

回傳 `{"backend":"...","token_valid":true,"expires_in_sec":N}` 且 `N > 0`。

- `token_valid == false` 或 `expires_in_sec <= 0` → **exit 10** + hint：`Your gws refresh token has expired (Google External + Testing policy: 7-day lifetime). Run: gws auth login`
- Keychain + file backend 都失敗 → **exit 18**

## 6. [ ] 所需 scope 已 grant（presentations + drive.file）

```bash
gws auth scopes 2>/dev/null | grep -q 'presentations' && \
  gws auth scopes 2>/dev/null | grep -q 'drive.file'
```

若未 grant → **exit 10** + hint：`missing required scopes. Run google-slides-setup and re-authorize`

（TECH-SPEC §4.4：只需 `presentations` + `drive.file`；其他 scope 被 least-privilege 原則拒絕）

## 7. [ ] 網路可通 googleapis.com

```bash
curl -sSf --max-time 5 -o /dev/null https://www.googleapis.com/discovery/v1/apis
```

- Fail（DNS / 封鎖 / proxy 問題）→ **exit 1** + hint：`network unreachable to googleapis.com; check connectivity / proxy`

## 8. [ ] `~/.cache/slides-toolkit/bin/gws` 存在 + executable

```bash
[[ -x "$HOME/.cache/slides-toolkit/bin/gws" ]] && \
[[ -x "$HOME/.cache/slides-toolkit/bin/jq" ]]
```

- Fail → **exit 1** + hint：`gws/jq binary missing — run google-slides-setup (scripts/google-slides/bootstrap.sh)`

（TECH-SPEC §2.3：由 `google-slides-setup` 的 `bootstrap.sh` 自動下載 + SHA-256 verify；builder 不自己抓）

## 9. [ ] issue #119 env vars 已 export（若 workaround 仍需要）

```bash
scripts/google-slides/env-guard.sh check
```

回傳 `{"workaround_needed": true/false}`。

- `workaround_needed == true` 且 env var 未設 → **exit 16** + hint：`issue #119 workaround not active; run google-slides-setup to apply (env-guard.sh apply)`
- `workaround_needed == false` → pass（gws 版本已修）

（TECH-SPEC §6.1：builder 只 `check`、不 `apply`；mutation 路徑歸 `google-slides-setup`）

## 10. [ ] Dry-run 模式確認

```bash
dry=$(jq -r '.dry_run // false' slide-plan.json)
```

- 若 `dry_run == true` → builder 跑 Step 1（本 checklist 1–9）+ registry lookup + local image 存在性，**不**進 recipe、**不**呼 Google API
- 完成後 stdout 印：`{"url":null,"presentation_id":null,"slides_count":0,"warnings":[],"dry_run":true}`
- 人讀訊息：`Dry-run complete. Pre-flight passed. Re-run with "dry_run": false to actually generate the deck.`

---

## Summary：exit code 對映

| Check # | 失敗 → exit |
|---|---|
| 1 | 12（unsupported target） |
| 2 | 15（schema fail） |
| 3 | 12（template not in registry） |
| 4 | 14（local image missing / too big / bad format） |
| 5 | 10（token expired） / 18（credential backend all fail） |
| 6 | 10（scope not granted） |
| 7 | 1（network） |
| 8 | 1（binary missing — setup 未跑） |
| 9 | 16（issue #119 workaround 未 apply） |
| 10 | — (dry_run 為設定，不 fail；成功後 short-circuit) |

**全通過** → 進 Step 2 resolve template（`protocols/recipe-copy-template.md`）。

## See also

- TECH-SPEC §4.2（完整 exit code 表）、§4.6（E2E data flow）、§9 OPEN-8（path 解析）
- `scripts/google-slides/credential-check.sh`、`env-guard.sh`、`bootstrap.sh`
- `templates/registry.md`（lookup 對象）
