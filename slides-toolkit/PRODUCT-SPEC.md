...
| **Template-based workflow return**（使用者自備 template deck + `registry.md` 查找 + template-copy + placeholder replaceAllText 的完整流程） | 實際使用後發現 Google 內建 predefined layouts 視覺品質不足（kouko 對字型 / 色票 / spacing 明顯不滿、或超過 20% 場景需要手動後製調整），或需要跨 deck 一致品牌視覺（logo / 配色 / 字型系統不可由 Google 預設層覆蓋）。Trigger 達成時，恢復 `skills/google-slides-builder/templates/registry.md` + template-copy recipe，並重新觸發 `[ASSUMPTION-2]` 80% 覆蓋率重評 |
...
`TITLE_AND_TWO_COLUMNS` / p9 `TITLE`）。`google-slides-builder` 自動：
...
`slide-plan.json`，最後用 `google-slides-builder` 合成。**總耗時：12 分
