"""Regression guard: product-principles-toolkit is registered in the root marketplace.json.

Pure-config test (tdd-iron-law exemption) — asserts the plugins[] entry
exists, points at ./product-principles-toolkit/, and that its name +
...
def test_exactly_one_product_principles_toolkit_entry():
    entries = [p for p in _load_plugins() if p.get("name") == "product-principles-toolkit"]
    assert len(entries) == 1, f"expected exactly one product-principles-toolkit entry, got {len(entries)}"


def test_product_principles_toolkit_source():
    entry = next(p for p in _load_plugins() if p.get("name") == "product-principles-toolkit")
    assert entry["source"] == "./product-principles-toolkit/", f"unexpected source: {entry.get('source')!r}"
