...
description: |
  Surface each team member's mental model as a CLD; negotiate convergence on fundamental values; sustain via measurable leadership-energy proxies (cadence count, active-listening minutes, value-revisitation count) that make the energy-pump metaphor falsifiable rather than poetic. Not one-shot offsites — continuous-input requirement. A team is an open thermodynamic system that sustains higher-than-random order only while energy keeps flowing through it; a leader-less team is structurally a closed system and will decay regardless of individual talent. NOT for short-term task forces or one-shot delivery teams (energy investment cost exceeds payback), remote / distributed / async-first cultures (the energy-pumping metaphor assumes co-presence; book pre-dates 2020 hybrid work), teams where psychological safety is the actual binding constraint (fix safety first — Edmondson), or cognitive-diversity-by-design teams (red team, devil's advocate, audit) where harmony is anti-goal. Triggers: "individually excellent but team is slow", "we built a band of brothers", "why are decisions taking forever", "the team used to just get it", "we need more team-building", "code reviews are contentious", "design reviews are contentious", "leadership vacuum", "the team is decaying", "how do we measure team coordination", "leadership energy is unfalsifiable", "we hired stars and now velocity dropped", "team doubled in 12 months and culture is fraying". KEYWORDS: mental-model harmony, mental-model surfacing, leadership as energy pumping, observable energy proxies, falsifiable leadership signals, cadence proxy, active-listening minutes, value-revisitation count, open system team, closed-system decay, band of brothers, exhortation anti-pattern, Nelson Trafalgar, Coopers Lybrand anti-case, continuous-input requirement. JA: メンタルモデル調和・メンタルモデル可視化・リーダーシップとしてのエネルギー注入・観測可能なリーダーシップ指標・カデンス指標・能動的傾聴時間・継続的投入要件・年次オフサイトは効かない. zh-TW: 心智模式和諧・心智模式浮現・領導力如能量泵浦・可觀測領導力指標・節奏指標・積極傾聽時數・價值重訪次數・開放系統團隊・閉合系統衰減・持續投入要求.
