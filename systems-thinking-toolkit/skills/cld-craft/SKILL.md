...
**OPTIONAL but RECOMMENDED for high-stakes CLDs**:

- [`references/cld-falsifiability-protocol.md`](references/cld-falsifiability-protocol.md) (~200 lines) — v0.8 protocol with 6 falsification tests (sign / loop closure / magnitude / behavioral signature / fuzzy proxy / regime split). MANDATORY when the CLD informs regulatory / compliance / financial-decision-grade / litigation / safety-critical use. Optional for facilitation, learning, or conversational use. The protocol annotates each claim in the CLD as `[verified]` / `[unfalsifiable]` / `[post-hoc]` / `[no proxy]` so downstream consumers know which parts are evidence-grounded vs conjectural. This is the plugin's first methodological improvement over Sherwood Rule 10 (social validation → empirical falsifiability).
