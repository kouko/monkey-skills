...
name: cld-archetypes
description: |
  Given a Mermaid CLD with R/B annotations (typically from `loop-and-link-primitives` via `cld-craft`), recognize which archetype is present and apply the intervention playbook. Covers two archetypes from Sherwood: (a) **limits-to-growth** — an R-loop braked by one or more B-loops, signature is *deceleration despite unchanged inputs*, intervention is constraint-relief ("take the brakes off"); (b) **variance/target/action (V/T/A)** — a single B-loop with feedback delay, signature is *oscillation around a target*, intervention is often "do nothing" (or attack the lag, not the target). Triggers: "growth is decelerating," "we're doubling spend but revenue won't move," "the engine feels stuck," "diminishing returns on [channel]," "we're saturating," "metric keeps swinging," "we keep adjusting and it's getting worse," "ping-ponging between two states," "every time we change [X] it gets worse," "should I act now or wait?", "I don't know if my last move worked yet," "the numbers go up and down." NOT for cold-start (no growth yet — pre-PMF), genuinely diverging / vicious-circle systems (use `loop-and-link-primitives` R-loop diagnosis), one-off non-recurring decisions, real-time control with zero detection lag, or random within-control noise (use SPC / control-chart tooling). KEYWORDS: limits-to-growth, take-the-brakes-off, constraint-relief, growth deceleration, diminishing returns, market saturation, binding constraint, balancing loop, B-loop, target, variance, action, actual, oscillation, ping-pong, over-correction, do nothing, feedback delay, detection lag, small moves long intervals, moving target, endogenous oscillation, growing amplitude, monetary policy committee, inventory control, NPS swing. JA: 成長エンジンの減速・成長限界・制約解除・バランス型ループ・目標と実績の差・振動・何もしない判断 / zh-TW: 成長引擎減速・成長極限・解除約束・平衡迴路・目標差異行動範本・振盪・無為而治的時機.
