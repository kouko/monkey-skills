...
description: |
  Translate a cld-craft CLD into a quantitative stock-and-flow model for learning, not point forecasts; optional Python simulator with CSV/ASCII charts. Use for 'model this', 'stock and flow', 'convert CLD to simulation', or 'simulate the S-curve'.
...
7. **Add fuzzy response curves where needed** — for non-linear effects (saturation, threshold), use hand-drawn graphs (compose with the fuzzy-variable sub-procedure in `cld-craft`, Rules 7 / 7a / 7b / 7c).
   - **Axis labels mandatory**: x-axis = cause magnitude (with units / scale anchor), y-axis = effect magnitude (with units / scale anchor). An unlabeled curve cannot be falsified.
   - **Two anchor points minimum**: (a) origin behavior — what does y do as x → 0? (linear pass-through, threshold-then-rise, S-curve toe); (b) saturation point — at what x does y plateau, and at what y level? Without these two anchors the curve is impressionistic and Step 11.5 sensitivity sweeps will be uninterpretable.
   - **Compose with `cld-craft` fuzzy-variable sub-procedure** — the split-fuzzy-variable trick (Rules 7 / 7a / 7b / 7c there) handles non-monotonic curves (curves that flip direction) by splitting one fuzzy node into two monotonic sub-variables. Do not invent ad-hoc non-monotonic curves here; route through `cld-craft` first.
...
10. **Run the doubling-time-vs-response-delay diagnostic.** For any reinforcing loop suspected of producing exponential behavior, compute (or estimate) the doubling time T and the response-deployment delay D. The last possible action moment is when the threat occupies 1/2^(D/T) of the system. If D ≥ T, the visible-bend point comes *after* it is too late.
    - **Worked example**: doubling-time T = 2 years (an R-loop doubles output every 2 years), response-delay D = 3 years (your detection-plus-decision-plus-deployment lag is 3 years). Last useful action threshold = 1/2^(D/T) = 1/2^(3/2) ≈ 0.354 — i.e. **~35% of saturation**, not the intuitive 50%. By the time the trouble is visible enough to act on, the system is already 65% saturated and your remaining lever-throw is much smaller than the curve "looks like" it should be. Practical implication: in exponential R-loops with D > T, the actionable window closes at ≤20% saturation, not at "the warning signal" — wait for the warning signal and you have already missed it.
    - Completion: explicit T, D, and last-action threshold reported alongside any growth chart.
...
11.5. **Sensitivity sweep + falsification check** — for each fuzzy input variable (anything routed through a hand-drawn response curve, anything called "morale" / "reputation" / "saturation effect" / "switching propensity"), perturb the input ±50% from baseline and observe how the output behaves over the full simulation horizon. Three interpretation patterns:
    - **Output moves substantially with input** → this fuzzy input is a *dominant lever* (high information value; tighten the response curve, gather data, narrow the uncertainty) OR a *dead-link candidate* you accidentally landed on (verify with a second sweep at ±25% — if the relationship is roughly linear-monotonic, it is a real lever; if the output is wildly erratic, the curve is mis-shaped).
    - **Output is insensitive to fuzzy input** → either (a) the causal link is *dead* in this region of the state space (the loop you drew doesn't actually transmit) OR (b) the response-curve shape is wrong (your curve has near-zero slope where it should bite). Re-sketch the hand-drawn response curve referencing Stage-1 Step 7 axis-labels + two-anchor-points discipline before concluding the link is dead.
    - **Output flips direction or shows discontinuous behavior** (saturating / threshold / hysteresis signature) → the response curve is non-monotonic; the split-fuzzy-variable trick from `cld-craft` (Rule 7 / 7a / 7b / 7c) is required — split the offending node into two monotonic sub-variables and re-run.
    - **Completion criterion**: at least one ±50% sweep documented per fuzzy input. Inputs whose output is non-moving are flagged for re-examination, not silently passed. This is the falsifiability discipline the source book does not provide; without it, Stage 2 produces unfalsifiable narrative.
...
- **No falsifiability discussion (partially patched by Step 11.5)**: Sherwood gives no procedure for knowing when a stock-flow model is *wrong*. Calibration against historical data, out-of-sample testing, sensitivity analysis to fuzzy-input perturbation are all absent from the book's teaching. "Useful for understanding" is social-validation by stakeholder lightbulb-moment, not empirical. Step 11.5 (±50% fuzzy-input sweep + non-monotonicity detection) provides a minimum falsifiability floor that the source book lacks; for stronger guarantees, 2026 practice adds causal-inference DAGs + counterfactual reasoning + ABM-validation.
