...
**總計**：89 個 skill、42 個 slash command（橫跨 11 個 plugin）。

> 在自身 description 中標示 ⚠️ 的 plugin（`investing-toolkit`、`slides-toolkit`、`tsundoku`、`salesforce-toolkit`，以及 `obsidian` 中的 `defuddle` skill）必須在 Claude Code CLI 環境執行 — Cowork sandbox 會封鎖其對外網路或 subprocess 操作。
