...
guardrail (category reminder only; full method lives in spec-toolkit:
spec-expansion; do not reproduce it here), and a forward-pointer to
spec-toolkit:spec-expansion marked Tier 2 / deferred.
...
def test_dry_guardrail_reminder_only():
    ...
    method lives in spec-toolkit:spec-expansion; do NOT reproduce it here."""
    text = _text()
    low = text.lower()
    assert "spec-toolkit:spec-expansion" in text, \
        "DRY guardrail must name the SSOT: spec-toolkit:spec-expansion"
...
def test_forward_pointer_tier2_deferred():
    """A forward-pointer to spec-toolkit:spec-expansion for high-coverage /
    high-risk greenfield, marked Tier 2 and deferred (active once writing-plans
    reads OpenSpec change-folders)."""
    text = _text()
    low = text.lower()
    assert "spec-toolkit:spec-expansion" in text, \
        "forward-pointer must name spec-toolkit:spec-expansion"
