...
The Claude manifest (`code-toolkit/.claude-plugin/plugin.json`) is the single
source of truth for the shared plugin metadata. The Codex manifest
(`code-toolkit/.codex-plugin/plugin.json`) derives those same fields but adds a
...
Usage:
    python3 code-toolkit/scripts/sync_codex_manifest.py
        Rewrite the Codex manifest's shared fields from the Claude SSOT.

    python3 code-toolkit/scripts/sync_codex_manifest.py --check
...
ROOT = Path(__file__).resolve().parent.parent  # code-toolkit/
