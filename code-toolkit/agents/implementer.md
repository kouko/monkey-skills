---
name: implementer
description: 'Plugin-level implementer agent for code-toolkit''s SDD workflow. Dispatched for a single atomic task (≤5-min, ≤1 module) under TDD iron law. Produces code + tests + status (DONE / DONE_WITH_CONCERNS / NEEDS_CONTEXT / BLOCKED). Carries the 12-rule engineering baseline baked in. Reusable cross-plugin via subagent_type "code-toolkit:implementer".'
---
...
- `code-toolkit/skills/subagent-driven-development/SKILL.md` — SDD
  ...
  execution).
- `code-toolkit/skills/tdd-iron-law/SKILL.md` — the Iron Law you work
  under.
- `code-toolkit/skills/subagent-driven-development/agents/spec-reviewer-prompt.md`
