# 2026-05-15 — code-toolkit Design Lock Session
...
> **Branch**: `feat/code-toolkit-design`
> **Worktree**: `/Users/kouko/GitHub/monkey-skills/.worktrees/code-toolkit-design/`
...
After the comparison, user asked: 「**是否有可能依照當前 monkey-skill 專案的模式，做一個 code-toolkit plugin，整合 superpowers 與 code-team 兩方的優點，使用方式類似 superpowers 的工具？**」

Reply offered 3 design options:
- **Option A**: Standalone `code-toolkit/` with full knowledge layer self-contained
...
First worktree was created at `/Users/kouko/GitHub/monkey-skills-code-toolkit/` (sibling layout — git's default). User expected `.worktrees/` subdirectory layout. Checked `.gitignore` — already had `.worktrees/` (plural) rule pre-configured. Moved worktree to current location.
...
| **Q2** code-team 並存 | 並存（不 deprecate） | Mental model 不同 — code-toolkit = 主動建構入口（從零）；code-team = 被動 gate 入口（已產出）。不衝突 |
| **Q3** 啟動方式 | 先寫設計文件 | 規模夠大（9 skill + hook + SSOT 機制）需先把邊界定下，避免 Phase 1 蓋到一半發現要重做 |
| **Q4** 工作位置 | worktree `feat/code-toolkit-design` | 隔離 main；後續可拆 PR；user 偏好放 `.worktrees/` 子目錄（已配置 `.gitignore`） |
...
```
Process Layer (code-toolkit native — Superpowers-style)
  using-code-toolkit, brainstorming, writing-plans, SDD,
...
Knowledge Layer (SSOT-and-functional-copy from code-team)
  ...
  Drift gate: code-toolkit/scripts/verify-drift.py
```

### 3.2 SessionStart hook 預算

`using-code-toolkit/SKILL.md` ≤ **2000 tokens**（hook 注入預算限制）。其他 skill 全部 lazy load via `Skill` tool。

### 3.3 Cross-plugin delegation 契約

| Source (code-toolkit) | Target | Trigger |
|---|---|---|
...
衝突點：雙 SessionStart hook + skill 名稱重疊。Resolution: `CODE_TOOLKIT_MODE=off` env var 退場。
...
| OQ-5 | `CODE_TOOLKIT_MODE` env var 實作位置？ | Phase 1.5 |
...
```
code-toolkit/PRODUCT-SPEC.md   ← 商業 / 使用者 / Q-lock × 8
code-toolkit/TECH-SPEC.md      ← 架構 / SSOT / hooks / interface 契約
code-toolkit/ROADMAP.md        ← Phase 0-4 計畫 / Decision Ledger
code-toolkit/docs/superpowers/specs/2026-05-15-design-lock-session.md  ← THIS FILE
```
...
1. **PoC `using-code-toolkit/SKILL.md`** — 控制 ≤2000 tokens；包含 Red Flags 表 + Skill Priority + 中日英 trigger
2. **PoC `hooks/session-start` + `hooks/hooks.json`** — bash 注入 + JSON 雙 key（Claude Code + Codex 兼容）
3. **Verify hook in fresh session** — 開新 Claude Code session，確認 hook 真的注入了 using-code-toolkit
...
- 開新 session → `using-code-toolkit` 自動載入（hook 生效）
...
```
我要繼續開發 code-toolkit plugin（monkey-skills marketplace 新 plugin）。

請先讀這 4 份文件，理解 Phase 0 已鎖定的決策：
1. code-toolkit/PRODUCT-SPEC.md
2. code-toolkit/TECH-SPEC.md
3. code-toolkit/ROADMAP.md
4. code-toolkit/docs/superpowers/specs/2026-05-15-design-lock-session.md ← 這份是 handoff，先讀！
...
我們從 hook + using-code-toolkit/SKILL.md PoC 開始。
```
...
- ❌ 直接動手寫 `using-code-toolkit/SKILL.md` — Phase 0 只做設計
