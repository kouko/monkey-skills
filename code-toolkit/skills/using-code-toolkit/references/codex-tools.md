...
Claude Code has **no such limit**. This is why 2 of `code-toolkit`'s skills had their `description` trimmed to load under Codex — keep every skill's `description` ≤ 1024 chars to stay dual-harness portable. After the trim, all 11 skills load under Codex 0.139.0 (verified).
...
codex plugin add <PLUGIN>@<MARKETPLACE>                  # ✓ exercised: `add code-toolkit@monkey-skills`
codex plugin add <PLUGIN> --marketplace <name>           # equivalent (from `plugin add --help`, not exercised)
codex plugin list                                        # ✓ exercised
codex plugin remove <PLUGIN>                             # ✓ exercised: `remove code-toolkit@monkey-skills`
```
...
|---|---|
| using-code-toolkit | `/using-code-toolkit` (also injected via SessionStart hook) |
...
If a skill name conflicts with another installed plugin (e.g. `obra/superpowers` ships `/brainstorming` too), use the plugin-scoped form: `/code-toolkit:brainstorming`.
...
Per TECH-SPEC §3.3-3.4, code-toolkit's `subagent-driven-development` skill dispatches three subagents per atomic task (implementer / spec-reviewer / code-quality-reviewer).
...
Claude Code reads `CLAUDE.md` for project conventions; Codex reads `AGENTS.md`. The skill bodies in code-toolkit reference `CLAUDE.md` (the original target); Codex users should mirror their `CLAUDE.md` to `AGENTS.md` for the rules to apply. Some plugins ship both, symlinked or duplicated. (Exact mirroring requirement **assumed** — not exercised in this test.)
