...
| P15-12 (Phase 2) | Promote remaining 4 agents to plugin-level (`spec-reviewer` / `code-quality-reviewer` / `reviewer` / `debugger`)；each gets baseline injection；CHK-SKL-012 false-positive 自然消解（per-skill agents/ 搬空） | Phase 1 ritual PASS 之後執行 — 是 v0.6.0 minor-version scope | ⏳ 排在 v0.6.0 — unblocked by Phase 1 ritual PASS |
...
- ⏳ P15-12-Phase2（v0.6.0 scope，unblocked — Phase 1 ritual PASS confirmed plugin-level agent dispatch + baseline injection mechanic）
