...
| # | Goal | 衡量 |
|---|---|---|
| G1 | Plugin installable from monkey-skills marketplace（Claude Code + Codex CLI） | `/plugin install code-toolkit` 跑得通 + `using-code-toolkit` 出現在 skill 列表 + Codex CLI 也能載入 |
| G2 | SessionStart hook 自動注入 `using-code-toolkit`（不靠 agent 主動呼叫） | 開新 Claude / Codex session，第一條訊息送 "Let's add a feature to X" → agent 自動 trigger brainstorming（沒要寫 code） |
...
| G5 | 知識層 SSOT-and-functional-copy 通過 drift gate | `code-toolkit/standards/*` 與 `domain-teams/skills/code-team/standards/*` byte-identical；`scripts/verify-drift.py` 綠燈 |
...
| 非目標 | 為什麼拒絕 |
|---|---|
| ❌ 取代 `domain-teams:code-team` | code-team 並存 — 它是被動 gate 評分入口；code-toolkit 是主動建構入口。兩個用途不同 |
| ❌ 跨 7 個 harness（Gemini / Cursor / Copilot / OpenCode / Droid） | Phase 1-3 只做 Claude Code + Codex CLI；其他 harness 等使用者要求再加 |
| ❌ 自己重寫 8 本書的 standards | 走 SSOT-and-functional-copy：canonical 留在 `domain-teams:code-team/standards/`，code-toolkit 用 distribute.py 拷過來 |
| ❌ Visual brainstorming / brainstorm server（Superpowers 5.1.0 的新 feature） | Phase 4+；MVP 不做。Superpowers 自己也是後加的 |
| ❌ 取代 `dev-workflow:complexity-critique` | complexity-critique 已存在；code-toolkit 在 brainstorming 階段可選擇性引用，但不重複實作 |
| ❌ 取代 `dev-workflow:git-memory` | git-memory 已是 commit 前的 gate；code-toolkit `finishing-a-development-branch` 直接 delegate 過去 |
| ❌ 取代 `skill-dev-toolkit:skill-creator-advance` / `skill-judge` / `skill-refactor` | 那些是「寫 skill」的 meta toolkit；code-toolkit 是「寫 code」的 toolkit。不重疊 |
...
| 維度 | Superpowers v5.1.0 | code-toolkit v0.1.0 (目標) |
|---|---|---|
...
| 維度 | `domain-teams:code-team` | code-toolkit |
|---|---|---|
...
```
使用者: "幫我加一個 feature X"
  ↓ session-start hook 注入 using-code-toolkit
code-toolkit:brainstorming → 探索意圖 → spec
code-toolkit:writing-plans → 切原子任務
code-toolkit:subagent-driven-development → 每任務 dispatch 3 subagent
  ↓ spec-reviewer / code-quality-reviewer 內部呼叫
  ↓ 載入 code-toolkit/standards/* (functional copy of code-team SSOT)
  ↓ 載入 code-toolkit/rubrics/quality-gate.md
  ↓ 評分 PASS / PASS_WITH_NOTES / NEEDS_REVISION
code-toolkit:finishing-a-development-branch → PR / merge
...
| 風險 | 影響 | Mitigation |
|---|---|---|
| **Drift**：code-team 改了 standards，code-toolkit 沒同步 | reviewer 用過時規則評分 | `scripts/verify-drift.py` CI gate（mirror legal-toolkit Phase 1.10 模式） |
| **Hook 干擾**：SessionStart hook 注入太多 token，每個 session 都付 cost | session 啟動慢 / token 成本 | hook 只注入 `using-code-toolkit/SKILL.md`（控制 ≤2000 tokens）；其他 skill 走 lazy load |
| **與既有 skill 衝突**：使用者也裝了 `obra/superpowers` | 雙 hook 同時注入；trigger 互搶 | README 明寫 conflict；提供 `code-toolkit-only-mode` env var |
...
| **與 `dev-workflow:complexity-critique` / `git-memory` 重疊** | 使用者搞混入口 | router skill `using-code-toolkit` 內附決策表，明示何時 delegate 出去 |
...
| OQ-5 | 是否提供 `code-toolkit-only-mode` env var 來關掉 hook（給已裝 Superpowers 的使用者）？ | Phase 1.5 |
...
| Q2 (code-team 去留) | **並存** — code-toolkit 主動建構入口，code-team 被動 gate 入口 | 兩用途不衝突；不破壞既有 `dev-workflow:complexity-critique` 對 code-team mindset 的 SSOT 指向 |
| Q3 (啟動方式) | **先寫設計文件（PRODUCT-SPEC / TECH-SPEC / ROADMAP）** | 規模夠大需要先把 SSOT 邊界 / Phase 切分定下來，避免 Phase 1 蓋到一半發現要重做 |
| Q4 (工作位置) | **新開 git worktree `feat/code-toolkit-design`** | 不影響 main；後續 Phase 拆 PR 容易 |
| Q5 (Knowledge layer SSOT) | **SSOT 留 `domain-teams:code-team/standards/`；code-toolkit 用 `scripts/distribute.py` 拷 byte-identical functional copy** | 符合 monkey-skills 既有慣例（legal-toolkit Phase 1.10）；避免雙份維護 |
| Q6 (Skill naming) | **Superpowers 風格** — `using-code-toolkit` / `brainstorming` / `writing-plans` / `subagent-driven-development` / `tdd-iron-law` 等 | 使用者要求「使用方式類似 superpowers」 |
