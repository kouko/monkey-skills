...
#
# Verify code-toolkit's brainstorming skill correctly references
...
BRAINSTORMING_SKILL="${REPO_ROOT}/code-toolkit/skills/brainstorming/SKILL.md"
...
Offline checks PASSED. Live verification (manual, in fresh Claude session):
  ...
  1. cd /Users/kouko/GitHub/monkey-skills/.worktrees/code-toolkit-design
  ...
  4. Expected agent behavior:
     - Skill(code-toolkit:brainstorming) auto-loads
     ...
     - Reference: code-toolkit/tests/brainstorming-pressure/prompts/
  ...
  5. PASS if agent mentions or invokes dev-workflow:complexity-critique
     ...
     code-toolkit/ROADMAP.md §Cross-plugin delegation)
EOF
exit 0
