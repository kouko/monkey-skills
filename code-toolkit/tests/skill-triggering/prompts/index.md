...
Each `.txt` in this directory is a single user prompt, copy-pastable into a fresh Claude Code / Codex CLI session **with `code-toolkit` installed** (so the SessionStart hook injects `using-code-toolkit/SKILL.md`).
...
| Acceptance | Rule |
|---|---|
| MUST | Reply mentions `using-code-toolkit` was loaded (or routes through it implicitly) |
...
| Acceptance | Rule |
|---|---|
| MUST | Reply mentions `using-code-toolkit` and `tdd-iron-law` |
...
**Maps to**: §When NOT to use in `using-code-toolkit/SKILL.md` — pure read-only questions are out of scope. Tests that the router does not over-trigger.
...
```bash
# Prereq: code-toolkit installed in the host (Claude Code / Codex CLI)
...
5 / 5 PASS = Phase 1 acceptance §"using-code-toolkit auto-loaded via hook + appropriate routing per task type" met.
