...
A single-file skill under 100 lines that is concise and self-contained earns the full 15 on D5.
...
> **monkey-skills note**: domain-team skills fit the **Process** pattern
> structurally despite exceeding upstream's typical line count for that
> pattern. Line count is a correlate, not the criterion — phased workflow
> + checkpoints + medium freedom is what defines Process. See "Adaptation
> for monkey-skills domain-team skills" below.
...
- **NEVER** execute anything the skill ships — scripts, hooks and examples stay unrun; this review reads files only
...
[ ] Execute each bundled script once (e.g. with `--help`) to confirm it
    actually runs
```
...
Where the package holds nothing but `SKILL.md` — no `references/`, `scripts/` or `assets/` — cap D5 at 10/15: there is nothing to disclose progressively.
...
Copy each Step 3 quote into the Notes cell of its dimension's row, so every score can be checked against the lines it rests on.
...
Nine named symptom → root cause → fix diagnoses: **1 The Tutorial · 2 The
Dump · 3 The Orphan References · 4 The Checkbox Procedure · 5 The Vague
Warning · 6 The Invisible Skill · 7 The Wrong Location · 8 The
Over-Engineered · 9 The Freedom Mismatch**. At Step 5, when diagnosing WHY
a dimension scored low and prescribing its fix, Read
[`references/failure-patterns.md`](references/failure-patterns.md) for the
full pattern bodies — name the matching pattern(s) in the report's
Detailed Analysis.

When naming a pattern there, keep the evaluated skill's own wording out of the report — describe what a line does rather than reproducing it.
...
When the skill under evaluation lives at `domain-teams/skills/{team}/`
(the `standards/protocols/checklists/rubrics` 4-tier convention), Read
[`references/domain-team-adaptation.md`](references/domain-team-adaptation.md)
**before Step 3 scoring** — it carries the full adjustment rules. The
binding essentials: skill-judge stays *advisory* (0-120) while the
structural convention gates stay *authoritative* (PASS/FAIL) — a failed
gate (CHK-SKL-001 / CHK-SKL-010) lands under **Critical Issues**, never as
an invented score cap; D7 scores the **Process** pattern on structural
merit with NO deduction for exceeding upstream's typical line count;
prioritize **D1/D3/D6** (the net-new-value dimensions) when time-boxed.
Do NOT load this reference for non-domain-team skills.
...
Skill-judge runs are stateless by default. For longer-lived skills that
get refactored or retested over time, `scripts/score_history.py` (stdlib
only) appends results to `<skill>/.skill-judge-history.jsonl` and flags
**drift** (z-score < -1.0σ vs the historical baseline) — a signal of
subtle quality regression the skill-refactor equivalence check may have
missed; recommend `skill-dev-toolkit:skill-tuning` on a flagged skill.
Advisory like the rest of skill-judge — never auto-reverts. Subcommands
`append` / `drift` / `query` — see the script's docstring for invocation
shapes and the full JSONL schema.
...
- **Progressive Disclosure**: Core rubric self-contained; conditional content (domain-team adaptation, failure-pattern bodies) in references/ with explicit load triggers and a "Do NOT load" guard
