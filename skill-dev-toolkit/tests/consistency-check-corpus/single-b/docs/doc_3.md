...
Survivors go forward to Phase 3 with the editor's one-line
dimension note ('warmer tone', 'list-first') printed under each
output, so the user can judge each variant against what it set
out to change.
...
Append the round's preference-log entry as soon as the outputs
are on screen, before the question is asked — if the session
dies mid-round, the log still shows that the round ran.
...
When several reviewers take part, run the A/B as one live
session: they talk the outputs through together first, then each
states a pick — the discussion surfaces reasons worth keeping in
the log's `notes` field.
...
When a round ends in DROP, never append to the preference log — a
round where nothing beat the baseline carries no preference for a
future judge to learn from.
