...
A claim stated with an absolute (*always*, *all*, *none*) MUST first be
restated in the narrower, conditional form the evidence can actually settle
— "all swans are white" becomes "swans in Europe are white" — and that
restatement is the claim under check from here through Stage C.
...
2. Rank/filter the raw hits into structured results. Treat the claim as a
   ...
   Grade `relevance` from the full page, not the search snippet: `high` only
   when the page's body squarely addresses the claim; a matching title or
...
Spawn each voter as a fork of this conversation rather than a blank agent —
it then already holds the Stage-A pool and your own reading of the claim, so
the per-voter prompt can stay short.
...
A voter that never comes back still gets a slot: fill it with a non-refuting
verdict (`refuted: false`, `confidence: low`) so `factcheck.py` always
receives three well-formed votes.
