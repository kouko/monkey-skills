...
This is why the baseline is not reserved for the full eval path: even on
the quick path, each manually run test case is paired with a baseline run
so the user can review the two outputs side by side.
