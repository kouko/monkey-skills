...
3. **Obvious instruction violations** — The skill explicitly says "always
   include X" or "never do Y" and the output violates this. A common
   case: a skill step that relies on an `AskUserQuestion` gate, where the
   with-skill transcript shows the question inlined as plain text instead
   of an actual `AskUserQuestion` tool call — treat that as an instruction
   violation, fix the skill, and rerun
