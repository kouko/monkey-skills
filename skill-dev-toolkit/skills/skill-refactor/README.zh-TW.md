...
- **Skill 輸出不好 / 錯誤** — 用 `skill-dev-toolkit:skill-tuning`
  — 品質改進需人類 A/B，不是等價保留 refactor
- **想加 phase / 改 agent / 重組 workflow** — 用
  `skill-dev-toolkit:skill-creator-advance` — 結構性重寫是 feature hat
- **建立新 skill** — 用 `skill-dev-toolkit:skill-creator-advance`
...
- **a proposal-triage gate** — 多個 refactor 提案要 triage
  時，先決定哪個先做
- **a complexity / deletion-first gate** — 問題是「該不該 refactor
  這個 skill」時（smallest-end-state 思考再呼叫 refactor）
- **`skill-dev-toolkit:skill-creator-advance`** — 改動是結構性的（加
  phase / 改 agent / 重設計）
- **`skill-dev-toolkit:skill-tuning`** — 問題從「輸出等價
  嗎」變成「哪個輸出更好」時
- **`skill-dev-toolkit:skill-judge`** — 可選 advisory；refactor 前 / 後
...
| **創意 skill 的「輸出保留」定義不清** | 寫作風格 / 散文 skill 沒有客觀「等價」 | 自動 abort 並推薦 `skill-tuning` |
...
│   ├── golden-anchor-protocol.md       ← 共用 convention（skill-tuning 也有）
