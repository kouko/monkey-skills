...
行層**（目前的 `slides-builder`，未來可加 `html-builder` /
...
**Scope Refinement（2026-04-23 v0.3 追補）**：MVP 進一步**以 runtime
minimalism 為重**——template 視覺品質（使用者自備 deck + registry.md
lookup）與 supply-chain SHA-256 pin 皆移出 MVP，改為 Phase 2+
trigger-gated。**Because** 個人使用閉環下，template 管理 overhead 大
於視覺品質邊際、SHA pin 維護成本大於 HTTPS + 官方 GitHub release 的
邊際安全增益；MVP 改用 Google 內建 predefined layouts（`TITLE` /
`TITLE_AND_BODY` / `SECTION_HEADER` 等）+ HTTPS + `curl -f` 已足以驗
證「3 core recipe + 3 分鐘」核心假設。
...
| 從原始碼編譯 `gws` / `jq` binary（build-from-source） | MVP 使用**官方 GitHub release 的預編譯 binary**；自編不增加安全保證（仍需信任原始碼 + toolchain），卻引入 Rust / C toolchain 與跨平台 build matrix 的依賴，違反「runtime minimalism」與「純 shell」核心限制 | 出現 supply-chain 信任事件需要 reproducible build、或 upstream release 嚴重延遲 |
| Template-based workflow（使用者自備 template deck + `registry.md` 查找 Drive ID） | 個人使用閉環下 template 管理 overhead（維護 template deck、維護 registry.md、處理 placeholder 漂移）大於視覺品質邊際增益；且 template schema 不穩定會在 MVP 引入額外 error surface（`[OPEN-10]` 原擬解問題）。MVP 改用 **Google 內建 predefined layouts**（`TITLE` / `TITLE_AND_BODY` / `SECTION_HEADER` / `TITLE_AND_TWO_COLUMNS` 等）+ `presentations.create` 建空 deck + Claude 在 `slide-plan.json` 明示 `layout_hint` enum；足以達成基本可讀性且零 template 管理成本 | 使用者對 Google 預設 layout 視覺品質不滿（字型 / 色票 / spacing 明顯不合需求）、或需要跨 deck 一致品牌視覺（logo / 配色 / 字型系統） |
| Binary supply-chain SHA-256 pin verification（對 `gws` / `jq` 做 shasum 比對） | 個人使用情境下（單人閉環、非 CI、未對外發布），SHA pin 維護成本（每次 upstream release 需更新 pin、跨 arch 需維護多組 hash）大於邊際安全增益；HTTPS 傳輸完整性 + GitHub release 官方 org 信任邊界 + `curl -f` 失敗即 abort 已足以防禦常見「下載錯誤 / 網路中斷」失效模式，不防禦的只剩「GitHub release 本身被植入惡意 binary」這類低機率威脅 | publish 給外部使用者（信任邊界擴大、SHA 成為最小契約）、或加入 CI（自動化流程需 deterministic integrity gate）、或發生實際 supply-chain 安全事件（upstream compromise）|
...
- `[ASSUMPTION-2]`（Value，high impact / medium evidence）
  「把 40–90 分鐘的工作壓到 ≤ 3 分鐘是可達成的——即『`presentations.create`
  建空 deck + 指定 Google 內建 predefined layout + replaceAllText +
  insert local image』四個 core recipe 足以覆蓋 kouko 實際簡報的
  ≥ 80% 場景。」
  - **Validation**：MVP 期間 kouko 實跑 ≥ 10 份真實 deck，記錄哪
    些場景 fallback 到手動。若 ≥ 3 份需要手動，重評 scope。
  - **⚠️ Revalidation required（v0.3 Scope Refinement）**：原假設基
    於「使用者自備 template deck」能覆蓋 80% 場景；v0.3 刪除 template
    workflow 後，此覆蓋率需以「Google 內建 predefined layouts
    （`TITLE` / `TITLE_AND_BODY` / `SECTION_HEADER` 等）」重新驗證。
    若 Google 預設 layout 覆蓋率 < 80%，觸發 §3.5「Template-based
    workflow return」trigger（而非直接擴大 MVP）。
- `[ASSUMPTION-3]`（Feasibility，high impact / weak evidence）
  ...
  掉。」
  - **Validation**：MVP 實裝時先驗 workaround，並在 `gws-setup`
...
> 「透過純 shell + gws binary + **Google 內建 predefined layouts**
> （`TITLE` / `TITLE_AND_BODY` / `SECTION_HEADER` / ...），能在不裝
...
- ✅ 3 個 core recipe：`presentations.create`（建空 deck，指定
  predefined layout）/ `replaceAllText`（填入文字）/ `insert local
  image`（上傳本機圖片到 layout placeholder）
- ✅ `slide-plan.json` 內每頁以 `layout_hint` enum 明示指定 layout
  （從 Google 內建 predefined layouts 挑選）
- ✅ Minto Pyramid + SCQA narrative 基礎 reference（來源錨定於
  設計知識層；見 §6.2）
- ✅ Chart-selection 對照（哪種資料用哪種圖表；不含生成）
- ✅ 4 skills 架構（router / gws-setup / slides-design /
  slides-builder；後兩者 backend-agnostic / Google Slides-
  specific 的分工見 §6.3.1）
...
- Learn → 哪些場景 recipe 不夠、shell 組 JSON 何時變痛、設計知識
  層哪塊需要深化、Google 預設 layout 視覺品質在哪類場景不足（觸發
  template-based workflow return trigger）
...
| **Template-based workflow return**（使用者自備 template deck + `registry.md` 查找 + template-copy + placeholder replaceAllText 的完整流程） | 實際使用後發現 Google 內建 predefined layouts 視覺品質不足（kouko 對字型 / 色票 / spacing 明顯不滿、或超過 20% 場景需要手動後製調整），或需要跨 deck 一致品牌視覺（logo / 配色 / 字型系統不可由 Google 預設層覆蓋）。Trigger 達成時，恢復 `skills/slides-builder/templates/registry.md` + template-copy recipe，並重新觸發 `[ASSUMPTION-2]` 80% 覆蓋率重評 |
| **SHA-256 supply-chain pin**（對 `gws` / `jq` binary 做 shasum 比對、mismatch 時 `bootstrap.sh` 回 exit 17 + abort） | publish 給外部使用者（信任邊界擴大，SHA 成為最小契約）、加入 CI（自動化流程需 deterministic integrity gate）、或實際發生 supply-chain 安全事件（upstream release 被植入惡意 binary / GitHub org 被攻陷等事件）|
...
本週提案是某 SaaS 產品建議書：在 Claude Code 打 `/slides`，Claude 根
據輸入內容組 `slide-plan.json`——每頁明示 `layout_hint` enum（e.g.
p1 `TITLE` / p2 `SECTION_HEADER` / p3–p8 `TITLE_AND_BODY` 或
`TITLE_AND_TWO_COLUMNS` / p9 `TITLE`）。`slides-builder` 自動：
(1) `presentations.create` 建一份空 deck，(2) 依 `layout_hint` 逐頁
`batchUpdate` 建 slide（指定 Google 內建 predefined layout），
(3) `replaceAllText` 填入各頁標題 / 內文，(4) 上傳本機圖片到 layout
的 image placeholder，(5) 回傳 Drive URL。kouko 打開 URL 做最後 5 分鐘
的排版微調。**總耗時：3 分鐘 skill 執行 + 5 分鐘人工微調 = 8 分鐘**
（vs 原本 60 分鐘）。

**Scenario B — 工作週報（~25% 場景）**

規格化重複：kouko 把本週 progress 條列成 JSON（標題頁 + 若干
`TITLE_AND_BODY` 內容頁），skill 建空 deck + 套 predefined layout +
replaceAllText，輸出 URL，幾乎無需後續微調。**總耗時：2 分鐘**（vs 原
本 40 分鐘）。

**Scenario C — 對外分享 / 技術 talk（~15% 場景）**

kouko 需要一份內容設計更精細的簡報：先用 `slides-design` 拿到 Minto
/ SCQA 結構建議 + chart-type 對照 + layout 選擇建議（哪些頁該用
`SECTION_HEADER` 切段、哪些適合 `TITLE_AND_TWO_COLUMNS`），再自己組
`slide-plan.json`，最後用 `slides-builder` 合成。**總耗時：12 分
鐘**（vs 原本 90 分鐘；設計部分仍需人工判斷，只省了機械勞動）。
...
- **使用 Google 內建 predefined layouts**（`TITLE` / `TITLE_AND_BODY`
  / `SECTION_HEADER` / `TITLE_AND_TWO_COLUMNS` 等）— 零 template 管
  理、零 Drive ID 洩漏風險、零 template schema drift；Claude 在
  `slide-plan.json` 用 `layout_hint` enum 明示指定。繞開「AI 自動設
  計投影片」這個 MVP 不打算解的難題，聚焦
  `presentations.create` + `replaceAllText` + `insert image` 三個可
  靠操作
- **Skill 化 + 知識層分離** — `slides-design`（知識，backend-agnostic）
  / `slides-builder`（執行，Google Slides backend）解耦，讓
...
2. **Layout-based，不自動生成** — 使用 Google 內建 predefined
   layouts（`TITLE` / `TITLE_AND_BODY` / `SECTION_HEADER` /
   `TITLE_AND_TWO_COLUMNS` / ...）；Claude 在 `slide-plan.json` 的
   `layout_hint` enum 明示指定每頁用哪個 layout，skill 依此
   `presentations.create` + `batchUpdate` 建 slide，不做 layout 自
   動選擇。**Because** (a) template 管理是個人使用閉環下的 overhead
   （維護 deck、維護 registry、處理 schema drift）不值得付；(b)
   Google 內建 layout 足以達成基本可讀性，覆蓋 kouko 多數場景（此
   假設待 `[ASSUMPTION-2]` revalidation）；(c) 自動選 layout
   （Option B：Claude 判斷每頁內容 → 自動挑 layout）屬 Phase 2+ 範
   圍，MVP 仍由 Claude 明示給出。原 template-based 模型改為
   trigger-gated Phase 2+（見 §3.5）。
3. **知識層與執行層分離** — `slides-design`（reference / rubric，
   backend-agnostic）與 `slides-builder`（gws 呼叫 + merge，
...
```
┌──────────────────────────────────────────────────────────────┐
│  Entry: Claude Code 內 /using-gws-toolkit                 │
...
└───────────────────────┬──────────────────────────────────────┘
        ...
        ▼               ▼                           ▼
┌───────────────────┐ ┌──────────┐   ┌──────────────────────────┐
│ google-slides-    │ │ slides-  │   │ slides-builder    │
...
└────────┬──────────┘ │ backend- │   └──────────┬───────────────┘
         ...
         ▼            └────┬─────┘              ▼
   gws 安裝 +              │                 gws 呼叫
   GCP Console             ▼                 presentations.create (空 deck)
   + OAuth auth       Minto/SCQA             + batchUpdate (layout_hint enum)
                     + chart-type            + replaceAllText
                     + layout 建議            + insert image
                       reference                   │
                     (對任何 backend 適用)          ▼
                                           Google Slides URL
...
├── skills/
│   ├── using-gws-toolkit/            # router（通用；依需求選 backend）
│   ├── slides-design/                   # 設計知識層（backend-agnostic）
│   ├── gws-setup/             # [renamed] gws + GCP Console onboarding
...
│   └── slides-builder/           # [renamed] 高層 orchestration：
...
│                                        #   bootstrap.sh：HTTPS 下載 + curl -f 失敗檢查
│                                        #   （v0.3: 不做 SHA-256 pin；見 §6.3.2）
...
- **`docs/` 不另設目錄** — PRODUCT-SPEC.md / TECH-SPEC.md 直接放
  ...
  artifact，放 `skills/slides-builder/templates/registry.md`
...
| Google 預設 predefined layout 視覺品質不足 kouko 場景（v0.3 取代原「template schema 不穩定」風險） | 中（新假設，待 revalidation） | 中（觸發 trigger、擴增 MVP scope） | MVP 期間 kouko 人工判斷；Build-Measure-Learn loop 的 Measure 項目明列「Google 預設 layout 是否覆蓋 ≥ 80% 場景」；若 < 80% 觸發 §3.5「Template-based workflow return」trigger |
| Binary supply-chain integrity（抓到被竄改的 gws / jq binary） | 低（GitHub release 官方 org） | 高（執行惡意 binary） | **v0.3 Mitigation 重寫**：HTTPS 傳輸信任 + 版本 pin（URL pin 到特定 release tag）+ 個人 scope 限定（未 publish / 無 CI）+ `curl -f` 失敗即 abort。**不做** SHA-256 shasum 比對。Phase 2+ trigger（見 §3.5）：publish 給外部、加入 CI、或發生實際 supply-chain 安全事件時恢復 SHA pin |

#### 6.3.4 External dependencies

**MVP（Google Slides backend 唯一啟用）**

- `gws` binary（Rust 靜態 binary，official GitHub releases；HTTPS +
  `curl -f` 抓取，v0.3 不做 SHA-256 pin）
- Google Cloud Console（一次性 OAuth 設定——4 步手動）
- Google Slides / Drive API（via gws；使用 `presentations.create` +
  Google 內建 predefined layouts，無 template deck 依賴，無
  `registry.md`）
- `jq`（由 skill 自抓 binary；HTTPS + `curl -f`，v0.3 不做 SHA-256 pin）
...
適用於 `google-slides-api` / `slides-builder` / `gws-setup`
...
適用於 `slides-design/*` + `using-gws-toolkit` routing table。理由：
...
- `[OPEN-3]` `slides-builder` 的 gws 呼叫：是否包裝成單一
  shell function，還是分多個 recipe script？
- `[OPEN-4]` `gws-setup` 的 state detection 實作：如何
  檢測 token 過期 / 未登入 / 未裝 gws 三種狀態的分支？
- `[OPEN-5]` `jq` binary 自抓的來源、版本 pin 策略？
  （**v0.3 已解決 integrity check 部分**：不做 SHA-256 shasum；採
  HTTPS + `curl -f` 失敗檢查。來源與 version pin 仍留 TECH-SPEC）
- `[OPEN-6]` gws binary 自抓策略：讓使用者手動裝，還是由
  `gws-setup` 自動抓？若自動抓，version pinning 策略？
  （**v0.3 已解決 integrity check 部分**：同 `[OPEN-5]`，不做 SHA-256
  shasum；採 HTTPS + `curl -f` 失敗檢查）
...
- `[OPEN-10]` ~~Template deck schema 校驗：MVP 是否需要 schema
  ...
  **已解決（v0.3 Scope Refinement）**：MVP 移除 template-based
  workflow，改用 Google 內建 predefined layouts；template schema
  fingerprint 問題不再存在。若 Phase 2+ trigger 達成恢復 template
  workflow，此問題會隨之回歸，屆時重開
...
| `skill-team` | 4 份 SKILL.md：`using-gws-toolkit`（router，不變）/ `slides-design`（知識層，不變，backend-agnostic）/ `gws-setup`（renamed from `slides-setup`）/ `slides-builder`（renamed from `slides-builder`） | 本 PRODUCT-SPEC §4 §5 §6.2 §6.3 |
| `docs-team` | README.md / GCP Console 逐步截圖 / Google predefined layouts 對照表（v0.3：`registry.md` template 產出項已刪除，因應 template workflow 移除） | 本 PRODUCT-SPEC §5.1 §6.3 |
...
- ~~`registry.md` 為 runtime artifact（非 spec 層級）~~
  **v0.3：`registry.md` 已隨 template workflow 移除，不再是 runtime
  artifact**
