...
If status fails, route the user to the `kobo-auth` skill (Flow A for new
...
| Path | Role |
|---|---|
| `scripts/kobo_query.py` | Filter `--export-library` JSON, multiple output formats |
| `scripts/kobo_get.sh` | Download books by RevisionId (args or stdin), idempotent |
| `$TSUNDOKU_KOBO_BINARY` | The kobodl CLI itself |
| `$TSUNDOKU_KOBO_LIBRARY_JSON` | Cached export at `~/.cache/tsundoku/library.json` (regenerable) |
...
Map the user's natural-language request to `kobo_query.py` filters:
...
Multiple filters AND-combine. Run `kobo_query.py --help` for the full list.
...
python3 ${CLAUDE_SKILL_DIR}/scripts/kobo_query.py \
...
python3 kobo_query.py --library "$TSUNDOKU_KOBO_LIBRARY_JSON" [filters] \
...
`kobo_get.sh` accepts RevisionIds via positional args OR stdin.
Skips books whose `<id8>.epub` already exists in `$TSUNDOKU_DOWNLOADS`.

```bash
# Single book (positional)
bash ${CLAUDE_SKILL_DIR}/scripts/kobo_get.sh "$REVISION_ID"

# Multiple books (positional, space-separated)
bash ${CLAUDE_SKILL_DIR}/scripts/kobo_get.sh "$ID1" "$ID2" "$ID3"

# Pipe from query (most common — filtered set)
python3 ${CLAUDE_SKILL_DIR}/scripts/kobo_query.py \
    --library "$TSUNDOKU_KOBO_LIBRARY_JSON" --series "Silent Witch" --format ids \
  | bash ${CLAUDE_SKILL_DIR}/scripts/kobo_get.sh

# Preview before committing (show what would download)
python3 ${CLAUDE_SKILL_DIR}/scripts/kobo_query.py [...] --format ids \
  | bash ${CLAUDE_SKILL_DIR}/scripts/kobo_get.sh --dry-run

# Convert each to PDF after download (requires Calibre)
bash ${CLAUDE_SKILL_DIR}/scripts/kobo_get.sh --convert-pdf "$REVISION_ID"

# Custom output directory for this run
bash ${CLAUDE_SKILL_DIR}/scripts/kobo_get.sh --output-dir ~/Books/research "$ID"
```
...
`kobo_query.py` filters (all optional, AND-combined):
...
**Caveats**:
- `Description` is HTML; `kobo_query.py` strips it for matching/output.
...
# Step 4: download
bash ${CLAUDE_SKILL_DIR}/scripts/kobo_get.sh "$picked_id"
```
...
```bash
"$TSUNDOKU_KOBO_BINARY" --config "$TSUNDOKU_KOBO_CONFIG" book list \
    -u alice@example.com --export-library "$TSUNDOKU_KOBO_LIBRARY_JSON"

bash kobo_get.sh --user alice@example.com "$REVISION_ID"
```
...
- This skill is macOS-specific. Linux/Windows users install kobodl via
  `pipx install kobodl` and override `TSUNDOKU_KOBO_BINARY` after sourcing
  `tsundoku_paths.sh`
