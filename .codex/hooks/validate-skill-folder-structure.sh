#!/usr/bin/env bash
# Thin shim over .claude/hooks/validate-skill-folder-structure.sh (the real
# check). Written by W1-02 — the command string below must never change:
# Codex binds hook trust to the DEFINITION, not to file content.
#
...
TARGET="$HERE/../../.claude/hooks/validate-skill-folder-structure.sh"
# Read stdin once — the recorder and the real hook must see the same
# payload, and a pipe can only be drained once.
INPUT=$(cat 2>/dev/null || true)
if [ ! -x "$TARGET" ]; then
  echo "BLOCK: .codex/hooks/validate-skill-folder-structure.sh cannot find its target $TARGET" >&2
  exit 2
fi
