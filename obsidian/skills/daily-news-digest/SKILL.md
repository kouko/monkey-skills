...
>   `scripts/cot_mermaid.py`, writes the file, and runs the link self-check** —
...
> ```
> The CoT `nodes` carry only content; `scripts/cot_mermaid.py` applies the fixed
...
through `scripts/cot_mermaid.py`** (feed it node content JSON), so the fixed
role-colours and style are identical everywhere; don't hand-write the `style`
lines. Follow the vault's Obsidian conventions
...
> [!important] 寫完跑連結解析自檢
> After writing the digest, **verify every wikilink resolves** before declaring
> done — extract all `[[…]]` targets and confirm a `<stem>.md` exists in the
> vault. This catches typos, truncated-title mistakes, and stale stems. A quick
> check:
> ```bash
...
- **One file per day.** If `news/<date> 每日新聞.md` already exists, ask whether to
  overwrite or merge before clobbering it.
...
- `scripts/cot_mermaid.py` — renders CoT node **content** (JSON) → coloured Mermaid
  ...
  byte-consistent; subagents emit node content, the main agent renders.
