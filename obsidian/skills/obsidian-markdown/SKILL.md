...
3. **Write content** using standard Markdown for structure, plus Obsidian-specific syntax below.
   - When unsure whether content should be a diagram, table, callout, list, or prose, see the [visualization decision guide](references/viz-decision-guide.md). It picks the form only.
...
1. **Non-flowchart type** — the diagram is not a plain directional flow. This covers:
   ...
   `gitGraph`, `gantt`, `timeline`, `mindmap`, `xychart-beta`, `pie`, `quadrantChart`,
