...
3. **Instinct-axis subagent dispatch**: Launch 3-5 subagents (one per instinct trigger). Each subagent produces 8 candidates using VS probability (same as base protocol), but the divergence direction comes from the instinct trigger rather than a 曼陀羅 direction. Concrete per-host dispatch call shape: `using-domain-teams/references/{claude-code-tools.md,codex-tools.md}`.
