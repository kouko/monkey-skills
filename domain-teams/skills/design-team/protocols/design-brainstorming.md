...
2. **Observe behavior**: 対象文脈におけるユーザーの無意識的行動を観察する。
   「考えずにやっていること」は何か？
3. **Extract Kansei vocabulary** (Verbalized Sampling):
   「この製品はどう感じるべきか？」を問い、
   10〜30 個の感性語（感覚形容詞）を確率分布とともに生成する。
   高確率の定型語（洗練された、シンプルな）だけでなく、
   低確率だが興味深い語（儚い、ざらつく、たゆたう）も含めること。
   関連語を提案して語彙を拡張する
...
9. **Generate meaning hypotheses** (Verbalized Sampling):
   5 個の意味転換仮説を確率分布とともに生成する。
   高確率の安全な仮説だけでなく、低確率だが革新的な仮説も 1-2 個含める。
   （例：高確率「ゲーム機＝家族で身体を動かす遊び」
         低確率「ゲーム機＝高齢者のリハビリ装置」）
   各仮説は製品の**新しい社会的役割**を記述すること。
   ユーザーに提示する際は 2-3 個に絞る
