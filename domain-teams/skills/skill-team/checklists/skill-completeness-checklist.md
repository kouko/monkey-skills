...
- [ ] **CHK-SKL-012 (Directory structure)** [FATAL]: The skill directory contains these required subdirectories: `standards/`, `protocols/`, `checklists/`, `rubrics/`. An optional `research/` subdirectory is permitted for grounding audit trail notes (see `standards/file-conventions.md` §Directory Semantics (research/ row) and `standards/skill-md-structure.md` §Research Subdirectory Convention). **Any other subdirectory is FATAL.** No nested subdirectories. Top-level files are limited to `SKILL.md` (required), plus optionally `README.md` and `README.{lang}.md` BCP 47 translations (see `standards/file-conventions.md` §Top-Level Files for the README.md / SKILL.md coexistence rule). Any other top-level file is FATAL.
