...
- **`quality-gate.md` evidence rule inverted to anchor-primary.** "File path + line number + specific problem" becomes "Anchor (verbatim string or stable heading) + specific problem; a line number is optional precision." Propagated into loom-code by `distribute.py`.

## [v5.10.3] — 2026-08-07

### Changed

- **`arch-gate.md` Deletion-First Scoring section move** (code-team
  `rubrics/arch-gate.md`): the four YAGNI / Speculative Generality flags
  (2× 🔴 Fatal, 2× 🟡 Warning) previously scattered across Approach
  Fitness / Module Cohesion / Change Tolerance are consolidated into a
  new "Deletion-First Scoring" subsection, so the `deletion-first`
  dimension on code-quality-reviewer / code-reviewer is scored exactly
  once instead of duplicated across gates. No new capability; flag text
  and grounding citations unchanged.
...
- **`CHK-SPEC-009: Reuse-adequacy Block in Plan Tasks`** (code-team
  ...
  MAY omit the block; it is opt-in by reuse presence. Mirrors the plan-side
  contract shipped in loom-code 0.43.0, whose `plan-document-reviewer` Check 17
  grades the same block and owns the adequacy verdict.
