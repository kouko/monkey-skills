"""RED test for Task 1 — strategy-dag plugin skeleton + marketplace + Codex mirror.
...
PLUGIN_ROOT = REPO_ROOT / "strategy-dag"
...
def test_manifest_marketplace_and_codex_mirror_are_consistent():
    ...
    assert plugin_json["name"] == "strategy-dag"
    assert plugin_json["version"] == "0.1.0"

    marketplace_path = REPO_ROOT / ".claude-plugin" / "marketplace.json"
    marketplace = json.loads(marketplace_path.read_text(encoding="utf-8"))
    entries = [p for p in marketplace["plugins"] if p.get("name") == "strategy-dag"]
    assert entries, "strategy-dag entry missing from marketplace.json"
    assert entries[0]["description"] == plugin_json["description"]

    result = subprocess.run(
        [sys.executable, "scripts/sync_codex_manifests.py", "--check", "strategy-dag"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
