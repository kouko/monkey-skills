...
| R12 | **Review debt cleared in-arc** (added 2026-08-31 with kouko's approval — "一次處理完吧"). (a) Adapter hardening in batch_review_cli.py: dispatch-receipt refusal keyed on batch_id (scan the receipt's directory for any unapplied receipt naming the same batch, not only the exact `--out` path); reopen-path crash recovery symmetric with finalize (receipt unapplied + owners already `pending` + non-owners still `implemented(<sha>)` matching the packet → flip idempotently); `_repository_identity` anchored on the member sha (`rev-list --max-parents=0 <sha>`), not HEAD; one shared TimeoutExpired→PacketRefused wrapper; `_member_statuses` computed once per build. (b) Hygiene sweep: bare `Status: ratified` (no name/date) test case for check_proposal_status; symlink-escape test through `_da_evidence_is_resolvable`'s artifact-path branch; plan_card 40-hex passthrough test asserts file content; test_governance_ratification trailing newline + shared Decision-Log extractor; decision-map SKILL.md "Read all three" → names the two files and the boundary section. Explicitly NOT in scope: the checker family's UnicodeDecodeError gap (pre-existing across five scripts — separate arc) and CAS simplification (rejected above). |
