...
- **Reverse (downstream consumer / SSOT)**: The output is consumed by `code-toolkit:writing-plans`, which (per `code-toolkit/skills/writing-plans/SKILL.md`) turns acceptance criteria into atomic RED/GREEN tasks. The brainstorming default brief path is `docs/loom/specs/` — spec-toolkit's output is the spec layer *upstream* of that. **No SSOT conflict**: spec-toolkit owns GENERATE artifacts, code-toolkit owns VERIFY; OpenSpec change-folder is the (future) neutral contract.
