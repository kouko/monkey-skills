...
- BI-2 — A plan header's `Goal:` value stays within the same 300-character ceiling and admits no nested body, because `plan_card.py` folds any indented content into the card's single `goal:` line. (Amended 2026-08-19: the original wording also said "one sentence". Sentence counting was abandoned under BI-1 after two review rounds proved regex cannot do it; keeping it here would have made the `Goal:` check re-derive the same two failures. Brevity guidance may still say one sentence — the mechanical check does not.)
