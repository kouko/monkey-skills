...
- BI-1 — A plan task's `Description` carries exactly one sentence on its first line, while `Acceptance.RED` and `Acceptance.GREEN` carry one assertion sentence plus one optional grounding clause; in all three, every further clause lives in a nested bullet or a markdown table beneath the first line. (Amended 2026-08-19 at plan time, after this brief was signed off: the original wording said "exactly one sentence" for all three fields, which a measurement taken after sign-off falsified — 33 of the 142 `RED`/`GREEN` first lines in the current corpus carry a second sentence, and nearly all of them are the `Fails today because ...` grounding clause `plan-format.md` itself teaches. The amendment is recorded here rather than patched in the plan, because the plan's `Brief item covered: BI-1` would otherwise read as satisfied while shipping a different rule.)
- BI-2 — A plan header's `Goal:` value is one sentence within a stated character ceiling and admits no nested body, because `plan_card.py` folds any indented content into the card's single `goal:` line.
...
We replace the judgment-shaped `one-assertion` rule with a positional rule the machine can check, and we extend it to the one slice of brief/spec prose that is structurally identical to the plan-field defect. The rule constrains WHERE a field's overflow goes, not how long the document is — content moves into bullets and tables, nothing is deleted. We will NOT ask any checker to classify a paragraph as narrative-or-not: that judgment stays with the author, who declares it inline, and with the reviewer, who can point at the declaration. We will NOT put a length cap on the whole document, on `Description`'s total size, or on the median paragraph — measured evidence says the corpus does not pad, so a size cap treats the wrong disease. The chosen threshold and the `Goal:` character ceiling are the falsifiable half of this design; the structural rule ("overflow must be a bullet or a table") is the load-bearing half and survives if the numbers are wrong.
