...
- **Reverse (output / SSOT)**: brainstorming's output is the brief at `docs/loom/specs/`; `writing-plans` consumes it. Tier 1 only enriches the brief's content — no distribution/sync script, no SSOT mechanism involved (pure in-skill prose).
