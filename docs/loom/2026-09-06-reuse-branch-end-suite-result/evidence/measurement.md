...
- Candidate: `9cb6f7a4fc544cde902574445cf6df04a1b0350e`, including the immutable-source, shell-boundary, executable-shadow, post-gate-hook, canonical-origin, remote-head-recheck, and effective-Git-configuration hardening.
...
- Fixed fixture: one accepted checkpoint is copied unchanged into every arm. Its HEAD was `b9ec4dcc9a666297b7ff3ca07caa9c02526091a9` in this run, on branch `work`.
- Canonical push input: every arm uses its sample repository's absolute path and generates the hook payload from these quote-all tokens: `'command' '/usr/bin/git' '-C' '<absolute-sample-repository>' 'push' '--no-follow-tags' '--recurse-submodules=no' '-u' '--no-verify' 'origin' 'b9ec4dcc9a666297b7ff3ca07caa9c02526091a9:refs/heads/work'`. The supported shell's standard `command` builtin is the trust root for bypassing aliases and functions; `/usr/bin/git` was the resolved trusted executable in this run. Every token is single-quoted, embedded single quotes use the standard `'"'"'` splice, and tokens are joined by one ASCII space.
...
|---:|:---:|:---|---:|---:|:---|:---|---:|---:|:---|
| 1 | B→C | 0.085021958, 0.085019917 | 2 | 2.269750084 | 0,0 / release | 0.085026542 | 1 | 1.253990916 | 0 / release |
| 2 | C→B | 0.085038292, 0.085023042 | 2 | 2.249802042 | 0,0 / release | 0.085028625 | 1 | 1.241923833 | 0 / release |
| 3 | B→C | 0.085031167, 0.085014375 | 2 | 2.256235917 | 0,0 / release | 0.085027917 | 1 | 1.209504333 | 0 / release |
| 4 | C→B | 0.085021750, 0.085022500 | 2 | 2.168229500 | 0,0 / release | 0.085014500 | 1 | 1.209004958 | 0 / release |
| 5 | B→C | 0.085014333, 0.085022875 | 2 | 2.192127167 | 0,0 / release | 0.085022375 | 1 | 1.199734834 | 0 / release |
| 6 | C→B | 0.085022250, 0.085017709 | 2 | 2.205527959 | 0,0 / release | 0.084786416 | 1 | 1.201516791 | 0 / release |
| 7 | B→C | 0.080437084, 0.085026125 | 2 | 2.191552125 | 0,0 / release | 0.085023375 | 1 | 1.213105875 | 0 / release |
| Median | — | — | 2 | 2.205527959 | release | — | 1 | 1.209504333 | release |

Observed totals are 14 baseline package-command invocations and 7 candidate invocations. Every candidate sample invoked the declared command exactly once and fewer times than its paired baseline. Both arms produced the same real gate verdict in every pair. Candidate median local-gate wait was `0.996023626` seconds lower, or 45.2% lower for this controlled fixture.
