...
- **被審查者駁回的重要發現** — 有一項：reviewer 要求 measurement fixture 在 squash merge 並刪除 feature branch 後仍可重跑；blind runner 依 `plan.md` 明確的 change-specific evidence 範圍駁回，因為驗收只要求發布前對固定 revisions 完成 baseline/candidate replay，不承諾 post-merge replay。另有一項函式長度 nit 因避免無第二使用者的抽象而駁回。
