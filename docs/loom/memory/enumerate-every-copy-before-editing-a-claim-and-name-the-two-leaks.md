---
name: enumerate-every-copy-before-editing-a-claim-and-name-the-two-leaks
description: Before editing a claim that may exist in more than one place, enumerate the population and record the count instead of describing it — and know that a single-line grep is NOT that population: this repo's prose is hard-wrapped, so a quote split across two lines matches nothing, and a proposition restated in synonyms is invisible to any string search; both leaks are named here because a rule that hides them ships as reliable and misses silently
type: practice
...
eleven hits, of which only three were the symbol. Two of the rest were a
deliberate same-named duplicate inside a stdlib-only push-guard hook, private
precisely so the hook stays dependency-free — the drafted instruction, executed
literally, would have broken that independence. Three more were the brief's own
prose, which implementation work does not refresh. The count is what converted a
plausible instruction into a correct one.

**The two leaks, both observed live, both in the same session.**
...
are out of scope — rather than writing "every reference"; (3) state that the
synonym leak stays open, because a rule that presents itself as complete is worse
than one that names its hole.
