...
[deploy-surface-ab-legs-run-post-merge](deploy-surface-ab-legs-run-post-merge.md) — A/B legs probing DEPLOYED surfaces (skill descriptions, hooks, preload cards) via headless `claude -p` read the installed plugin cache, and the marketplace source is github main — a feature branch's pre-merge B-leg silently re-probes the OLD cache (false no-regression); author such gates as explicit post-merge steps, pre-merge safety = review-side static trigger diffs
[description-sweeps-must-run-owning-plugin-suite](description-sweeps-must-run-owning-plugin-suite.md) — Frontmatter-description edits look test-free but plugins pin description content in scripts/ guard tests — a sweep dispatch omitting the owning plugin's package suite ships red guards to close-out (and the reverse: a whole-file guard can force a description clause to STAY); sweep packets carry the plugin suite as Resolved test command + grep scripts/ for SKILL.md-reading tests before cutting
[doc-string-tests-pass-while-weak-readers-misread](doc-string-tests-pass-while-weak-readers-misread.md) — Tests that pin a SKILL.md's documentation STRINGS verify the words exist, not that a weak executor reads the intended semantics — three live cases shipped green-tested prose that haiku cold-readers misread; behavioral cold-read rounds on the weak tier are the only layer that catches this class, and each fix was a few words
...
[parallel-wave-commit-discipline](parallel-wave-commit-discipline.md) — Parallel subagent waves share one git index — commit orchestrator-serially with explicit pathspecs (never git add -A, never plain git commit), verify with git show --stat HEAD; and NEVER recover a swept commit with reset --soft while siblings are still committing — verify HEAD is still yours first, else the reset orphans a sibling's commit
...
[sibling-attractor-makes-lexical-tuning-unstable](sibling-attractor-makes-lexical-tuning-unstable.md) — When a slimmed description regresses via SIBLING-skill attraction (cross-family first-fire), partial keyword restoration only moves WHICH record loses (measured 2 runs); the stable states are full revert or fixing the attractor side — budget one experiment, then revert per the A/B bar
