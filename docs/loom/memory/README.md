...
[workflow-agent-results-and-courier-args-need-guards](workflow-agent-results-and-courier-args-need-guards.md) — Workflow scripts — every agent() result can be null (skip/terminal error) and must degrade to a failed item, and any operator arg interpolated into a courier agent's Bash instruction text needs a per-segment character allow-list; leading-slash or non-empty checks are not enough
