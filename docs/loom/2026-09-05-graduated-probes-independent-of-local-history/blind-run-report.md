...
- **有審查者認為「紅了不得畢業」這句話只是寫在說明文字裡、沒有機制會在有人不理它時真的擋下推送，判定這是嚴重問題** — 已讀到這個意見，但維持原判：把這句話正式登記成本專案認可的「文字規則」之一（配了專屬測試釘住它、也做了冷讀檢查），讓這類文字規則的總數多了一條，登記的理由寫進了版本紀錄。之所以不改成真正擋下推送的機制，是因為你在這個改動一開始就已經決定不要為了這件事新增一條擋推送的機制，而本專案本來就承認「登記過的文字規則」是正當的一種——同一份文字旁邊已經有兩條規則也是這樣運作，不是特例。這對你的意思是：如果之後真的有人不理紅字、硬推送，現在沒有任何東西會自動擋下來，只有審查會讀到；如果這件事之後真的又出過一次事，下一步就是把它做成真正擋得住的機制，但那要你重新決定，不是這次改動的範圍。
- **審查過程中另外有一條「重要」等級的意見被撤回，不算數**：有一位審查者原本認為「盲跑報告用中文寫」是個問題（他讀到的規則版本要求內部文件一律用英文），但另一位審查者指出——依這個專案自己的語言規則，盲跑報告跟提交給你的說明文字本來就是用你看得懂的語言寫的，不受「內部文件用英文」那條規則管，所以這個意見被撤回、沒有改動任何東西。如果你覺得這個判斷是錯的（你其實希望連給你看的報告都用英文），這就是需要你出聲的地方。
- 除了以上兩條被撤回的意見，這個檢查點沒有其他「重要」以上等級卻被放行不處理的審查意見。
