...
def _graduated_sources() -> dict[str, str]:
    # This change's own graduated copies (test_probes_rehearsal_*.py) carry
    # the deleted names as string constants -- the very lists below -- so
    # they are excluded; the residue check targets the pre-existing suite.
    return {
        path.name: path.read_text(encoding="utf-8")
        for path in sorted(SCRIPTS_DIR.glob("test_probes_*.py"))
        if not path.name.startswith("test_probes_rehearsal_")
    }
