...
- Supplying only `--model` or only `--effort` → prints a JSON `input-error` diagnostic to stderr and exits 2 before starting Claude; the caller retries with both values or neither.
- Supplying an effort outside the grounded five-value set → prints a JSON `input-error` diagnostic to stderr and exits 2 without starting Claude, because the checked-in probe shows Claude Code would otherwise ignore it and use default effort → REQ-2.
- A supported model rejected by Claude with the grounded unrecognized-model marker before task execution → the Review caller feeds the rejection to the resolver and invokes the override-free replacement once in the same task attempt; that replacement consumes the transient retry slot, so no further invocation occurs for the digest regardless of its result.
