...
def run_review_edits(repo: Path, change_id: str = REVIEW_CHANGE_ID) -> subprocess.CompletedProcess:
    return run_checker("review-edits", change_id, cwd=repo)
...
def run_plan_edits(repo: Path) -> subprocess.CompletedProcess:
    return run_checker("plan-edits", PLAN_CHANGE_ID, cwd=repo)
