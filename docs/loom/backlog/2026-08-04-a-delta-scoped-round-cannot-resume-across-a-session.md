...
- But nobody invokes `review-pass` on a `NEEDS_REVISION` verdict; there is no
  ...
  ledger: branch `fix-docs-review-mechanisms` ran four docs-review rounds and
  has **two** ledger rows — the two times a marker was actually minted.

So the ledger records mint attempts, not review rounds, and it systematically
misses the failing round that is the only one whose sha a round 2 needs.
