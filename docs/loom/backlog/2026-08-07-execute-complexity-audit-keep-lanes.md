...
1. Mechanical dedup arc — B1 state-anchor wording into distribute.py
   ...
   fork into distribute.py; C2 brief-before-asking clause (verbatim ×4
   in design-side routers) to one source; D2 router-card five rules into
   verify-drift.py coverage. Behavior-zero, existing machinery.
...
4. Prose slim arc — A2 extract requesting-docs-review's convergence
   block (1,424 w; cap headroom 72 w today, the only item blocking
   future edits); A1 collapse finishing Step 8's five ONCE-checklists
   (1,123 w, semantics preserved); A3 downgrade writing-plans'
   hypothetical wrong-bind reversal protocol (315 w) to one line.
   Weak-model cold-reads mandatory; ~42 test files pin these skills.
