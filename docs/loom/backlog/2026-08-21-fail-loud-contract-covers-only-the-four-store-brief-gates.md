...
description: the unreadable-input fail-loud contract binds only the four store/brief gates; 27 other modules in loom-code/scripts/ are EXEMPT, and 17 of them are leaky by the contract's own metric today (measured, not estimated — the count is pinned by test_exempt_leak_count_matches_the_filed_ledger)
status: closed
...
start: the next time a non-gate script in loom-code/scripts/ dies on a raw traceback in front of a user, or the next arc that touches ANY of the 17 leaky modules the pinned metric names for any other reason
---
...
- Start: the next time a non-gate script in loom-code/scripts/ dies on a
  ...
  17 leaky modules the pinned metric names for any other reason
...
- What: `test_gate_scripts_fail_loud_on_unreadable_input.py` binds four
  ...
  `leaky_scopes()` over the 27 exempt modules returns **17 leaky** —
  ...
  live_host_review_gate, loom_init, plan_card, verify-drift.
