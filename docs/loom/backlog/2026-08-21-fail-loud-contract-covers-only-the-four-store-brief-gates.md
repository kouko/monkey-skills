...
description: the unreadable-input fail-loud contract binds only the four store/brief gates; 28 other modules in loom-code/scripts/ are EXEMPT, and 18 of them are leaky by the contract's own metric today (measured, not estimated — the count is pinned by test_exempt_leak_count_matches_the_filed_ledger)
status: closed
...
start: the next time a non-gate script in loom-code/scripts/ dies on a raw traceback in front of a user, or the next arc that touches ANY of the 18 leaky modules the pinned metric names for any other reason
---
...
- Start: the next time a non-gate script in loom-code/scripts/ dies on a
  ...
  18 leaky modules the pinned metric names for any other reason
...
- What: `test_gate_scripts_fail_loud_on_unreadable_input.py` binds four
  ...
  `leaky_scopes()` over the 28 exempt modules returns **18 leaky** —
  adjudication_lint / _render / _split, archive_change_folder,
  check-living-spec-index, check-skill-crossrefs, check_attack_catalogue,
  check_contract_citations, check_doc_citations, check_field_microstructure,
  check_open_questions, check_scenario_coverage, distribute,
  live_host_review_gate, loom_checker, loom_init, plan_card,
  verify-drift. (`loom_checker.py` joined the list in W0-05; it is EXEMPT
  only provisionally — it is the single deterministic gate and belongs in
  FAMILY once its store reads have per-rule unreadable-input cases.)
