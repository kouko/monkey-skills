---
name: 2026-08-18-loom-decision-trail-as-dag-view-via-strategy-dag-render
description: loom's engineering decisions are recorded in four disconnected carriers (brief Decision / plan Decision Log + Kickoff decision lines / commit Decision trailers / review verdicts) with no "what does this rest on" edge between them — give them one shared inputs field and reuse strategy-dag's `render` as the human-facing DAG view instead of building loom a fifth trail
status: OPEN
origin: 2026-08-18 strategy-dag Part 1 SDD session — user asked whether loom itself needs a DAG-form CoT view to explain current decisions/specs; assessment was "the need is real, the missing piece is the dependency edge, not another renderer"
start: after the strategy-dag Part 1 real-material checkpoint file (docs/loom/dogfood/2026-08-*-strategy-dag-real-material.md) records go — the DAG-as-decision-trail bet must survive its own experiment first
---

- Start: after the strategy-dag Part 1 real-material checkpoint file
  (docs/loom/dogfood/2026-08-*-strategy-dag-real-material.md) records go —
  the DAG-as-decision-trail bet must survive its own experiment first

- Origin: 2026-08-18 strategy-dag Part 1 SDD session — user asked whether
...
- **The gap**: to answer "why did we pick A, and what premise does it rest
  ...
  2026-08-18 strategy-dag session is a live case: the framing moved
...
- **Proposed minimal shape (to brainstorm, not decided)**: give the four
  ...
  strategy-dag's node frontmatter `inputs: [{ref, load_bearing}]`), and
  reuse `strategy-dag/skills/strategy-dag/scripts/dag.py render` (Part 1
  Task 13) as the view over them — strategy-dag becomes loom's viewing
  surface rather than loom growing its own. Do NOT port strategy-dag's
...
- **Evidence pointers**: strategy-dag umbrella brief
  `docs/loom/specs/2026-08-18-strategy-dag-plugin.md` (§Problem: "CoT
  ...
  `docs/loom/plans/2026-08-18-strategy-dag-plugin-part-1.md` Task 13
...
- **Not this entry**: strategy-dag Part 2 (views/proposal/milestones) — that
  is `docs/loom/specs/2026-08-18-strategy-dag-plugin-part-2.md`.
