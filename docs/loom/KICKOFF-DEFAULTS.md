...
- session-start-baseline: 923fb84a 5278 — measured with `bash loom-code/hooks/session-start </dev/null | LC_ALL=C wc -w` in an empty git repo, merge-base of the loom 1.0 change; `wc -w` disagrees with itself across locales on some unicode punctuation, so the count is pinned to LC_ALL=C (2026-09-03)
