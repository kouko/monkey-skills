...
Round 6: Codex NEEDS_REVISION on the drift intent's own wording (round attribution; two exclusive paths), sonnet PASS_WITH_NOTES (one citation nit, corrected in the record). Round 7 reads the one-commit fix. Four late-intent rounds.
Round 7: both PASS_WITH_NOTES, one important each (needs-design reason still named two paths; my citation fix missed one of five occurrences). The sonnet nit that a lone-nit verdict should be PASS is recorded as returned: verdicts are stored as the reviewer gave them, never edited. Round 8 reads a one-line fix and a one-token record fix. Five late-intent rounds; each round is now catching my own previous fix.

## Addendum at PR #782 first CI run (two red jobs after a green local suite)

1. `loom-workflow/skills/decision-map/scripts/test_skill_doc.py:200` pins the plugin version as the literal `4.0.0`; W1-03 bumped it to 4.0.1 and no local run caught it because that suite is outside the repo's package-tests line. Version-in-N-places, again — this time a test is one of the places.
2. `test_close_commit_crlf_line_ending_on_status_line_passes` (graduated copy and the evidence probe) expects PASS. On this machine git's global `core.autocrlf=input` rewrites the `\r\n` to `\n` at `git add`, so the committed blob never carried the CR and regenerate-and-compare matched. On CI (autocrlf unset) the CR reaches the blob, the regenerated canonical differs by one byte, and the rule blocks — which is the designed behaviour. The expectation is a leftover of the first (grammar-strip) design; the after-task:W0-04-08 finding that said exactly this was dismissed as a nit. CI overruled the dismissal. Lesson for the adversary contract: any probe about bytes pins the repo's normalisation settings (`core.autocrlf=false`) inside the temp repo it builds.
Cost: one implementer dispatch (W1-06), one narrow round (code lens), one review-only commit, one more push.
