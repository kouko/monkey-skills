...
Behavioral burden retained: one new Task status, a six-field Batch declaration, immutable aggregate identity, finding ownership plus blocking ground, atomic multi-line status update, and one stable advisory lock shared by existing plan writers. The lock only serializes the existing write protocol; it is not a Batch object, lock manager, or second lifecycle. Each retained item exists to preserve Task traceability while removing repeated full-review setup. Dropped as speculative or redundant: Batch queue/status/claim state, score-based eligibility, arbitrary Batch-size caps, new dependency-edge kinds, a separate pass-receipt store, stale-claim redesign, blocked-reason redesign, retention service, lock manager, timeout service, SHA-rebinding transaction, telemetry platform, and legacy plan execution compatibility.
...
- **Aggregate-verification amendment round 1**: policy and NFR lenses found that inert plan prose needs fail-closed trusted-command resolution plus Packet identity over the resolved command, scope, and result. Both were re-seeded under REQ-105. [critic-found]
- **Aggregate-verification amendment round 2**: policy returned dry; NFR found that persisting secret-bearing resolution context would leak data. The first candidate non-secret identity was rejected because it opened collision/equivalence complexity. [critic-found]
- **Aggregate-verification amendment round 3**: the rule was simplified to reject every secret-bearing verification context without Packet creation, redaction, hashing, or equivalence guesses. NFR returned dry; unchanged policy input makes the next policy observation dry by definition, satisfying the two-dry stop without another dispatch. [critic-found]
