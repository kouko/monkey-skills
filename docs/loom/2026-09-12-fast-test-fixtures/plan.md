...
5. The version gate reads any change under a `skills/` tree as skill content, so the fixtures oblige a bump. Tests bind that version to the root README cell and the changelog's top entry; missing either turns the suite red.
