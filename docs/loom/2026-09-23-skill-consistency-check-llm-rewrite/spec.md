...
- agent-decided: token estimate = CJK characters counted as one token each plus remaining characters divided by four; no tokenizer dependency, because installs are forbidden and the thresholds are conservative.
- agent-decided: "core" = SKILL.md, every file under agents/, and every package file whose relative path appears in SKILL.md text.
- agent-decided: when the core alone exceeds 25,000 tokens, groups still carry the whole core and the report flags the run as over the validated size.
