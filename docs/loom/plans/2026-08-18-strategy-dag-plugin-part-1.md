# Plan: strategy-dag plugin — Part 1（格式・機械閘・假設傳播・核心對話協定・骨架 → 真實素材檢查點）

**Source brief**: docs/loom/specs/2026-08-18-strategy-dag-plugin-part-1.md
...
**Umbrella brief**: docs/loom/specs/2026-08-18-strategy-dag-plugin.md（總覽；Part 2 = `…-part-2.md`，只在 T12 檢查點檔存在後開工）
...
- **Description**: Create the `strategy-dag/` plugin skeleton: `.claude-plugin/plugin.json` (name `strategy-dag`, version `0.1.0`, description, keywords), the Codex mirror `.codex-plugin/plugin.json` produced by `python3 scripts/sync_codex_manifests.py strategy-dag`, `README.md` / `README.ja.md` / `README.zh-TW.md` (short: what it is, one usage line, "Part 1 — pre-release"), `CHANGELOG.md` with a `0.1.0` entry, a stub `skills/strategy-dag/SKILL.md` (frontmatter `name` + `description` + a one-paragraph body marked "Part 1 draft — protocol lands in T11"), and the marketplace entry appended to `.claude-plugin/marketplace.json` with a description byte-identical to plugin.json's. Write the test first.
- **Module**: strategy-dag (plugin root)
- **Files touched**: NEW: strategy-dag/.claude-plugin/plugin.json, NEW: strategy-dag/.codex-plugin/plugin.json, NEW: strategy-dag/README.md, NEW: strategy-dag/README.ja.md, NEW: strategy-dag/README.zh-TW.md, NEW: strategy-dag/CHANGELOG.md, NEW: strategy-dag/skills/strategy-dag/SKILL.md, NEW: strategy-dag/skills/strategy-dag/scripts/test_plugin_manifest.py, .claude-plugin/marketplace.json
- **Context paths**:
...
- **Acceptance**:
  - **RED**: `strategy-dag/skills/strategy-dag/scripts/test_plugin_manifest.py::test_manifest_marketplace_and_codex_mirror_are_consistent` — asserts plugin.json exists with `name == "strategy-dag"` and `version == "0.1.0"`, marketplace.json has an entry `name == "strategy-dag"` whose `description` equals plugin.json's, `python3 scripts/sync_codex_manifests.py --check strategy-dag` exits 0, and the three READMEs + CHANGELOG + SKILL.md exist — fails because none of the files exist.
  - **GREEN**: the test passes; `python3 scripts/check-marketplace-description-sync.py` exits 0; `bash .claude/hooks/validate-skill-folder-structure.sh strategy-dag/skills/strategy-dag/SKILL.md` exits 0.
- **Dependencies**: none
- **Independent**: false
...
- **Module**: strategy-dag/skills/strategy-dag/references
- **Files touched**: NEW: strategy-dag/skills/strategy-dag/references/research-rules.md, NEW: strategy-dag/skills/strategy-dag/references/blind-spot-checklist.md
- **Context paths**:
  - /Users/kouko/.supacode/repos/monkey-skills/strage-dag-skill/docs/loom/specs/2026-08-18-strategy-dag-plugin-part-1.md
  - /Users/kouko/kouko-obsidian-vault/research/2026-08-18 決策推演 plugin v0 設計定案.md
- **Acceptance**:
  - **RED**: diagnostic `grep -c "findable in the docs\|找得到出處" strategy-dag/skills/strategy-dag/references/research-rules.md` returns 0 / file missing, and `grep -c "" strategy-dag/skills/strategy-dag/references/blind-spot-checklist.md` fails (file missing).
  - **GREEN**: both files exist; research-rules.md contains the hard rule sentence and the four search triggers; blind-spot-checklist.md lists the six blind spots; `bash .claude/hooks/validate-skill-folder-structure.sh strategy-dag/skills/strategy-dag/SKILL.md` still exits 0.
- **Dependencies**: none
- **Independent**: false
- **Review-weight**: prose
...
- **Module**: strategy-dag/skills/strategy-dag/scripts/dag.py
- **Files touched**: strategy-dag/skills/strategy-dag/scripts/dag.py, strategy-dag/skills/strategy-dag/scripts/test_dag.py
- **Context paths**:
  - /Users/kouko/.supacode/repos/monkey-skills/strage-dag-skill/docs/loom/specs/2026-08-18-strategy-dag-plugin-part-1.md
...
## Task 7 — CI workflow `strategy-dag-ci.yml`

- **Description**: Add `.github/workflows/strategy-dag-ci.yml` modelled on `tsundoku-ci.yml`: trigger on `push`/`pull_request` with `paths: ['strategy-dag/**', '.github/workflows/strategy-dag-ci.yml']`; steps: checkout, setup-python 3.11, `pip install pytest pyyaml`, `python3 -m pytest strategy-dag/skills/strategy-dag/scripts/ -q`, `bash .claude/hooks/validate-skill-folder-structure.sh strategy-dag/skills/strategy-dag/SKILL.md`, `python3 scripts/sync_codex_manifests.py --check strategy-dag`. Write the test first. Runnable-capability line: the pytest verb for this plugin is declared in this workflow (the plugin has no other command surface) and verified to run locally.
- **Module**: .github/workflows/strategy-dag-ci.yml
- **Files touched**: NEW: .github/workflows/strategy-dag-ci.yml, NEW: strategy-dag/skills/strategy-dag/scripts/test_ci_workflow.py
- **Context paths**:
...
- **Acceptance**:
  - **RED**: `strategy-dag/skills/strategy-dag/scripts/test_ci_workflow.py::test_workflow_runs_pytest_structure_hook_and_codex_check` — parses the workflow YAML and asserts the `paths` filter contains `strategy-dag/**` and the `run:` steps contain the pytest invocation over `strategy-dag/skills/strategy-dag/scripts/`, the structure-hook invocation, and the codex `--check` — fails because the workflow file does not exist.
  - **GREEN**: the test passes and `python3 -m pytest strategy-dag/skills/strategy-dag/scripts/ -q` passes locally.
...
- Kickoff decision: 測試檔位置 → 與腳本同層 `skills/strategy-dag/scripts/test_*.py`（copywriting-toolkit 先例；skill 資料夾不可再嵌套）
