...
- **driver 重建紀律**：asset 是編譯產物，不可手改；改完 driver 源碼必須 `python3 loom-design/scripts/pipeline/build_driver.py` 重建，drift test 驗證 byte-identical。build_driver.py 與 drift test 的 path 都相對 script 位置解析，搬遷後已自動指向 loom-design/，無需改路徑邏輯。
