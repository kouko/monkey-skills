...
### DL-3 — Batch checkpoint on this plan: three refusals observed on the first live run (2026-08-31, SDD batch checkpoint)
- Class: implementation-discovered operating facts, below kickoff threshold (no product consequence; recorded so the next orchestrator does not repeat them).
- Fact 1: the sealed packet's `source_digest` covers the whole plan text, so flipping ANY ledger line between `packet` and `apply-result` — here T4/T5 `done` and T7 `claimed`, none of them batch members — changes the rebuilt identity and Task 1's new receipt binding refuses ("packet_identity does not match the rebuilt packet"). Correct fail-closed behaviour; operationally the plan must stay untouched from `packet` to `apply-result`. Recovery used: re-seal, re-record, rebind the identical reviewer verdicts (member shas and sealed bytes unchanged) to the new identity.
- Fact 2: `record-dispatch`'s sibling scan (`_sibling_unapplied_receipt`) treats ANY parseable receipt-shaped JSON in the `--out` directory as a sibling — a `tee`'d copy of its own stdout counted as an orphan receipt and refused the re-send. This is the F7 backlog entry's shape, observed live.
- Fact 3: the quality arm returned `PASS_WITH_NOTES` with one `blocking: true` finding; `_classify_findings` treats a blocking finding under a non-`NEEDS_REVISION` verdict as `unassignable_finding` → `individual_fallback`. The contradiction was sent back to the reviewer to resolve rather than edited by the orchestrator.
- Decision: none of the three is fixed in this arc; Fact 1 and Fact 2 are filed as backlog (`2026-08-31-packet-identity-binds-whole-plan-text`, and the existing F7 entry), Fact 3 is a contract-wording item for the result-file schema (a blocking finding requires NEEDS_REVISION) added to conditional-operations.md only if the reviewer confirms the contradiction was a schema-knowledge gap.
