...
- **Round-2 correction to brief §Build 2's class-C approach (review-driven, supersedes the "acceptance-context phrases" wording)**: code-quality review proved acceptance-phrases are unsound under substring semantics — for ANY token S, the natural rejection (「不支援S」 / "does not offer S") contains S verbatim, so no phrase choice can express "accepted-only". The three bait-leak negatives (seed3 mock server / API gateway, seed5 企業版) are DEMOTED to grader-side `# note:` prose instead, mirroring the corpus's existing "(not machine-checked)" precedent and #533's semantic-checks-stay-with-graders scope decision. A characterization test encodes the limitation. Amendment is additive to the plan schema; task structure/DAG unchanged.
- **Round-3 policy correction (CQR round-2 🔴)**: the general negation-superstring claim applies to EVERY machine negative, so consistency cannot mean "demote the observed three and keep the rest silently". The operative rule, now stated in the corpus README (grader contract SSOT): machine negatives are a lossy approximation of "bait accepted"; a token is demoted on the FIRST observed-in-run or mechanically-reproduced rejection-mention false positive; untriggered tokens are retained (cold-operator's 8 correction-regression pins have real value, 0 false positives), and eval semantics bound the residual risk (misses are always human-reviewed, never CI). The two round-2 survivors (seed3 `上傳 schema 到雲端`, seed5 `簡訊通知作為 v1 原則`) were reproduced by the reviewer → demoted per this policy.
