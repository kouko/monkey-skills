...
- **Version coordination (T1 round-2 review-driven)**: plan-format.md's Decision Log heading is version-tagged `v0.29.0+` per the file's convention — Task 6's loom-code plugin.json bump MUST be exactly 0.29.0 (minor). product-principles bump stays an ordinary minor (0.5.1 → 0.6.0).
