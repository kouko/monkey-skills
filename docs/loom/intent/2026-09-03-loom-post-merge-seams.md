...
## Acceptance
1. 在一個乾淨的 clone 裡，照 ship 站寫的順序（PR 號碼已知 → commit intent `closed <date> — PR #<N>` → 關 intent 的 commit 也照常由兩個 fresh reviewer 機器審過、我不看 diff → 只動審查記錄的收尾 commit）做完後 `git push` 這個分支：push 閘用原本的規則放行（不需要任何人用 `!` 繞過、不需要新規則），而對那個 intent 跑 write-plan 的 intake 會被擋並說明「這個 change 已關閉」。上一個 change（2026-09-02-simple-loom-flow，已合併但仍 confirmed）由本 change 的 diff 順手改成 closed。
