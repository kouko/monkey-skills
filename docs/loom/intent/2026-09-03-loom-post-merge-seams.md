...
## Acceptance
1. 在一個乾淨的 clone 裡，照 ship 站寫的順序（PR 號碼已知 → commit intent `closed <date> — PR #<N>` → 只動審查記錄的收尾 commit）做完後 `git push` 這個分支：push 閘用原本的規則放行（不需要任何人用 `!` 繞過、不需要新規則），而對那個 intent 跑 write-plan 的 intake 會被擋並說明「這個 change 已關閉」。上一個 change（2026-09-02-simple-loom-flow，已合併但仍 confirmed）由本 change 的 diff 順手改成 closed。
...
3. 在一個乾淨的 clone 裡（Claude Code 這一側，也就是有 plugin 正本可比的情況）只重跑 `codex_scaffold.py --repo .`（刷新 `.codex/hooks/` 副本）並 commit，不帶 `Task:` trailer：`loom_checker.py push` 不再因 `push.dispatch-covers-tasks` 擋這個 commit；而把副本裡任一檔改一個字（或多放一個檔）再 commit，同一條規則照樣擋。（反例由 round 2 審查者要求補上，2026-09-03）
