...
- 火痕檔繼續放 `.codex/hooks/`、繼續 gitignore；探針只讀它，不讀使用者家目錄下 Codex 的 config.toml（那是使用者的私人設定，格式也沒承諾穩定）。
