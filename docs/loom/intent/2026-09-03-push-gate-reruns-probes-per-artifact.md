...
## Problem
`push.probes-adversarial` 對每一筆對抗探針紀錄，都執行該筆的 artifact 檔案本身（不是紀錄裡的命令），逐筆串行。紀錄的粒度是「每個案例一筆」（同一個檔案 `-k` 不同案例），重跑的粒度卻是「整檔」，所以首個真實 change 的 22 筆單案探針讓 checker 把同兩個檔案各整檔重跑十幾次，每次約十秒。門檻「至少 3 支可用探針」也因此曖昧：3 個檔案還是 3 筆紀錄？一輪 ship 因此多一輪窄審（記錄在 evidence）。
...
2. 門檻的計數單位在 `--list-rules` 的說明裡一句講清；既有的 loom-post-merge-seams 紀錄（2 個檔案、24 筆）在新規則下的結果與現在一致或被明確說明。
