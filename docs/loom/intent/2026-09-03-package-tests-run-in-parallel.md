...
status: closed 2026-09-03 — PR #783
...
## Proposed outcome
`package-tests` 那一行改成用 pytest 的多程序外掛（`pytest-xdist`）分到多核心跑，平行度見 Constraints，KICKOFF-DEFAULTS、CI 與 checker 逐字比對的命令三處同步改；外掛列為開發依賴。目標本機 60 秒以內。
