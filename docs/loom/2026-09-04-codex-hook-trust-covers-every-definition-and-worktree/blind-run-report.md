...
大部分沒有——這次改動只碰了探針腳本（`codex_scaffold.py`）、一支新的共用記錄器
（`loom_record_fire.py`）、本 repo 自己那兩支 `.codex/hooks/` 副本改成薄殼、三處站文字、還有版本號跟 CHANGELOG；我這次盲跑本身的測試都在全新的暫存資料夾裡跑，沒有讀寫任何你原本就有的 change 記錄或設定；`.codex/hooks.json` 的內容逐位元沒變，你原本按過的信任照樣有效。**但有一件事要老實說**：build 期間（不是我這次盲跑），有兩個對抗者寫的探針測試（`test_abuse_hook_trust.py` 第 389-390 行、第 413-414 行）在跑測試時，cwd 用的是這個 repo 本身，直接觸發了這個 repo 真正的 `.codex/hooks/*.sh` 薄殼腳本；因為記錄器 `loom_record_fire.py` 是用自己的檔案位置（`__file__`）去找火痕檔，不是用測試給的假路徑，所以那兩次測試真的把紀錄寫進了**這個 repo 自己的** `.codex/hooks/.loom-hook-fired`。這個檔案是 gitignore 忽略的本機檔案，不是 git 追蹤的內容——W1-02、W1-04 的實作者跟這次的對抗者，都各自在提交前發現並刪掉了這個檔案，所以沒有任何一次提交把它帶進版本紀錄，git 追蹤的東西完全沒被動過。這個漏洞已在修正輪修好（commit `c46c950d`）：那兩個探針測試改成只觸發暫存資料夾裡的複本，並加了一道守衛——只要這個 repo 自己的火痕檔在測試中出現，整個測試檔直接判失敗。修好之後兩位讀者都重驗過，本 repo 沒有火痕檔。
...
- **第二位讀者抓到一個順序上的漏洞，也已經進同一輪修正**：薄殼腳本目前是先記下「這條 hook 被觸發了（fired）」，才去執行真正要做的檢查；而且它找「自己要轉呼叫的正本在哪裡」是用當下的工作目錄（cwd）去算，不是用薄殼自己所在的位置去算。這代表如果 Codex 哪天從一個子目錄而不是 repo 根目錄去執行這條 hook，薄殼會先把「fired」寫進去，然後才發現找不到正本、整個當掉——結果會是火痕檔已經說這條 hook 驗證過了，但其實這次呼叫從頭到尾沒有真的跑完檢查。修正輪已把這個順序反過來（commit `c62ac169`）：薄殼先用自己的檔案位置去找正本（不管 cwd 是哪裡），確定正本真的在，才把這行記錄寫進去；找不到正本就印出缺的路徑、以錯誤結束、不留記錄。有一個從子目錄觸發的回歸測試釘著這件事。
