...
confirmed-behavior: 2026-09-10 @773b079
...
REQ-1 — Continue in the active task
  WHEN required CI fails while its publication task remains active, Loom shall identify the failed required checks, retrieve each available failure log, and continue the same change instead of treating the publication command's failure as the end of the task; unavailable required diagnostics shall follow REQ-4 → Acceptance #1

REQ-2 — Repair functional failures under existing verification
  WHEN the failure is functional or test-related, Loom shall keep the repair in the original change, reuse an existing test that exposes the root cause or add the smallest permanent regression case when coverage is missing, apply Build's test-first discipline, and resume the existing bounded Closing Review episode at its next functional-content digest before Ship whenever that digest changes, without resetting the episode or creating another retry budget → Acceptance #2

REQ-3 — Reuse evidence for publication-only repair
  WHEN a repair changes only the pull request title, body, or other publication data not committed to the repository and the attestation still matches the functional digest, Loom shall fix and republish without repeating functional verification; every committed-file change, including a version-file change, shall return through Closing Review → Acceptance #3

REQ-4 — Stop only at a real decision or external blocker
  WHEN safe continuation would require changing requirements or guarantees, permission or required diagnostics are unavailable, or available diagnostics and known external state identify a persistent external failure, Loom shall stop and report the concrete blocker; an unattributed or safely undiagnosable failure shall not be rerun automatically → Acceptance #4
...
- user-decided — Keep the existing digest boundary: every committed-file change, including version files, reruns Closing Review rather than adding repository-specific publication-only rules.
...
- A required check fails while the task remains active → the task identifies the failed checks, retrieves each available failure log, and continues repairing the same change without waiting for another instruction.
- The repair changes the functional-content digest, including code, test, or committed version files → the task uses a test that reproduces the problem, applies the smallest fix, resumes the same bounded closing review episode, and then checks required CI again.
- The repair changes only the pull request title, description, or other information not committed to the repository → the task corrects it, retains the matching functional verification, and checks required CI again without repeating the functional suite.
- Required diagnostics or permission are unavailable, available evidence identifies a persistent external failure, the failure cannot be attributed safely, or the requested behavior would need to change → the task does not rerun it automatically, reports the concrete blocker, and stops.
