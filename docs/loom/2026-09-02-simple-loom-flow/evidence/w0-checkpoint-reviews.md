...
處置：af0870e4 全部修（新規則 `push.review-schema`；probe 加 `sha`；spec-pass 要求 adversarial probe；trunk 候選四個＋upstream，全不解 exit 2；timeout 30）。

## wave-end（W0 全部，含 5a758ff8 / 50ffb29e / 9531cef9 / af0870e4）
...
處置：e31c4703（文件面：dispatch 併入 review.json、§3 計 JSON 頂層鍵、§11 逐欄位、模板與 session-start 措辭、plan 註明 backlog-README 留到 W1-06）；checker／scaffold 面另派 implementer（見 review.json W0 輪）。
...
## wave-end 複審（3c5d14ca / ae1be4bd / 4c4aa20f / e31c4703 / b28d7586）
...
處置：docs notes → b28d7586、a6f583a8；N1–N3 → sonnet implementer（見 review.json W0 輪）。
