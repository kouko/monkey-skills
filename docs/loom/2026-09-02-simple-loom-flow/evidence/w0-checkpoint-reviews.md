...
## after-task（W0-03，checker b476e95b..a9eeeea3）
...
處置：b2e9e1c0 全部修（新規則 `push.review-schema`；probe 加 `sha`；spec-pass 要求 adversarial probe；trunk 候選四個＋upstream，全不解 exit 2；timeout 30）。

## wave-end（W0 全部，含 5a758ff8 / 50ffb29e / fd30caef / b2e9e1c0）
...
處置：8b325a7f（文件面：dispatch 併入 review.json、§3 計 JSON 頂層鍵、§11 逐欄位、模板與 session-start 措辭、plan 註明 backlog-README 留到 W1-06）；checker／scaffold 面另派 implementer（見 review.json W0 輪）。
...
## wave-end 複審（bb85a744 / 0459e0ac / 2f91feb7 / 8b325a7f / c51d4518）
...
處置：docs notes → c51d4518、6e8e2c92；N1–N3 → sonnet implementer（見 review.json W0 輪）。
