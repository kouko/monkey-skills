...
4. 單向門（架構／框架／元件／決定輸出品質的選型）：「A 用你的三段錄音測，準確率 91%、每小時 0.9 美元、錄音送雲端；B 本機跑，78%、免費、不外傳。我建議 A，除非你在意隱私。」→ 選一個／問更多。框架、語言、資料庫、認證、託管、付費服務、資料格式、模型／演算法都算；能先量的先量；PRINCIPLES.md 或 intent 已釘住的不問；一個 change 合成一次問。
