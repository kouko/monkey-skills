...
named_anchors:   # each MUST appear in ## Anchors with a non-empty version/edition cell
  - Working Backwards
  - Material Design
  - WCAG 2.2        # named standard counts as canon; AA level stated
  - React           # frontend stack — named in prose, must still anchor
  - TypeScript
  - FastAPI         # backend stack
  - PostgreSQL
  - 12-Factor App   # deployment convention named in passing prose

deferred_items:   # each MUST appear in ## Open Questions with a — re-trigger:
  - 預約記錄保留期（法規待查）        # Q8
  - SLA/uptime 目標（待首批店家營業時間分布）
...
negative:
  - 企業版                            # explicitly scoped out
  - 簡訊通知作為 v1 原則              # deferred channel
