...
named_anchors: Shneiderman's Eight Golden Rules; Nielsen's 10 Usability Heuristics; Ports & Adapters; 12-Factor App; Working Backwards / PR-FAQ; Rust
...
negative: 上傳 schema 到雲端; mock server support; API gateway support
# note: 上傳 schema 到雲端 must not read as accepted behavior (privacy stance
# is 純本地不上傳). mock server/API gateway are Scope-out items — presence
# means they leaked back in as an in-scope Product Principle. Separately (not
# machine-checked — no single literal string captures an absence-of-marker
# check): "好用的 CLI" or any unchecked adjective with no `— check:` is a
# NEGATIVE finding for LLM/human graders.
