...
code-toolkit 現有的 brainstorming / writing-plans 產出**主要活在對話 context 裡**，跨 session、跨工具、跨機器都會失。`docs/code-toolkit/{specs,plans}/` 雖然存在但是 free-form Markdown — 沒有 machine-readable schema、沒有 brownfield delta 追蹤、沒有 task-level progress query API。
...
本 brief 描述「如何採用 OpenSpec」但本身寫在 pre-OpenSpec `docs/code-toolkit/specs/` — Q3 已選擇 freeze archive。下個 OpenSpec change（v0.1 落地的第一個）才走 `openspec/changes/<id>/`。
