---
title: Repo Wiki Query SKILL.md 設計
type: research
date: 2026-05-02
tags:
  - SKILL-md
...
# Repo Wiki Query SKILL.md 設計

> [!abstract] 這個檔案是什麼
> 這是 `repo-wiki/skills/query/SKILL.md` 的設計稿。
> 這是人類互動的唯一入口——**所有關於 codebase 的問題都透過這個 skill 回答**，不需要直接讀程式碼或 .repo-wiki/ 頁面。

---

## 設計脈絡

決策 10：v1 stale check 只看 `last_updated > 60 days`，不跑 git 比對。完整 git-aware staleness 留給 v2 `/repo-wiki:lint`。

決策 7：所有 link 用標準 markdown，不用 `[[wikilinks]]`。

---

## SKILL.md 完整內容

把以下內容存為 `repo-wiki/skills/query/SKILL.md`：
...
description: |
  ...
  "how does module Y work", "what depends on Z", "where should I add this",
  "/repo-wiki:query". Do NOT trigger for: writing new code, debugging
  runtime errors, questions answerable by reading a single known file
  directly, first-time setup (use /repo-wiki:init), updating knowledge
  (use /repo-wiki:ingest).
---
...
If unsure which pages are relevant, load index.md, scan titles + one-line
descriptions, then pick.

## Step 3: Draft Answer from .repo-wiki/

Draft the answer using only the .repo-wiki/ pages loaded in Step 2. Do
not yet present to user — Step 3.5 will decide whether to verify against
src/.

Identify in the draft, for the verification stage:
- Which claims describe **current behavior** (e.g., "AuthModule uses JWT")
- Which claims describe **past decisions** (e.g., "we switched from
  jsonwebtoken to jose in 2026-04-20")
- Which claims are **pointers** (e.g., "Auth lives in src/auth/")
...
## Step 3.5: Verification Trigger Evaluation (Decision 13)

.repo-wiki/ is a *cache*, not the authority. Evaluate these triggers; any
positive trigger initiates src/ verification of current-behavior claims:
...
| **T2** | Question contains "currently", "now", "today", "still", "現在", "目前", "還有沒有" or equivalent | Spot-checking current-behavior claims against src/ |
...
| **T4** | Loaded source page has TODO or "subject to change" / "待確認" markers | Reading the corresponding src/ area and updating the answer |
...
For each verifiable claim, locate the relevant src/ file via the loaded
entity's `paths:` frontmatter. If `paths:` is missing on an entity:
- Log warning: "Entity <name> missing `paths:` frontmatter — falling back
  to grep-based discovery (slower)"
...
| Lang | Keywords |
|---|---|
| EN | `currently`, `now`, `today`, `right now`, `at the moment`, `still`, `as of`, `these days`, `still…?`, `is…still`, `does…still`, `is the current` |
...
(b) **Active code work in session** (LLM checks its own tool history):
- Write / Edit / MultiEdit tool was called within the last 10 turns
  on a path that overlaps with the question's subject
...
**Rule**: For each loaded page (entity OR source OR concept), search the
body text (case-insensitive) for any of:
...
**Note**: Markers in code blocks within the page body still count
(documentation often quotes code with TODOs).
...
- Decision in source page that **directly negates** a claim in entity page
  (e.g., source says "removed JWT validation from middleware", entity says
  "validates at middleware layer")
...
**Negative example**:
- entity/AuthModule.md mentions src/auth/jwt.ts as entry point
- entity/PaymentService.md doesn't mention auth at all
...
#### Detection order and short-circuit

Evaluate triggers in this order; the first positive determines branch:

1. T7 (user explicit) — if positive, verify all claims, skip other triggers
2. T6 (pure decision) — if positive, exit verification entirely
3. T1, T2, T3, T4, T5 — evaluate all; any positive → segmented output

T7 takes precedence over T6 (user explicit override).
T6 takes precedence over T1-T5 (decision questions are immune to staleness
because decisions are historical facts).
...
This makes verification an active feedback loop into the ingest cycle —
...
If ANY trigger fired, present in **segmented format** (Decision 13):
...
The Verified section always appears when triggers fire — even if empty,
it tells the user "I checked but had nothing to verify".

## Step 5: Offer to Save as Synthesis

Ask:
> "Would you like me to save this answer to .repo-wiki/syntheses/
> so it's available for future queries?"
...
**Synthesis disclaimer (always include in saved synthesis body)**:
...
(Append the log entry whether or not the synthesis was saved — the query
itself is the operation worth recording.)
...
> Option A: "I can read src/<module>/ directly to answer this,
>            but the answer won't be saved to the knowledge base."
> Option B: "Run /repo-wiki:ingest <scope> to build the knowledge base
>            for this area first, then re-ask."
> Option C: "If you have context I can capture, run
>            /repo-wiki:ingest \"<the context>\" to seed it."
...
to use to fill it (Decision 8's polymorphic ingest).
...
- Present current-behavior claims as authoritative without running
  verification triggers (Decision 13)
- Skip verification when T1-T5 or T7 fires (Eager policy)
- Auto-trigger /repo-wiki:ingest from query (user must invoke explicitly)
...
- Be explicit when the knowledge base has a gap — gaps are feedback for
  the next /repo-wiki:ingest run
...
> [!important] 為什麼 gap 要明確說出來
> 當 query 說「knowledge base 沒有這個模組的頁面」時，這是一個信號——下次跑 `/repo-wiki:ingest` 時要補上。
>
> 讓 LLM 直接讀 src/ 回答問題沒問題，但答案不會被記住。明確的 gap 會促使工程師觸發 ingest，讓知識庫成長。

> [!tip] Synthesis page 的價值
> 當同樣的問題被問第二次，query 在 `.repo-wiki/syntheses/` 找到上次的答案直接回傳——不需要重新推理也不需要讀多個頁面。
> 這是知識庫越用越聰明的機制：實際被問的問題會自動 cache。

> [!note] Stale check v1 升級為 verification trigger
> 早期設計把 stale check 當 soft warning（「page 過期，自己決定要不要 ingest」）。
> 決策 13 升級為 trigger T1：page 過期就**自動讀 src/ 比對**，把 stale 從 warning 變成 active discovery。
> Eager policy（T1-T5+T7 都 trigger）配合分段輸出，user 立刻知道哪些主張被驗、哪些是 cache、哪些有 discrepancy。
> v2 的 `/repo-wiki:lint` 會做更深的 git-aware 全庫檢查（lint 跑得慢沒關係，query 必須快但不能不準）。

> [!important] paths frontmatter 是 verification 的支點
> 沒有 entity 的 `paths:`，verification 只能 grep 找——慢且不準。
> Init 跟 ingest 都會自動填 `paths:`（從 git stat），等於實質強制。
> Query 在 paths 缺失時 fallback grep + 給 warning，不擋使用但提示修。
