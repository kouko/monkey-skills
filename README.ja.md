...
| [`loom-workflow`](loom-workflow/) | 4.0.0 | 11 | 4 | git ベースの project memory、`critique`（proposal / complexity）、recap、handoff、session distill、`goal-create`、`independent-advisor` によるクロス executor のセカンドオピニオン、デリバリーアークを intent で回す長期 Outcome Map 向けの loom workflow tools。 |
