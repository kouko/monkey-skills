...
4. **Brief before you ask a complex fork.** When a design fork this router surfaces is genuinely complex, run `dev-workflow:brief-before-asking` first — the trigger threshold and stakes-first framing live in `loom-pipeline/hooks/family-reception.md §Brief before a complex fork` (the single source; reference it, don't copy it).
