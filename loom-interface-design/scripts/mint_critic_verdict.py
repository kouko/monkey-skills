...
`sha256` is over the concatenated bytes of `--files` (typically
DESIGN.md,ui-flows.md for this plugin's artifacts), resolved relative
to `--change-folder` (or used as-is if absolute), in the order given.
Overwrite-in-place: a second `mint` for the same critic replaces its
file (atomic tmp+os.replace), latest wins — round history stays in
the critic's prose Round summary, not here.
...
def main(argv: list[str] | None = None) -> int:
    ...
    Typical `--files` for this plugin's artifacts:
