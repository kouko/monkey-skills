...
**Skills**:
- `/dbt-wiki:init` — first-time setup. Reads `target/manifest.json` (model metadata, refs/sources, schema.yml columns, tests) plus `target/compiled/<project>/**/*.sql` parsed via [sqlglot](https://github.com/tobymao/sqlglot) for column-level lineage. Generates one markdown page per model / source / macro (used) / seed / snapshot / singular test / exposure under `.dbt-wiki/`. Writes `.dbt-wiki/SCHEMA.md`, `index.md`, `lineage.md`, `log.md`, `_internal/extract_column_lineage.py`, plus an idempotent CLAUDE.md drop-in (markers `<!-- dbt-wiki:start --> ... <!-- dbt-wiki:end -->`). Re-runnable: refreshes manifest-derived fields, archives orphans, preserves custom body sections.
