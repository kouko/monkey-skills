...
| Skill | 何時用 | 主要輸入 |
|---|---|---|
| [`/dbt-wiki:init`](skills/init/) | 每專案一次（重跑安全） | `target/manifest.json` + `target/compiled/**/*.sql`（先跑 `dbt parse && dbt compile`） |
| [`/dbt-wiki:refresh`](skills/refresh/) | `dbt parse` / `compile` / `run` 後 model 有變 | 對 `manifest_sha` 做 diff，只更新改動的 page |
| [`/dbt-wiki:query`](skills/query/) | 想問 dbt model 結構 / lineage / 欄位時 | `.dbt-wiki/index.md` + 相關 model page；自動偵測 drift |
...
.dbt-wiki/
  ...
  models/<name>.md       # 每個 model 一份：frontmatter (materialization、欄位含 sqlglot 抽出的 sources、
                         #   depends_on、feeds_into、tests) + body (description、SQL preview、column chain)
  ...
  _internal/extract_column_lineage.py      # sqlglot 助手腳本 (init 寫一次)
