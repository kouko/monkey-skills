...
| Skill | 何時用 | 主要輸入 |
|---|---|---|
| [`/dbt-wiki:using-dbt-wiki`](skills/using-dbt-wiki/) | 不確定該用哪個 skill 時（初次建置／更新／查詢／認證／交付）從這裡開始 | 你的意圖；只負責路由到下面對應的 skill，本身不讀 wiki |
...
| [`/dbt-wiki:rescan`](skills/rescan/) | *進階* — 只更新證據層、0 LLM。`update` 會幫你跑；只有在你明確想跳過語意那半時才單獨用 | 對 `manifest_sha` 做 diff，只更新變動證據頁；受影響的知識頁標 stale；保留 user-owned `## User Notes` 段 |
...
| [`/dbt-wiki:update`](skills/update/) | **維護主動詞**——「把 wiki 一次弄到最新」一個指令。想手動叫 `rescan` / `redistill` 前先想想是不是該用它 | 一趟跑完所有機械步驟：（選擇性）把你帶來的筆記 `ingest` → `rescan` → LLM `redistill`（要明確 yes，且只對 **material**、非 cosmetic 的證據變動開閘）→ **phantom-column lint 閘**（抓出引用了 model 根本沒有的欄位的頁）→ **交棒給 `review`**——列出需要人工認證的頁面佇列後就停住，絕不自行認證 → 結構化成績單 |
...
| [`/dbt-wiki:review`](skills/review/) | 把 LLM 寫的頁面認證為人工已驗證（`developing` → `mature`）。`update` 會幫你排好佇列，這裡是人真正讀過並確認的地方 | review 佇列——`developing` 頁，加上已變 stale 的 `mature` 頁，按風險排序（推測值、landmine、被引用數） |
...
5. 改完 dbt 後：
   ...
   /dbt-wiki:update
   ```
   （`/dbt-wiki:rescan` 是想跳過 LLM 那半時的便宜替代——但預設該用 `update`。）
