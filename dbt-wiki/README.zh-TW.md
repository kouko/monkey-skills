...
| [`/dbt-wiki:refresh`](skills/refresh/) | `dbt parse` / `compile` / `run` 後 model 有變 | 對 `manifest_sha` 做 diff，刷新證據層；受影響的知識頁標 stale；保留 user-owned `## User Notes` 段 |
...
| [`/dbt-wiki:to-sql`](skills/to-sql/) | 想把業務問題**轉成可執行的 SQL 查詢**時（「幫我寫上個月各店 GMV 的 SQL」「寫個查詢算各方案的活躍用戶數」等）。想理解資料用 `query`；想產 SQL 用 `to-sql`。（`query` = 理解資料意義 / 解釋血緣；`to-sql` = 把業務問題變成可執行 SQL） | 自由文字的業務問句；透過知識庫把業務術語解析成物理欄位、組裝 join 路徑，並靜態驗證生成的 SQL |
...
證據層（支撐，機械萃取）
  _evidence/    manifest + sqlglot 萃取的 model / source / macro 結構血緣頁
                是知識層的原料與引用對象；refresh 後若有變動則標知識頁 stale
```
...
5. **Refresh idempotent**——對 `manifest_sha` diff，只更新改動的證據頁；受影響的知識頁標 stale
...
- `refresh` 自動重蒸受影響知識頁（MVP 僅標 stale，重蒸排 fast-follow）
- `ingest` 寫入知識層頁面
