...
| [`/dbt-wiki:rescan`](skills/rescan/) | `dbt parse` / `compile` / `run` 後 model に変更（安価・LLM なし） | `manifest_sha` で diff、変更証拠ページのみ更新；影響する知識ページを stale フラグ；user-owned `## User Notes` は保持 |
...
| [`/dbt-wiki:update`](skills/update/) | 「wiki をまとめて最新に」を 1 コマンドで | `rescan` を実行し、知識ページが stale になったら LLM `redistill` を明示的な yes でゲート |
