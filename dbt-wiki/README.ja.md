...
| [`/dbt-wiki:to-sql`](skills/to-sql/) | ビジネスの問いを**実行可能な SQL クエリとして生成**したいとき（「先月の店舗別 GMV の SQL を書いて」「プラン別のアクティブユーザー数を取得するクエリ」など）。データの意味を理解したいなら `query`；SQL を生成したいなら `to-sql`。（`query` = データの意味・lineage を理解する；`to-sql` = ビジネスの問いを実行可能な SQL に変換する） | 自由形式のビジネス要求文；ナレッジベース経由で業務用語を物理カラムに解決し、JOIN パスを組み立て、生成 SQL を静的検証する |
...
知識ページは自身がどの `_evidence/` ページから蒸餾されたかを引用する。refresh は引用先の証拠が変わったことを検知し、その知識ページを stale フラグする。
