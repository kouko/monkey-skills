...
跑 `legal-playbook-author bootstrap` 可從 bundled fallback 遷移到你自家的客製化 playbook。Phase 1.5 (v0.2.0) 會把 bundled baseline 擴到 8 條（加上 LoL / Indemnification / DPA / IP-Assignment 的 variant-folder 版）。
...
- **Stability**：pre-MVP scaffold；尚未 dogfood 驗證
- **Phase**：1 (MVP shell) — v0.1.0 → v1.0.0 plan 見 [ROADMAP.md](ROADMAP.md)
