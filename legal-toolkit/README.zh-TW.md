...
- **Stability**：MVP shell + Phase 1.5 DSL 基建完成；尚未 end-to-end dogfood 驗證
- **Phase**：1.5 (DSL + scripts + 8-clause baseline) — v0.1.0 → v1.0.0 plan 見 [ROADMAP.md](ROADMAP.md)
- **Test suite**：80+ tests（schema / discover / validate / detect_conflicts / abac_filter / build_baseline / seed_baseline）— `uv run --with jsonschema --with pyyaml --with pytest` 全綠
