...
case_citations_verified_at: 2026-05-12  # v0.3.2+: all previously-shipped judicial case citations verified-or-removed at https://judgment.judicial.gov.tw; if you add a new case citation, pin a case_verified_at:<date> note on the line
owner: "[請編輯為你的姓名]"
...
## 相關規範與學說參考
<!-- v0.3.2 update: previously cited specific judicial decisions; on 2026-05-12
     verification at https://judgment.judicial.gov.tw + tpa.judicial.gov.tw,
     the bundled case-numbers were not located. Replaced with statute +
     subordinate-regulation + commentary pointers per soft-citation rule
     (L7 Step 9.3.0). If you have a verified case to add, please pin
     `case_verified_at: YYYY-MM-DD` on the citation. -->

- **消保法 §17** + 行政院公告「定型化契約應記載及不得記載事項」目錄（依產業別逐項公告，cpc.ey.gov.tw 查詢；SaaS / 線上訂閱目前尚無業別專屬公告，但行政院消保處對自動續約常以「不得記載事項」框架審查）
- **公平交易法 §25** 顯失公平交易條件之一般審查框架（單一雙邊合約須有 market-impact element 始有適用）
- 王澤鑑《債法原理》「定型化契約之解釋與規制」commentary（自動續約條款之 §247-1 / §148 信義原則檢驗）
- ⚠️ 引用具體判決前，請於 https://judgment.judicial.gov.tw 檢索查證；若有可驗證之 anchor case，請以 `case_verified_at` frontmatter 追蹤
