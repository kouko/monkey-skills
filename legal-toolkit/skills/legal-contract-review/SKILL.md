...
**Signal split** (v0.3.5+ Phase 1.9): the `[!danger]` block lists **only real legal high-risk** findings (walk-away / risk_default red / low-confidence / criminal liability / threshold deals / cross-border 個資). **Placeholder warnings** (`escalate_to` still `[請編輯…]` bundled-fallback default) move into a SEPARATE `[!warning]` block below the danger block. v0.3.4 audit caught that mixing ops-level placeholder noise into high-risk signal diluted the `[!danger]` warning's credibility; v0.3.5 splits them so each signal carries its own weight.
