...
當 `state.json.forced_stop == true` 或 query 涉及 **刑事 / 訴訟 / 跨境 / 重大金額** 時，`executive-summary.md §Escalation` 同樣 hard-wired（§6.4 Escalation Override）— LLM 不能 soften 也不能 skip 律師 建議。
