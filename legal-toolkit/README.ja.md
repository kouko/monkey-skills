...
legal-toolkit は社内法務（個人 / SME / 上場中堅）に対して、**Markdown-first / runtime-portable** な workflow を提供する。対象は in-house 業務時間の高頻度 80% — **契約レビュー**（主戦場）、**playbook authoring**（自社の交渉ルール）、**文書 drafting**（privacy / ToS / DPA / NDA；v0.4.0+）、**incident response**（個資外洩 / 主管機關函覆 / 合約違約；v0.4.2+）、**issue spotting**（IRAC fact-pattern 分析；v0.5.0+）、**legal research**（iterative-search + 三角驗證 + document-level citation；v0.5.2+）、および (Phase 4-5 予定) 契約 lifecycle tracker / regulation watch / corporate governance / DD quickscan。
...
| Skill | 役割 |
|---|---|
| `using-legal-toolkit` | **Router** — 6 cluster（Playbook / Template / Runbook / IRAC / Tracker / Compliance）にわたる意図を識別、dispatch する。残る Phase 4-5 の sub-skill は not-yet-available として明示。 |
...
| `legal-contract-review` | **📋 Playbook cluster — 主戦場** — TW overlay 付きの 7 層 pipeline。ABAC pre-filter が LLM 評価前に matched variant を選ぶ（LLM は単一 variant のみ参照、選択させない）。3 mode：`review`（full output）/ `redline`（substitute clause text に focus）/ `nda`（簡易構造のため L2-L3 skip）。出力：`legal.md` + `business.md` + `findings.json`（v0.3.4+ で統合）。 |
| `legal-document-draft` | **📝 Template cluster（v0.4.0+）** — 4 mode：privacy / ToS / DPA / NDA。Skeleton-and-LLM-fill template、現行台湾法 pin（Path A discipline）。2-file output（`<doc-type>.md` 公開可 + `compliance.md` 法務 review）。Hand-curated per-mode 合規 checklist と法条引用（個資法 / 民法 / 消保法 / 公平交易法）。 |
| `legal-incident-response` | **🚨 Runbook cluster（v0.4.2+）** — 3-path classifier（個資外洩 / 主管機關函覆 / 合約違約）+ auto-classify + confirm path routing。Audience-shaped 2-file output（`legal.md` + `business.md`）+ optional `handoff-context.json`（合約違約 path で `legal-contract-review` に handoff）。ISO 8601 timeline section（未確定 anchor は ⏳ marker）。 |
| `legal-issue-spot` | **🔍 IRAC cluster, sub-skill 1 of 2（v0.5.0+）** — pure-LLM IRAC issue-spotting。business-language fact pattern（「我們想做 X，能不能做？」）から issue 矩陣 + per-element 構成要件 涵攝 + reverse-side counterfactual + 🔴/🟡/🟢 risk grade + escalation 建議 を生成。2-file output（`issues.md` for 法務 / `business.md` for 業務 + escalation 建議）。外部 fetch なし、profile.yml 依存なし、playbook 結合なし — `legal-contract-review` に対して直交。⚠️ low-confidence subsumption 検出時は `/legal-research` へ具体的 query string と共に soft handoff。 |
| `legal-research` | **🔍 IRAC cluster, sub-skill 2 of 2（v0.5.2+）** — 法源 citation research with **Agent 抽象**（plan-adapt-interact 半互動 — plan-first、Y/n 確認後に autonomous loop 突入）。**WebFetch のみ**（Python scraper なし；fallback chain = 一次 source URL → Google cache → archive.org Wayback）。4-protocol pipeline（plan → iterative-search → triangulate → cite）。Loop cap ≤ 5 round OR ≤ 30 fetch；早期終了は source ≥ 8 件 AND 法源類型 ≥ 2 種に達した時；`forced_stop` には ⚠️ marker。4-file output（`plan.md` / `state.json` / `research-memo.md` for 法務 / `executive-summary.md` for 業務）+ Harvey-style document-level citation manifest（3-line shape：URL + title + access date）。§6.4 Escalation Override の trigger を `forced_stop` / 刑事 / 訴訟 / 跨境 / 重大金額 に拡張。 |

Phase 4-5 でさらに 4 skill 追加 — v0.1.0 → v1.0.0 plan / version strategy / critical path / risk callouts は **[ROADMAP.md](ROADMAP.md)** 参照。
...
- **Stability**：7 active skills（Phase 1 MVP + Phase 2 Template/Runbook + Phase 3 IRAC 完全 close）
- **Phase**：3 SP3-b（legal-research ship）— IRAC cluster 完全 active（issue-spot v0.5.0 + research v0.5.2）；Phase 4 Tracker（v0.6.0）が次；v0.1.0 → v1.0.0 plan は [ROADMAP.md](ROADMAP.md) 参照
- **Test suite**：259+ tests（schema / discover / validate / detect_conflicts / abac_filter / build_baseline / seed_baseline / self_grade / grade_draft / grade_response / grade_issue_spot / grade_research / canonical drift 4-grader bank）— `uv run --with jsonschema --with pyyaml --with pytest` で全 green
