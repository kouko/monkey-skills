...
![version](https://img.shields.io/badge/version-0.4.1-blue) ![license](https://img.shields.io/badge/license-MIT-green) ![phase](https://img.shields.io/badge/phase-2_Template_dogfooded-orange)
...
- **Stability**：MVP + Phase 1.5 DSL + Phase 1.6 Eval 基建完了；dogfood calibration は owner-run（ship-blocking ではない）
- **Phase**：1.6 (Eval rubric + self_grade.py + dogfood 手順) — v0.1.0 → v1.0.0 plan は [ROADMAP.md](ROADMAP.md) 参照
- **Test suite**：110+ tests（schema / discover / validate / detect_conflicts / abac_filter / build_baseline / seed_baseline / self_grade）— `uv run --with jsonschema --with pyyaml --with pytest` で全 green
