...
- Bundled NDA-native fallback baselines（current 4 fallback 偏 SaaS-shaped）
- ANS-21 stance_asymmetry_check 由 self_grade.py 從 protocol-only 升級到 semantic-tier rubric
- Fallback baseline 相關判例 audit + 標注（v0.3.1 ships statute_verified_at but not 判例_verified_at）
