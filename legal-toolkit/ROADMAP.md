...
## Phase 1.8 — Output consolidation（v0.3.4，1 天）✅ **DONE 2026-05-12**

**Scope**：UI/UX cleanup — output file count 7 → 3；audience-shaped 重新組織；**不新增 skill，不改 pipeline 邏輯**。

**Trigger**：2026-05-12 conversation, user 提出「能否留法務看的 + 非法務看的兩種 markdown 就好」。Phase 1-1.7 累積下來 7 個 output files 中，5 個是 findings.json 的 rendered view，內容重複 70%，每個 audience 只看 1-2 檔，其餘是噪音。

### Audience-shaped 重組

| Output | 受眾 | 內容（吸收 v0.3.3 哪些檔）|
|---|---|---|
| `legal.md` | 法務 / GC / ops 簽核 | issues + escalation + memo-legal + redline (送對方版 + 內部 fallback) + favorable + self-grade summary |
| `business.md` | 非法務 (CEO / BD / 業務 / 老闆) | memo-business + Top 3 + favorable + **主要 redline 重點（完整 條文 + why；user 強調這是重點不能砍）** |
| `findings.json` | Machine + self_grade.py | Canonical SoT；v0.3.4 新增 `self_grade` block 吸收 self-grade.md |

→ 7 → 3 outputs；user-facing footprint 從 ~60-100 KB → ~30-50 KB（**砍 50-60%**）

### Banner 規則簡化

- v0.3.0：全 5 個 .md 都套 banner（dogfood audit 評：banner spam）
- v0.3.1：narrow 到 memo-legal + escalation 兩檔
- v0.3.4：**只 legal.md**（business.md 永遠不帶，non-lawyer audience 不該被噪音訓練略過 banner）

### 完成狀態（commits c3b7450 → \<commit-2\> on `feat/legal-toolkit-v0.1.0`）

**commit 1** — output consolidation (feat/legal-toolkit)：
- SKILL.md §Outputs / Mode dispatch / §Disclaimer & Override / §Schemas / §Output contract 全 rewrite
- L7-evaluate.md 新增 Step 10 (Render outputs)；全 cross-reference 從 `<file>.md.<key>` 改成 `findings.json#<key>`（rendered in `legal.md §<section>` or `business.md §<section>`）
- answer-criteria.md ANS-04/05/06/07/09/10/11/14/17/20 row text 全 align v0.3.4
- source-criteria.md self-grade.md references → findings.json#self_grade
- escalation-override.md HTML comment 描述新 banner placement 規則
- self_grade.py OUTPUT_FILES 5 → 2；ANS-05 helper 檢 legal.md；ANS-06 evidence 改字；main() 預設改 update findings.json#self_grade in-place 不寫 self-grade.md；main(argv) accepts optional argv
- tests/test_self_grade.py _write_outputs 改 5 → 2 檔；test_ans_06 loop 改；+4 new v0.3.4 tests（writes_to_findings / format-markdown-no-file / ans-05-legal-md / ans-05-missing-banner）

**commit 2** — version bump + ROADMAP（chore/legal-toolkit）：
- plugin.json + marketplace.json + 3 READMEs v0.3.3 → v0.3.4
- README badge phase: `1.7_Runtime` → `1.8_Outputs`
- ROADMAP Timeline + 版本策略 + 本 section

### Quality gate

- ✅ **167/167 tests pass**（163 + 4 new v0.3.4 tests）
- ✅ Mode dispatch 邏輯 unchanged（pipeline 不動）
- ✅ Banner scope 邏輯一致（legal.md only / business.md never）
- ✅ self_grade.py 行為改變被 4 個新 tests 鎖住
- 🟡 **dogfood A/B 待跑** — pre/post Phase 1.8 on 2 contracts，verify (a) legal.md 內容 unfragmented 跟 lawyer mental model 一致 (b) business.md 完整 redline section 對非法務真的有用 (c) 沒有 regression vs v0.3.3 7-file output

### 仍 deferred（v0.3.5+ / Phase 1.9）

- 舊 5 個 fragment schemas（issues / redline / memo-legal / memo-business / escalation）內聯入 output-schema-findings.json（目前還是 $ref 來保 chain 完整）
- output-schema-self-grade.json 移除（self_grade block 已寫入 findings.json）
- `legal-outputs/<timestamp>-<slug>/` 目錄結構是否要加 `.md.bak` 給 user 對比舊版輸出（用戶 muscle memory friction mitigation）
- Markdown-level schema validators for legal.md + business.md（runtime ANS-05/06 已 enforce 核心 structural rules；fully schema-driven verify 是 nice-to-have）
...
### v0.4.1 (Dogfood patches) ✅ **DONE 2026-05-13**

Self-dogfood of v0.4.0 on 2 hypothetical use cases (墨韻設計 — privacy + nda) found 1 P0 + 3 P1 + 3 P2:

**P0** — `template-privacy.md:102` 殘存 pre-2020-amendment 「未滿二十歲限制行為能力人」，而 SP3a 的 SP2 research note (`research/2026-05-12-pdpa-2025-11-verify.md:168`) literally locked 「滿7歲未滿18歲 per 民法 Art. 12 as amended in 2023」。 Author transcribed legacy value; structural grader + hand checklist both PASS-ed (compliance check tests "did you cite 民法" not "is the age correct"). Fixed to 「未滿十八歲」+ statute-citations.md amended in lockstep.

**P1a** — Add Path A anti-pattern grep to `grade_draft.py`：4 high-confidence regexes (`未滿二十歲` / `72 小時` / `controller-processor` / `資料控管者`) checked against `<doc-type>.md` only (compliance.md still allowed to cite anti-patterns in 「NOT X」 explanatory form). 7 new parameterized tests + 1 compliance-only acceptance test. This grader-layer catches the same class of bug that slipped P0 through.

**P1b** — `template-nda.md:51` authoring note 「典型範圍 3-7 年；技術機密可延至 10 年或永久」 was rendering into the delivered counterparty NDA. Moved to HTML comment. Also dropped undocumented `{{survival_years_explanation}}` variable (was in template but missing from `protocols/draft.md` ASK_GAPS list — P2 #1 absorbed here).

**P1c** — `template-privacy.md:34` 「如蒐集 §6 特種個資 ({{special_category_handling}})，將另行書面同意。」 rendered awkwardly when user answer was 「不主動蒐集...」 (self-contradicting parens). Restructured to clean heading + paragraph form.

Deferred to v0.4.x backlog (P2): compliance.md checkbox flip convention `- [ ]` → `- [x]` codification in protocol; deterministic `assets/ask-gaps-labels.yml` for human-friendly variable labels.

### Quality gate

- 201 tests pass (was 194 + 7 new parameterized anti-pattern tests)
- New grader rerun on yesterday's dogfood output catches the 「未滿二十歲」 regression that previously passed
...
Pure-LLM IRAC issue-spotting sub-skill. Subagent-driven plan executed in 1 session.
