...
| [`goal-create`](skills/goal-create/) | goal condition を作成する — SESSION はホストが対応する場合に 4 項目ゴールを有効化し、非対応時は手動コマンドを示す。ARC は repository の purpose artifact（`Why` / `Done when`）を起草する。 |
