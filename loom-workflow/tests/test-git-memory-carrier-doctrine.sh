...
#
# Pins the O4 carrier-hierarchy doctrine in loom-workflow/skills/git-memory/
...
#
# Usage:
#   bash loom-workflow/tests/test-git-memory-carrier-doctrine.sh
...
if grep -ril "durable substrate" "$DEV_WORKFLOW_DIR" 2>/dev/null \
  ...
  fail "'durable substrate' still present somewhere in loom-workflow/"
else
  pass "'durable substrate' absent from the whole loom-workflow plugin (excl. this test)"
fi
