...
**Scope**: loom-workflow plugin's family of skill-modifying skills.
**Created**: 2026-04-29
...
|---|---|---|
| **H1** (Option II baseline) | skill-creator-advance scope; skill-refactor; skill-tuning | ✅ Complete |
| **H2** (Layer 2 upgrades to refactor / tuning) | Multi-judge ensemble (refactor); structured equivalence; constitutional pre-filter (tuning); blind A/B; per-iteration human gate | ✅ Complete |
...
| skill-tuning: 1 real-skill walkthrough validating A/B flow | PR-3 | OUTSTANDING | quarterly-audit-runbook step 5 |
...
**Add two new skills to loom-workflow** as siblings to `skill-creator-advance`:

- `skill-refactor` — Phase A: token / structure refactor with auto equivalence check + git ratchet
- `skill-tuning` — Phase B: human-judged A/B for output quality
...
| Major rewrite (add/split/merge phases, change agent decomposition, change input/output contract) | `skill-creator-advance` | Spec re-审 + full eval; refactor/tuning can't reach |
...
| Output quality improvement / A/B variant exploration / **human-judged taste calls** | `skill-tuning` | Human preference accumulates real signal |
...
| AI-judge blind A/B (existing capability) | `skill-creator-advance` Advanced (comparator agent) | Existing optional advanced tool; not duplicated by skill-tuning |
...
| "test outputs" / "A/B variants" / "improve writing style" / "compare different phrasings" / "我來評哪個比較好" | **skill-tuning** | Human judge + variant exploration signal |
...
(a) Token reduction / structure cleanup with output behavior unchanged
    → Hand off to `loom-workflow:skill-refactor`. Do not handle here.
    
(b) Output quality / A/B variant exploration with human judgment
    → Hand off to `loom-workflow:skill-tuning`. Do not handle here.
...
This section is fine as-is, but should add a note distinguishing from skill-tuning:

> The blind comparator uses a subagent as judge (LLM-as-judge). For taste-sensitive output where LLM-judge is known to be unreliable (creative writing, voice, design feel), use `loom-workflow:skill-tuning` instead — that skill uses human judgment per iteration and accumulates a preference log.
...
Required components:
- `loom-workflow/skills/skill-refactor/SKILL.md` — Iron Law / Gate Function (equivalence Q + token Q + invariant Q) / verdict (PROCEED / RESHAPE / REJECT) / refactor hat editor prompt
- `loom-workflow/commands/skill-refactor.md`
- LICENSE / NOTICE if any upstream attribution
- `loom-workflow/skills/skill-refactor/README.{en,ja,zh-TW}.md`
...
### Phase D — implement skill-tuning (higher uncertainty)

Required components:
- `loom-workflow/skills/skill-tuning/SKILL.md` — variant-generation editor prompt, A/B harness protocol, preference logging
- `loom-workflow/commands/skill-tuning.md`
- LICENSE / NOTICE
- `loom-workflow/skills/skill-tuning/README.{en,ja,zh-TW}.md`
...
**Validation gate before merging**: pick a real skill the user has wanted to improve, run skill-tuning on it manually (without the skill itself, as a process), see if the workflow produces value. If the manual version doesn't produce value, the skill version won't either.
...
| 2 | Implement skill-tuning? | (a) Yes — Phase D; (b) Defer — manual A/B works for now; (c) No — comparator agent + human review of comparator output is enough |
...
| 5 | LICENSE / attribution for skill-tuning? | (a) MIT, attribute to darwin-skill where applicable; (b) MIT, no attribution; (c) Other |
...
| 7 | Test prompt registry strategy | (a) Per-skill `test-prompts.json` like darwin; (b) Centralized at loom-workflow level; (c) Skip until first user |
...
| 10 | Should skill-tuning also accumulate test-prompt registry? | Sharing with skill-refactor's registry would be efficient; but they test different things. |
...
Add explicit "refactor mode" and "tuning mode" inside skill-creator-advance. Single skill, multiple modes.
...
### Alternative 2: Adopt darwin-skill wholesale into loom-workflow

Install `alchaincyf/darwin-skill` as-is or fork it into a 5th loom-workflow skill.
...
### Alternative 3: skill-refactor only, skip skill-tuning

The refactor case has a robust equivalence check; the tuning case has uncertain ROI. Ship A only.
...
### Risk: skill-tuning is too expensive to run

Human A/B every iteration is high cost. If users only run it once and abandon, the dataset accumulation benefit is lost.

**Mitigation**: scope skill-tuning to *high-value skills only* (skills the user runs daily, skills with creative output, skills used by multiple people). Don't recommend running on every skill.

---

## 9. Out of Scope

- Auto-detect which skill (refactor / tuning / creator-advance) to use based on user prompt — that's the description / trigger system's job; not solved here
- Cross-plugin: this stays inside loom-workflow; no coupling with domain-teams beyond what already exists
...
- Building automatic preference-judge from accumulated skill-tuning log — possible future work; out of scope for v1
...
- [ ] SKILL.md ≤4500 words
- [ ] Frontmatter description with negative triggers vs skill-tuning / skill-creator-advance
- [ ] Iron Law + Gate Function with three explicit checks (equivalence / token reduction / invariant preservation)
- [ ] Verdict vocabulary parallel to other loom-workflow critique skills (PROCEED / RESHAPE / REJECT)
...
| Refactor verdict vocabulary (PROCEED / RESHAPE / REJECT) | `loom-workflow/skills/proposal-critique` (KEEP / DEFER / DROP), `complexity-critique` (PROCEED / PROCEED-WITH-CAVEAT / RESHAPE / REJECT) |
...
| Human-anchor preference (skill-tuning) | `copywriting-toolkit/voice-anchors` (90 anchors curated by human) |
