...
> ゴール条件を作成する。**SESSION** はホストに受理された場合に 4 項目ゴールを
> 有効化し、それ以外は正直な復旧操作を示す。**ARC** はリポジトリの
...
- **SESSION モード** は、長時間実行エージェントの run が照合される
  ...
  その後、受理された場合は native Goal を有効化し、それ以外はまだ有効でない
  ことを明示して復旧操作を示す。Codex に未完了 Goal がある場合は
  `/goal clear` 後に再実行し、能力がない場合は手動の `/goal` を 1 つ示す。
