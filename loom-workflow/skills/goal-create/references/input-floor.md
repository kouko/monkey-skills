...
only. The one standing exception is the **Standing decision rule** in
`goal-shape.md` §2 — a fixed `Constraints` entry SESSION mode emits by
default, whose `derived` anchor is that section rather than conversation
evidence.
