...
動かすには `loom-workflow` のインストールだけで足りる。同じ 4 つの
...
## loom-workflow の中での位置

loom-workflow の「critique」ラインは現在こうなっている：
...
joshuadavidthomas/agent-skills（MIT、original）
  → softaworks/agent-toolkit/skills/reducing-entropy（MIT、fork）
    → kouko monkey-skills/loom-workflow/complexity-critique（本ファイル）
```
...
MIT — [repository root LICENSE](https://github.com/kouko/monkey-skills/blob/main/LICENSE) と skill レベルの
