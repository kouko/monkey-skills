...
**Required preamble**, carried forward verbatim from the upstream
...
1. List the 4 files under `references/` in this skill directory.
2. Read the opening section of each to see which mindset applies
   to the change in front of you.
...
**Do not proceed to Q1 until you've done this.**

This skill is fully self-contained: the 4 mindsets are bundled at
`references/` and require no other plugins to be installed. If
`domain-teams:code-team` is available, the canonical SSOT versions
of the same mindsets live there with richer cross-references to
mechanical standards (Pragmatic Programmer, SOLID, Clean Code,
Refactoring) — but they are not required for the gate to run.
...
loaded per §Before You Begin). All four are bundled in this skill's
`references/` directory; canonical SSOT versions live at
`domain-teams:code-team/standards/mindset-*.md` and are used by
code-team's brainstorming / refactoring protocols. The bundled
copies carry a header note pointing back to the SSOT.

| Mindset (bundled at `references/`) | Core principle | Use when… |
|---|---|---|
...
Mindsets are bundled, not delegated — this skill is installable and
fully functional with just loom-workflow installed. The SSOT-and-copy
arrangement is a drift-management policy: edits land in
`domain-teams:code-team/standards/` first; the bundled copies in this
skill are updated to match in the same PR. Adding a 5th mindset is
governed by `domain-teams:code-team/standards/mindset-extension-standard.md`.
