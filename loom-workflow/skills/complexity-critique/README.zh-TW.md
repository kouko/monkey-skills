...
此 skill **完全自包含** — 只需安裝 `loom-workflow` 即可運作
...
## 它在 loom-workflow 中的位置

loom-workflow 的「critique」線目前是：
...
joshuadavidthomas/agent-skills（MIT，原始）
  → softaworks/agent-toolkit/skills/reducing-entropy（MIT，fork）
    → kouko monkey-skills/loom-workflow/complexity-critique（本檔）
```
...
MIT — 請見 [repository root LICENSE](https://github.com/kouko/monkey-skills/blob/main/LICENSE) 與 skill 層
