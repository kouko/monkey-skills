...
| Situation | Skill |
|---|---|
| Lost the thread **mid-session** (you're still in the conversation) | `loom-workflow:recap-state` (L3 in-session re-orientation, chat output only) |
| **Ending a session**; want a cold AI to resume later | `loom-workflow:handoff` prepare mode |
| **Starting a new session**; loading prior state | `loom-workflow:handoff` resume mode |
...
`loom-workflow:recap-state` is L3 — it writes nothing to disk, targets a warm human
reader still inside the session, and produces a 7-block chat summary.

`loom-workflow:handoff` is L2 — it writes a file to `.claude/handoffs/`,
...
`loom-workflow:handoff` always writes HANDOFF files to `.claude/handoffs/`.
